...
%TODO: Intro to sequential decision making. 
...
%\cite{DBLP:journals/jmlr/JainKT12,DBLP:conf/nips/ThakurtaS13,DBLP:conf/uai/MishraT15,tossou:aaai2016} have observed the importance of privacy to MAB applications. 
...
%In both global and local contexts, differential privacy is achieved by the addition of noise. 
...
%presence or absence of a particular interest, as well as exact birthday 
...
%TODO Prctially revealnt problems, extension to non-stioanry setting as done in data collection by https://papers.nips.cc/paper/2018/hash/a01610228fe998f515a72dd730294d87-Abstract.html
...
%If the mean is a sufficient statistic of the reward distribution, then the learner can build their own corruption function from the corruption scheme and the two definitions are equivalent. 
%The following is an example of a corruption scheme.
...
%when the mean reward is a particular value. 
...
%Pratik comment: R was being used for both reward and regret so I changed regret to \operatorname{Regret} 
$
...
%where $N_a(T) \defined \sum_{t = 1}^T \ind_{(\ArmAt_{t} \IsEqual a)}$ denotes the number of pulls of arm $a$ up to time $T$ and $\Delta_a \defined \mu_1 - \mu_a$ i.e. the gap between the optimal mean reward and the mean reward of arm $a$.
...
%Extending the work of \citet{pmlr-v83-gajane18a}, 
To solve the problem at hand, we propose \SWKLUCBCF,
%(see Algorithm~\ref{algo:SWKLUCBCF})
...
%i.e.,  $d(x,y) \defined \mathrm{KL}(\cB(x),\cB(y))$. 
...
%The quantity $\hat{\lambda}_a(t)$ in the algorithm denotes the empirical mean of the feedback observed from arm $a$ until time $t$: $\hat{\lambda}_a(t) \defined \frac{1}{N_a(t)}\sum_{s=1}^t F_s \cdot \ind_{(\ArmAt_s = a)}$. 
...
%(see for example, \citet{pmlr-v99-auer19a}).
...
\begin{flushleft}
\textbf{Input:} 
%A bandit model with a set of arms $ A \defined \{1, \dots, K\}$ with unknown mean rewards and unknown mean feedbacks and monotonic and continuous corruption functions $g_1, \dots, g_k$, 
...
\label{sec:privacy}
%As done in \citet{pmlr-v83-gajane18a}, a straightforward approach to achieve differential privacy using corrupt bandits is to employ a differentially private mechanism on the user feedback.\
...
%We shall use the following corruption scheme to provide local differential privacy. 
...
%
%By ensuring the appropriate values for the parameters of randomized response, users can send differentially private feedback to the learner. The learner can then employ \SWKLUCBCF \ to learn from such feedback in a non-stationary environment while maintaining privacy. 
%
...
%given in \citet[Eq. (15)]{JMLR:v17:15-135} for binary rewards and feedbacks. 
%Moreover, this 
...
%At one extreme, for $\epsilon=0$, feedbacks are independent of rewards and learning about rewards from feedbacks is not possible. 
%This can be verified by substituting $\epsilon = 0$ in Eq. (\ref{eq:DPmatrix}) as the resulting corruption scheme makes $p_{00} = p_{11} = 0.5$. 
%On the other extreme, for $\epsilon= \infty$, feedbacks can be made equal to rewards.   
...
\end{align*}
%For the purpose of this proof, we further decompose the index computation with $\mathrm{Index}_a(t)$ defined as follows: 
%$$
%\defined
%    \begin{cases}
%      g_a^{-1}({\ell_a(t)}) \\
%      \quad \text{ with } \ell_a(t)   \defined  \min \left\{q: N_a(t, \windowsize)\cdot{}d\left(\hat{\lambda}_a(t, \windowsize), q\right) \leq f\left( \min\left\{t, \windowsize\right\}\right)\right\} \\ 
%      \qquad  \text{ if } g_a \text{ is decreasing}, \\
%      g_a^{-1}({u_a(t)}) \\ \quad \text { with } u_a(t)  \defined   \max \{q: N_a(t)\cdot{}d(\hat{\lambda}_a(t), q) \leq f(t)\} \\
%      \qquad \text{ if } g_a \text{ is increasing,}
%    \end{cases}
%$$
%
...
\begin{enumerate}
    ...
    %following the corresponding algorithm given for stationary stochastic corrupt bandits in \citet{pmlr-v83-gajane18a}.
    \item Propose a (near-)optimal differentially private algorithm which does not need to know the number of changes.
    %as done for non-stationary stochastic classical bandits in \citet{pmlr-v99-auer19a}. 
    %This is done using a change point detection procedure similar to \citet{hartland:hal-00113668}
\end{enumerate}
...
%\begin{figure}[h]
%\centering
%\includegraphics[width=0.6\linewidth]{img/KL-indices.pdf}
%\caption[width=.7\linewidth]{KL indices calculation.}
%\label{fig:KLindices}
%\end{figure}
...
\end{align}
%PG: Should the s go from 1 to w or from 1 to min(t,w)
...
% where the last inequality follows from Lemma \ref{lem:IndCountNextArm}
