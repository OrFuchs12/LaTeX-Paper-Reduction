...
\vspace{-0.5cm}
\end{figure}
%The participant and the robot were sitting facing each other with twelve markers placed on a table between them. 
%The upper side of the markers represents the fiducial marker (see figure \ref{pic3}), which will be used to generate and indicate the marker ID using Augmented Reality (AR) functionality; the AR will not be visible to the participant as it is solely for the use of the experimenter and the robot. The underside of the marker indicates the symbol which the marker represents e.g. a picture of an apple. 
