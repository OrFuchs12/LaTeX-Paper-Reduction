...
%\jb{Generating human-like questions gets increasing attention of both academy and industry due to its various uses.}
...
% * <jbshin@snu.ac.kr> 2018-09-05T21:37:40.320Z:
%
% > \jb{Generating human-like questions is gaining increasing attention in both academy and industry due to the variety of its uses.}
%
%
% ^.
% * <jbshin@snu.ac.kr> 2018-09-05T04:16:42.087Z:
%
% > as a way
%  , as a way   to blah~ or to blahblah~   ?
%
% ^.

% Automatic question generation from a passage is especially challenging owing to its unstructured nature. One of the most fundamental problems is how to incorporate the question target, which is referred to as the target answer, along with the passage. Because there are several different target answers within a passage, it is natural to allow the NQG models to be aware of the target answer. For example, given the passage in Figure \ref{fig:replace_a_token}(a), we can ask about the person John Francis OHara or about the year 1934. Previous NQG models are mostly based on the RNN encoder-decoder architecture, which is referred to as the sequence-to-sequence model henceforth. Given that RNNs do not have the ability to model high-level variability \cite{serban2017hierarchical},  those NQG models which do not take the target answer into account may generate unintended questions \cite{du2017learning}.
...
% * <jbshin@snu.ac.kr> 2018-09-06T06:29:54.647Z:
%
% > \kj{unintended questions}
% for example
%  reference     ()
%
% ^ <jbshin@snu.ac.kr> 2018-09-06T09:16:28.003Z.
% * <jbshin@snu.ac.kr> 2018-09-06T06:28:24.086Z:
%
% > \kj{multiple target answers}
%    ?
%
% ^.

% \begin{figure}[t]
% 	\includegraphics[width = 0.48 \textwidth]{intro_example5.png}
%   	\caption{
%     (a) A sample of passage, target answer and the golden question. (b) An incorrectly generated question which ignores the given target answer. (c) Replace the target answer with special \textless a\textgreater{} token.
% %     \jb{An example of overall idea for QG in this paper: by replacing the answer into a special mask token within the passage, we could raise the possibility of generating the desired question rather than the improper question in NQG models.}
%     }
...
% \kj{This observation inspires us to introduce a new approach of answer separation in NQG.}
% * <jbshin@snu.ac.kr> 2018-09-06T06:54:29.289Z:
%
% > \kj{This observation inspires us to introduce a new approach of answer separation in NQG.}
%
% +  masking  answer inclusion problem   .
%
% ^ <jbshin@snu.ac.kr> 2018-09-06T09:16:17.703Z.

% The first step in our model is answer separation. With a simple pre-processing of the data, we extract the target answer in the input passage and replace the corresponding target answer with the special \textless answer\textgreater token. This approach to separate the target answer from the passage helps NQG models in two ways. First, it helps the model to identify the question type related to the target answer. Second, the model learns to capture the position and contextual information of the target answer with the help of the \textless answer \textgreater token. Further inspired by how people keep the target answer in mind when they ask questions, we propose \textbf{keyword-net} as another part of answer-separated seq2seq to make the model consistently aware of the target answer. Moreover, keyword-net extracts key information from the target answer, supplementing the information deficiency caused by replacing the target answer in the passage. Lastly, we adopt a retrieval-style word generator proposed by \cite{ma2018query} which better captures the word semantics during the generation process.
...
% * <jbshin@snu.ac.kr> 2018-09-06T06:47:26.439Z:
%
% > \kj{both sides}
%
%
% ^ <jbshin@snu.ac.kr> 2018-09-06T09:16:07.183Z.
...
% * <jbshin@snu.ac.kr> 2018-09-06T06:43:59.278Z:
%
% > \kj{effect}
%  ,    -> benefits
%
% ^ <ad26kt@gmail.com> 2018-09-06T09:14:40.230Z.
% * <jbshin@snu.ac.kr> 2018-09-06T06:42:31.919Z:
%
% > \kj{Furthermore, a machine comprehension system, which is only trained with the questions generated by our model, achieved a comparable result.}
% Furthermore, by training a machine comprehension system on the synthetic data generated by our model, we verify that our model has an enough ability to generate natural and fluent questions.
%
% ^.

%the \textless a\textgreater{} token actually give most of it's attention weights to the interrogative word. Further, we find that with our \textcolor{red}{Model}, the recall of interrogative words predictions significantly outperform the baseline model. Furthermore, only with questions generated with our model, the extractive QA system achieved comparable result. %compared to full data setting.
...
%Although \cite{duan2017question} explicitly generated questions \textcolor{red}{for QA}, \cite{tang2017question} tried to utilize the question generation for question answering tasks (and vice versa) implicitly using the duality of the two tasks as a regularization.
...
% * <kindaichi7207@gmail.com> 2018-09-06T06:50:37.821Z:
%
% > token.
% tokens? ...
%
...
% * <kindaichi7207@gmail.com> 2018-09-06T06:51:26.188Z:
%
% >  \cite{bahdanau2014neural} is combined with the encoder-decoder model,
%
% which attention mechanism(encoder decoder model )
%
% ^ <kindaichi7207@gmail.com> 2018-09-06T06:52:36.279Z.
...
% * <kindaichi7207@gmail.com> 2018-09-04T06:34:17.275Z:
%
% > 'CEO' in 'Apple CEO'
%      .   question   ?  keyword apple?
%
%
% ^ <ad26kt@gmail.com> 2018-09-04T10:59:48.672Z.
...
% * <kindaichi7207@gmail.com> 2018-09-04T02:25:51.541Z:
%
% > we further develop a new architecture which can generate words by querying distributed word representation
%   wean      new architecture...  ...
%
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-04T07:18:30.007Z:
%
% > when the generated token is  \textless EOS\textgreater{}.
%  model generates EOS token
%
%
% ^ <ad26kt@gmail.com> 2018-09-04T11:37:30.313Z.

%\textcolor{red}{As most of the NLP systems take words as basic unit of input/output, it is one of the most crucial issue that how many words the system can cover\textcolor{purple}{(pointing the unknown words)}. In the majority of cases, we pick K-most frequently appeared words in the training corpus and substitute the other words with \textless UNK\textgreater\ token.}
...
% * <kindaichi7207@gmail.com> 2018-09-04T07:24:58.126Z:
%
% > main
% common standard    automatic
%
% ^ <ad26kt@gmail.com> 2018-09-05T05:56:05.495Z.
...
% * <kindaichi7207@gmail.com> 2018-09-04T07:27:10.622Z:
%
% > Meteor compares the candidate with reference in terms of exact, stem, synonym, and paraphrase matches between words and phrases.
%
%
% ^ <ad26kt@gmail.com> 2018-09-04T12:17:05.967Z.
...
% * <kindaichi7207@gmail.com> 2018-09-05T13:34:01.351Z:
%
% > This is a very natural result
% This result can be explained by the characteristic of the answer-separated decoder, which utilizes knowledge of the target answer. Hence, without the answer-separated decoder, the model can use only the position information of the answer, not the knowledge of the answer at all.
%
%   !!!!
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T13:31:49.222Z:
%
% > This results from the fact that
% This is because
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T13:28:41.626Z:
%
% > fact
% -> assumption hypothesis
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T13:27:35.824Z:
%
% > from including the answer in the generated question.
% generated question from including the answer.
%
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T13:22:08.447Z:
%
% > separate utilization of target answer information
% exploiting target answer information separately
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T17:34:32.905Z:
%
% > Grasp of Target Answer
%   prediction, attention weight  ,
% Answer copying frequency of target answer in generated question(   )
%
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:22:38.463Z:
%
% > . \hh{Since \cite{du2017learning} ignores the target answer, we choose seq2seq+AP to represent \cite{du2017learning} with answer position feature. } Further,
%    seq2seq+AP         Besides the baseline model, we also choose  further    ?
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T17:37:58.637Z:
%
% > ignores
%      ?
% do not use?
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T13:52:00.738Z:
%
% > percentage
% ratio
%
% ^ <kindaichi7207@gmail.com> 2018-09-05T14:45:16.871Z.
...
% * <kindaichi7207@gmail.com> 2018-09-05T14:31:45.072Z:
%
% > figure out an interesting observation:
%  discover critical problem from previous models.
% figure out
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T14:03:10.902Z:
%
% > percentage
%  percentage of the data,  of the xxx
%  amount?(  )      percentage
% the amount of the generated questions that include the target answers either completely or partially is significantly lower in our model.
%  ->   !!
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:12:41.575Z:
%
% > has better recall score over
%  outperform
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T18:08:55.591Z:
%
% > To figure out
%
%  ,     ?
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:29:25.291Z:
%
% > replacing answer \textless a\textgreater{}
% replacement of target answer with <answer> token
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:32:28.933Z:
%
% > the attention weights of the interrogative word who is mostly influenced by the \textless a\textgreater{} token with our answer-separated seq2seq
%
% The interrogative word "who" gets most of the attention weights(higher attention weights) from the <answer> token in our answer-separated seq2seq
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:36:04.726Z:
%
% figure      (    )
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T18:49:53.417Z:
%
% > Since named entities are one of the largest sources for answers, from training part of data split-1 we extract words and phrases that are named entities as new target answers and pair them with corresponding passages.
%
% Since most of the answers are nouns, a large portion of them correspond to named entities. Hence, we use words and phrases that are named entities from training part of data split-1 as target answers. And then we pair those answers with corresponding passages.
%
%
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T19:10:19.632Z:
%
% > Since the generated dataset should be organized SQuAD dataset which has (\textit{paragraph}, \textit{question}, \textit{answer position}) triplets, we trace the passage in data split-1 to the original paragraph and re-compute the answer position as well. We finally make a synthetic data with about 50k questions and train the machine comprehension system only with our synthetic dat
%
% To make the dataset in the same way as SQuAD dataset,(\textit{paragraph}, \textit{question}, \textit{answer position}) triplets, we trace the passage in data split-1 in the original paragraph and re-compute the answer position as well.
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T19:25:42.897Z:
%
% score
%
% ^.
...
% * <kindaichi7207@gmail.com> 2018-09-05T19:27:23.957Z:
%
% > when the model is trained with the
% on
%
% ^.
% * <kindaichi7207@gmail.com> 2018-09-05T19:24:11.301Z:
%
%  ( )'s implementation(GitHub  ) ,
%
% ^.

%We generate sentence-answer pairs with different named entities and generate sentence-answer-question triplets with our pre-trained model.
