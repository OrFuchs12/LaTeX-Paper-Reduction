...
%
%\title{Summoning the Demon? Or improving human life? Tracking Trends in the Public Perception of Artificial Intelligence}
%This is okay, but trying some alternative titles.
\title{Long-Term Trends in the Public Perception of Artificial Intelligence}
%we need to distinguish sentiment expressed in the press and by the public.  Reddit and commments, etc. on articles would give us %more of a sense for the "public" sentiment.  For now, we have the press coverage as a proxy.
%\author{Anonymized}
\author{Ethan Fast \and Eric Horvitz\\
...
%Note: relevant to say that something like sentiment analysis doesn't capture this idea?
...
%We also used free form text boxes to capture additional hopes or concerns not present in the ontology defined by the AI index. 
...
% that use $p>=1$ ratings, $p>=2$ ratings, and $p=3$ ratings.\footnote{For $p>=1$, define at least one worker vote as a positive prediction, for $p>=2$ define at least two votes as a positive prediction, and for $p=3$ define three votes as a positive prediction.} 
...
%To address the impact of outside events on AI discussion, we manually constructed a set of relevant events, such as AI-related movie or product releases (Table \ref{tbl:events}). We randomly sampled 100 days between January 1, 1986 and May 31, 2016, and then tested for statistical differences in AI discussion between one month periods that followed the relevant events and these randomly sampled days using Welsch's t-test. When testing for the impact of events, we removed all content that directly referenced the event in question by filtering for relevant keyword strings, e.g., for the release of ``The Terminator,'' we excluded data with the strings ``Terminator,'' ``Schwarzenegger,'' and ``Skynet.''
...
%There is a clear shift in optimistic and pessimistic predictors over time. From 1986-1990, we see words such as ``future'' and ``products'' and ``space'' as predictors of optimism. For example, consider an article from 1986: ``Much has changed since the mystique of \textit{space} flight first grabbed the public imagination. Computers are more powerful; artificial intelligence is about to burst from its infancy.'' In the same period, words such as ``impossible'' and ``retrenchment'' and ``neural'' are predictors of pessimism. For example, in 1987: ``We do not believe, as Mr. Johnson writes, that artificial \textit{neural nets} make successful artificial intelligence.'' 

%In contrast, from 2010-2016 we see words such as  ``deep learning'' and ``start up'' and ``big data'' as predictors of optimism. For example, in  and ``weapons'' and ``jobs'' and ``existential threat'' as predictors of pessimism.  
...
% \begin{quote}
% \small
% Representatives from many nations meet today in Geneva to consider restrictions on weapons relying on artificial intelligence. Critics warn they could become difficult for humans to \textbf{control}. (2014)
% \normalsize
% \end{quote}
% Another tells us: 
\begin{quote}
...
% \begin{quote}
% \small
% ``Loss of control of A.I. systems has become a big concern,'' Horvitz said. ``It scares people.'' Rather than simply dismiss these dystopian claims, he said, scientists must monitor and continually evaluate the technologies.
% \normalsize
% \end{quote}
...
% For example, this excerpt from 2015 suggests that some in the field do not want to repeat past mistakes:
% \begin{quote}
% \small
% But artificial intelligence researchers have a more basic concern: that their work will \textit{once again fall short of expectations}, leading to yet another fallow period for their field.
% \normalsize
% \end{quote}
...
% Indices are a useful tool for capturing and reflecting social phenomena on an ongoing basis. For example, the Future Orientation Index measures the value countries place on the future over time---a stronger future orientation correlates with GDP \cite{future-index}. Others have proposed a National Happiness Index and explored the value of measuring happiness across time and culture \cite{happiness-index,happy-culture}. The AI index expands on these ideas to orient discussion and public policy around AI.
...
%   \begin{tabular}{p{9em}p{9em}p{46em}}
%     \textbf{Index Measure} & \textbf{\# of mentions} & \textbf{Example} \\
%     \hline
%     Optimistic attitude & 1586 & Despite his concerns, Dr. Horvitz said he was hopeful that artificial intelligence research would benefit humans, and perhaps even compensate for human failings... \\ %He recently demonstrated a voice-based system that he designed to ask patients about their symptoms and to respond with empathy. When a mother said her child was having diarrhea, the face on the screen said, ``Oh no, sorry to hear that.'' \\
%     Pessimistic attitude & 566 & The theoretical physicist Stephen Hawking told us that ``the development of full artificial intelligence could spell the end of the human race.'' Elon Musk, the chief executive of Tesla, told us... \\ %that A.I. was ``potentially more dangerous than nukes.'' Steve Wozniak, a co-founder of Apple, told us that computers are going to take over from humans and that ``the future is scary and very bad for people.'' \\
%     Military applications & 97 & Some robotics experts doubt that a computer will ever be able to reliably distinguish between an enemy and an innocent, let alone judge whether a load of explosives is the right, or proportional, response... \\% What if the potential target is already wounded, or trying to surrender? And even if artificial intelligence achieves or surpasses a human level of competence, the critics point out, it will never be able to summon compassion. \\ 
%     ... & ... & ... \\

%     \hline
%   \end{tabular}
%   \caption{An overview of measures in the AI index. \textit{\# of mentions} describes how many paragraphs that mentioned AI matched the category over thirty years of data we gathered from the New York Times. \textit{Example} provides a representative paragraph that matched the category. }
%   \label{tbl:index}
% \end{table*}
