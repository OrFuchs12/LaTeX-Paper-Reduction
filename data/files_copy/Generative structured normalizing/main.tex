...
%SNFGP incorporates both dimension reduction and flexible nonlinear transformations to effectively model the input/output relationship via a GP.
...
%We used the generative model likelihood to infer the composition for a new spectrum.
...
%For this short paper, we used only one of the composition values (the oxide weight percent of SiO$_2$).
%Certainly, the presence and quantity of other oxides will also affect the spectrum, so we anticipate that including these in the model could reduce data uncertainty in the latent space and improve the generative model.
%To obtain predictions of the composition for a given spectrum, we used maximum likelihood with uncertainty intervals based on the likelihood ratio test, but we could instead take a Bayesian approach to obtain full posterior distributions over the compositions (which could incorporate prior information, indicate multimodality, could be better suited to describing dependencies between multidimensional inputs).


%One difficulty that arises in both methods is the choice of the latent dimension $K$; if $K$ is too small, we may lose important information in the data during compression, but if $K$ is too large, the model may become noisy and more difficult to estimate.
%Furthermore, we plan to investigate potential identifiability issues in the model, as the normalizing flow parameters can trade off with the GP parameters during inference. 
