...
\pdfinfo{
/Title (Intelligent Frame Selection as a  Privacy-Friendlier Alternative to Face Recognition)
/Author (Mattijs Baert,
    Sam Leroux,
    Pieter Simoens)
...
%Key Frame Selection for Face Recognition in Surveillance Video using VAE reconstruction probability
%Data minimization through key frame selection for face recognition in surveillance video
...
%privacy is enhanced by only collecting the minimal necessary amount of personal data. This method is recognized as the ``minimize'' strategy for designing privacy preserving technologies \cite{DomingoFerrer2020}.
...
%Another option to perform face identification is training a classification layer which uses the feature embeddings as input.
\\
\newline
...
%\section{Privacy-friendly alternative [ToDo: better title]}
% Towards privacy preserving face recognition
...
\\
\newline
%Current surveillance systems transfer all data to apply large scale facial recognition on it. Saving one high quality snapshot on the edge is a noteworthy alternative to preserve the user's privacy. To determine the highest quality snapshot, a novel face image quality assessment method is proposed. This is, by our knowledge, the first time a generative model like a VAE is used to tackle the problem of face image quality assessment. Unlike other data driven approaches, no facial recognition model is used for training and no explicit definition of face quality is given. 
...
%We believe optimizing the VAE architecture and latent distribution will further improve results. It is also worth investigating to what extend different training data will affect the performance. Moreover, our method should be evaluated on other datasets and in combination with alternative feature extractors.
