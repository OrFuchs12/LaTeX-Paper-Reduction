...
%\newcommand{\IWa}{{IW(1)}}
%\newcommand{\IWi}{$\textsc{IW}(i)$}
%\newcommand{\IWk}{$\textsc{IW}(k)$}
%\newcommand{\SIW}{{\textsc{SIW}}}
...
% \CHECK{It seems we should present results for IW(1) and then for Rollout IW(1)}
...
$N\!-\!1$ frames.
% We use a frameskip of 15, which given that there are 60 frames per second, 
% means that real-time play involves decisions  every  0.25 seconds, a decision rate
% that is also feasible  for humans. 
...
in a high score.
% The ``observable states'' or simply ``states''  below refer
% to the values of a set of boolean features determined by  the observations
% (screen pixels). The  truly Markovian system (RAM) states  are referred to
...
% no se necesita creo:
% where a feature $f_1\land\cdots\land f_k$ in $F^k$
% is said to hold at state $s$ if each $f_i$ holds at $s$ for $i=1,\ldots,k$.
...
% \CHECK{Following sentence is related to multiple novelty tables.
%   If we remove such tables, we should remove the sentence.}
% A polynomial score-sensitive variant of IW(1) is presented below. 
...
%% THIS IS UNNECESSARY
%Checking if a state $s$ has novelty $1$, i.e., if $s$ makes true
%a feature $f$ for the first time in the search, would be expensive
%if each one of the $20,598,848$ features is  checked in turn.
%We check instead  which of the $28,672$ basic features are active
%(true) by scanning the $160 \times 210$ screen pixels,
%and  compute the subset of active  B-PROST features from them.
...
\begin{enumerate}[$\bullet$]
  ...
  \item Case 2: if node $n$ is new in the tree but $d \geq d[f]$ for each e-feature $(d,f)$ true in $n$, \textbf{terminate} rollout.
    %for no e-feature $(f,d)$ true in $n$, $d < d[f]$, terminate rollout,
...
% The noded $n$ was possibly  optimal  for some feature $(f,d)$ when the node $n$ made it into the tree, else it'd would have
% been pruned following case 2).
...
% This follows from the two pruning cases 2 and 4 above:  the parent of the node labeled UNSOLVED is clearly UNSOLVED,
% and if it was not in the tree before the last rollout, it must have been a new node,  falling in case 1.
...
%% Previous def if width 1 states was wrong; used conventional width 1 goals 
...
%\CHECK{I SEE A POTENTIAL IMPROVEMENT HERE BY DIFFERENTIATING BETWEEN NEGATIVE REWARDS}
...
pixels.
%\footnote{There are 9 games on which the references report no results:
% \game{berzerk}, \game{defender}, \game{elevator} \game{action},
% \game{kaboom}, \game{phoenix}, \game{pitfall}, \game{skiing}, \game{solaris},
% and \game{yars} \game{revenge}.
% We do not report results on 3 additional games as their screens are not $160 \times210$
% pixels.
% These are \game{carnival}, \game{journey} \game{escape}, and \game{pooyan}.
%}
...
%Executions may differ because an initial random number, drawn between 0 and 30,
%of noops is applied when the episode begins, and since Rollout IW(1) has
%a randomized component.
...
%    \input{table1-011-15-onesec}
%  }
%  \caption{\small \CHECK{CHECK CAPTION} % small font?
%    Scores for Rollout IW(1) with a time budget of 1 second vs.\ Human, DQN, and RL-Blob-PROST
%    on 49 games. Best overall entries across rows are shown in bold and the resulting count
...
\begin{table}[t]
  \centering
  \resizebox{\columnwidth}{!}{
    %\input{table1-110-15-halfsec}
\newcommand{\B}{\color{red}}
\begin{tabular}{@{}rrrrr@{}}
\toprule
                               &              &               &                 &    RAS Rollout IW(1) \\
%                         Game &        Human &           DQN &   RL-Blob-PROST &       budget of 0.5s \\
                          Game &        Human &           DQN &   RL-Blob-PROST &          budget 0.5s \\
\midrule                                                               
%                          rom &        human &           dqn &            blob &       roll.110.15.5s \\ %best.score
...
%\begin{figure}[t]
%  \centering
%  \includegraphics[width=.95\columnwidth]{011-15-halfsec}
%  \caption{\small \CHECK{CHECK CAPTION} % small font? %
%    Scatter plots for the score of Rollout IW(1) with time budgets of 0.25, 0.50, 1, and 32
%    seconds vs.\ a human player over 49 games.
%    Points on or above the diagonal correspond to games on which Rollout IW(1) is as good as
%    the human player.
%    For a budget of 1 second, this happens in 23 of the 49 games.
%  }
%  \label{fig:011:scatter}
%\end{figure}
%}
\begin{figure}[t]
  \centering
...
%  }
%  \caption{\small \CHECK{CHECK CAPTION} % small font?
%    Scores over 58 games for Human, IW(1) over RAM, IW(1) and Rollout IW(1) with time budgets of 1 and 32 seconds,
%    and Risk-Averse Rollout IW(1) with time budgets of 0.5 and 1 second. Entries marked as n/a are not available. 
%    Entries highlighted in red are scores as good as the score of the human player
%    (except for IW(1) over RAM). Entries shown in bold are best scores across rows.
%    A summary of each column is shown at the bottom of the table.
%  }
%  \label{table:big}
%\end{sidewaystable*}
%}
...
%                       Game &&      Human &         RAM &\textcolor{white}{xx}&  budget of .5s &  budget of 32s &\textcolor{white}{xx}&  budget of .5s &   budget of 32s &\textcolor{white}{xx}&  budget of .5s &   budget of 32s &\textcolor{white}{xx}&  budget of .5s &   budget of 32s \\
%                       Game &&      Human &         RAM &&  budget of .5s &  budget of 32s &&  budget of .5s &   budget of 32s &&  budget of .5s &   budget of 32s &&  budget of .5s &   budget of 32s \\
\midrule
%                        rom &&      human &         ram &&  bfs.011.15.5s & bfs.011.15.32s && roll.011.15.5s & roll.011.15.32s && roll.010.15.5s & roll.010.15.32s && roll.110.15.5s & roll.110.15.32s \\
...
%% Will Dabney, Dan Horgan, Bilal Piot, Mohammad Azar, and David Silver. "Rainbow: Combining Improvements in Deep Reinforcement Learning." arXiv preprint arXiv:1710.02298 (2017).

% Asai, Masataro, and Alex Fukunaga. "Classical Planning in Deep Latent Space:
%% Bridging the Subsymbolic-Symbolic Boundary." arXiv preprint arXiv:1705.00154 (2017).
