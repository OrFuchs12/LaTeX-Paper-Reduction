...
/Keywords (social robotics, human-robot interaction, machine ethics, rule induction, AI safety)}
...
% One drawback of incremental rule induction is the need to iterate over existing examples and rules, so as to ensure consistency. This gives worst-case time complexities of $O(n+r)$, $O(nr^2)$, $O(n+r)$, $O(n+r^2)$ for Algorithms \ref{alg:CoverExample} to \ref{alg:UncoverRule} respectively, where $n$ is the number of examples, and $r$ the number of rules per action. However, under the reasonable assumption that $r = O(\log n)$, i.e., the number of objects is exponential relative to the number of normative categories they fall under, the algorithms are $O(n \log n)$ at worst, which is more than sufficiently fast for a social robotics environment.
