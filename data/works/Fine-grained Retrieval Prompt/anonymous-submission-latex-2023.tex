...
%With the great success of deep neural networks in FGOR, existing fine-grained retrieval approaches can be roughly divided into two categories. The first category, \textit{designing specialized loss functions}, is that  learning an embedding space where similar instances are attracted and dissimilar ones are repelled by fine-tuning the pre-trained models \cite{DBLP:conf/eccv/TehDT,DBLP:conf/cvpr/WangHHDS19,DBLP:conf/eccv/BoudiafRZGPPA20}. Despite their successful performances, they do not pay sufficient attention to the discriminative details of fine-grained objects and suffer from the background clutters. Therefore, the second category, \textit{object/part localization schemes}, focuses on localizing the object and even the discriminative parts via reformulating the model architectures or designing complex localization sub-networks \cite{DBLP:journals/tip/WeiLWZ17,DBLP:conf/ijcai/ZhengJSWHY18,DBLP:conf/mm/ShenZJCJCH17,DBLP:conf/wacv/MoskvyakMDB21}. Success behind these existing works can be largely attributed to being able to learning a discriminative embedding space or capturing more discriminative local details for fine-grained retrieval task. 


%All previous works heavily rely on classification or metric losses to map samples to a discriminative embedding space, where similar samples lie closer than dissimilar ones. Intuitively, they have practical difficulties in producing meaningful adjustments to the optimization trajectory driven by these losses due to adopting parameter inefficient global update and heavily relying on high-quality data \cite{DBLP:conf/cvpr/HuangZGS21, DBLP:conf/icml/HuangZTMSGS19, DBLP:conf/icml/HiranandaniMNFK21}. Therefore, this easily leads to models converging to a suboptimal solution, unavoidably suffering from the missing of class discriminability. When global fine-tuning is prone to have a negative impact on model optimization, how to effectively tune the models to provide a more discriminative embedding space is worthy of investigation for FGIR.


%Inspired by natural language processing \cite{DBLP:conf/acl/GaoFC20, DBLP:conf/acl/LiL20}, prompt-based learning is to reformulate the downstream tasks to those solved during the original pre-training, rather than fine-tuning pre-trained model to adapt to downstream tasks. Following this spirit, we propose a Discriminative Prompt Tuning (DPT), a parameter-efficient fine-grained retrieval tuning paradigm, which is a lightweight alternative to fine-tuning and significantly improves the retrieval performance. The core mechanism of DPT is to partition the model parameters into two parts: \textit{prompt parameters} that serve as additional input and output to the vision model (i.e., ResNet) and are adapted on discriminative learning, and \textit{frozen parameters} that are pre-trained and not updated during the training phase. The prompt parameters consist of Discrimination-aware Amplifying Module (DAM) and Discriminative Information Bottleneck (DIB). In particular, DAM selectively highlight the local regions contributing to category predictions by performing an inhomogeneous sample on the input image. Moreover, DAM is regarded as a retrieval-related prefix which steers a pre-trained vision model to pay more attention on discriminative details. However, since pre-trained vision models could capture extensive semantic knowledges related to the pre-trained class information from input images,  these knowledges contain the redundancy, irrelevance, and noise information and thus are not beneficial for retrieval performance. To handle this limitation, DIB is viewed as a retrieval head, which designs an attention-guided instance normalization layer to cope with these useless features while preserving the discriminative ones.

%Since our DAPro applies the prompt scheme in FGOR, it inherits several advantages of prompt tuning. First, the size of pre-trained vision models can be chosen appropriately to make a trade-off between powerful representations and retrieval speeds. Second, we do not need to access and perform operations on the weights and biases of pre-trained models to set up the computation graphs, thus decreasing heavily computational overhead and reliance on large-scale fine-grained images. Finally, from the system deployment perspective, our DAPro can help distributed learning systems, where the same model is deployed to different machines and we wish to learn different prompt parameters concurrently.
...
\end{itemize}
%FRPT only needs to optimize about 10\% of the parameters than being fully fine-tuned, and even achieves the new state-of-the-art results on the widely-used fine-grained retrieval datasets, including the CUB-200-2011, Stanford Cars and FGVC Aircraft.

%optimizes parameters than fine-tuning paradigm, and thus exhibits its outperforming retrieval performance on the widely-used fine-grained retrieval datasets, including the CUB-200-2011, Stanford Cars and FGVC Aircraft.
...
%We propose Discriminative Prompt Tuning (DPT) for fine-grained image retrieval. DPT injects a small number of learnable parameters into the pre-trained vision model's input and output spaces and keep the pre-trained model frozen during the training stage. The overall framework is presented in Fig. 2. 
...
%FRPT is a method for tailoring pre-trained representation models to fine-grained retrieval tasks without changing their weights.

%Specifically, FRPT is mainly organized by three modules: structure perturbation prompt (DPP) that selectively zooms and even exaggerates the idiosyncratic elements contributing to category prediction in objects via a content-aware inhomogeneous sampling operation on each input image; pre-trained vision model for extracting the semantic features from the modified images, while freezing it; category-specific awareness head (CAH) to reduce the negative impact of feature redundancy and noise  within the semantic features while preserving their category-specific information as much as possible.
...
%It should be clarified that only the parameters of DPP and CAH are updated while the pre-trained representation model is frozen during training on the fine-grained retrieval task. Therefore, our FRPT can compute gradients in a low-dimensional parameter space, and thus make it easy to learn a discriminative embedding space and provide better generalization accordingly.



\subsection{Discriminative Perturbation Prompt}
%A well pre-trained representation model can bring high-quality cross-task representation that assists to guarantee the generalization ability for fine-grained object retrieval \cite{DBLP:journals/tip/WeiLWZ17}. 
...
%Each channel of detail features $ M_S $ corresponds to a visual pattern, however, such features cannot directly reflect the location and intensity of discriminative information due to the lack of consistency and robustness. Meanwhile, since the semantic features are derived from the shallow layers of pre-trained vision model, they only contain extensive detail information while lacking of semantic information.
...
%Emphasizing certain elements contributing to decision boundary from input images is a pre-requisite for making the frozen representation model capture more discriminative details. A typical approach is to introduce bounding boxes or segmentation annotations, which cost much on data collection. For typical image retrieval task that lacks localization or segmentation annotations, we propose a new module called structure re-sampling to help pre-trained representation model focus on discriminative details. 
...
%CDP first decouples the semantic features into the discriminative and complementary groups via performing channel-wise attention, and then reduces the negative impact of redundancy and noise within the complementary group through using an instance normalization.
...
\hline 
\hline
		%EPSHN \cite{DBLP:conf/wacv/XuanSP20}& R50& 64.9 & 75.3 & 83.5 &-& 82.7& 89.3&93.0&-& -& -&-&-\\
		%NSM \cite{DBLP:conf/bmvc/ZhaiW19} & R50& 65.3 & 76.7 & 85.4 &91.8& 89.3& 94.1&96.4&98.0& -& -&-&-\\
		%ST \cite{DBLP:conf/iccv/QianSSHTLJ19} &In3 &65.4 &76.4 &84.5 &90.4 &84.5 &90.7 &94.5 &96.9 &- &- &- &-\\
		%MS \cite{DBLP:conf/cvpr/WangHHDS19}& In3& 65.7 & 77.0 & 86.3 &91.2& 84.1& 90.4&94.0&96.1& -& -&-&-\\
		%HORDE \cite{DBLP:conf/iccv/JacobPHK19}& In3& 66.8 & 77.4 & 85.1 &91.0& 86.2& 91.9&95.1&97.2& -& -&-&-\\
		...
		%Proxy \cite{DBLP:conf/cvpr/KimKCK20}&In3 &71.1 &80.4 &87.4 &92.5 &88.3 &93.1 &95.7 &97.5 &- &- &- &-\\
...
%As shown in Tab. 2, our approach outperforms these deep metric learning-based methods in the second group, which indicates that the proposed method can better minimize the intra-class variances and maximize the inter-class distances by directly adapting pre-trained models to fine-grained retrieval according to modifying the location of the pixels of the input images. Compared with recent localization-based works, they demonstrate the importance of localizing objects/parts. We run FRPT to directly learn category-specific nuances from images for emphasizing discrepancies among subcategories and achieve the new state-of-the-art. 
...
%, such that the accuracy is close to the performance obtaining by fine-tuning pre-trained model. 
...
%In contrast to the fine-tuning scheme, FRPT has fewer parameters to train, but still learns embeddings with greater discrimination and generalization owing to DPP and CAH, thus solving the convergence to suboptimal solutions caused by fine-tuning the entire FGOR model.
Extensive experiments demonstrate that our FRPT with fewer learnable parameters achieves the state-of-the-art performance on three widely-used fine-grained datasets.
