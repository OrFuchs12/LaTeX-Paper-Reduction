...
%
% \title{Label Organized Memory Augmented Neural Networks}
...
% Fast online adaptation of batch trained neural networks is important in applications like user trajectory prediction where domain-relevant labeled data is available during deployment.  Memory is useful for fast adaptation as it allows fast storage and recall of online data.  However, existing architectures for  memory augmented neural networks (MANNs) are not designed for online model adaptation.
...
% Each sequence is driven by a combination of its own local patterns and global patterns shared with other sequences in the training set. An ideal model should capture the global patterns in an offline training process, and during deployment also respond to per-sequence patterns revealed online. 
...
% Each sequence is driven by  a combination of its own local patterns and global patterns shared with other sequences in the training set.An ideal model should capture the global patterns in an offline training process, and during deployment also respond to per-sequence patterns revealed online.
% This can be viewed as a meta-learning task for which many techniques have been proposed~\cite{Rei15,Finn2017ModelAgnosticMF,Huang2015MaximumAP,SantoroBBWL16,ravi17}.
%
%
...
% \shiv{Added this paragraph here for reviewer 3}
...
% Due to representation overlap in the layers of a neural net, when weight updates are applied to a network, prior information is likely to be affected.  During sequential meta-learning updates, the weights deteriorate as new input gets superimposed over top of the old input.
...
% The second category attempt to augment a neural network with a large memory where new labeled data can be stored and recalled while the number and values of parameters remain fixed irrespective of memory content~\cite{SantoroBBWL16,kaiser2017,VinyalsBLKW16}.  These networks called Memory augmented neural networks (MANNs)~\cite{GravesNTM,GulcehreCCB16} 
...
% This requires for carefully organizing the memory to allow fast and unbiased storage and recall and carefully  level of integration of the memory and pre-trained model.
...
% by bringing in principled methods like boosting and online kernel learning to the learning power of deep networks.
% \shiv{Does this sound too loud?}
% \sunita{I think this might be too aggressive, especially when our own setup doesn't fit nicely into it, and we have to handwave a little later.}
% 
% As deep learning models get deployed in practical settings, they are grappling with challenges of limited labeled data, class-skew, and the ability to quickly absorb new labeled data local to a specific deployment.  
%%introduce online sequence learning here?

 % The challenge is to adapt to sequence-specific local patterns fast and in an online manner.

% Recently, a new architecture in the form of  has emerged as a partial solution to the above challenges.  Memory networks were originally developed with goals far loftier than plain supervised learning.  For example, in \cite{GravesNTM}, an end-to-end differentiable memory network is used to learn basic tasks and in \cite{Weston16} memory is used to memorize facts relevant for question answering.  They have also been found useful for tasks like few-shot learning and learning rare events
%%classification with limited labeled data (few-shot learning)


%In this paper we propose a new architecture for memory augmented neural network that is specifically suited to the task of 
%  as an online kernel learner
...
% Our idea is similar to Boosting~\cite{Schapire:1999adaboost} as the memory gives preference to contents that are misclassified by the regular network. 
...
% In our setting since the memory is evolving it makes more sense to evolve its relative importance too. Normally, this is challenging   
% \sunita{ you wanted to change this}
...
% real-life online adaptation datasets.
...
%Here the sequence is processed a token at a time. At each time $t$ the model predicts the next token, at the next time slot the true token is made available to the model before the next prediction.  This setting is also called teacher-forcing.  The training data consists of several such sequences.  Each sequence is driven by  combination of its own local patterns and shares global properties from other sequences in the training set.  An ideal model could capture the global patterns in an offline training process, and during deployment also respond to per-sequence patterns revealed online.
...
% In these applications we expect both the global pattern over all sequences and local pattern specific to a sequence to be useful in predicting the next output. 

% We are given a batch trained classification model. During deployment  The dataset $D$ is a collection of similar classification tasks. Each task  is a set of tuples $(x_1,y_1),\dots,(x_n,y_n)$. During testing for a new task    While this may seem restrictive,
...
\end{equation}
%
% We will use the term primary classification network (PCN) to refer to this component of our network.
...
% \footnote{MANNs used for few-shot learning do not transform memory reads further but in this paper our focus.}.
...
% where $f_{att}(\vh_t,\vM_m)$ is a function used to compute the attention given to cell $m$.  Following practice in existing MANNs, within all cells with label $y$ we use soft attention via a cosine similarity function, i.e., $f_{att} = (\frac{\vh_t\vM_m}{\|\vh_t\| \| \vM_m\|})$. 
...
% Our choice of kernel is an exponential cosine kernel $K = \exp(\lambda f_{att}(\vh_t,\vM_m))$, where $\lambda$ is a positive focusing parameter.  This kernel has the advantage that it is both bounded and always positive allows us to transform memory scores directly into probabilities.
...
\begin{split}
 ...
 \end{split}
\end{equation}
% where $\mu_t^y$ is the RNN state at time $t$ for label $y$, $e^{pcn}_{t-1}$ indicates the error in the predicted label from PCN alone in the previous time step, i.e., whether  $\argmax_y r_{t-1 y}$ equals the true label $y_{t-1}$, and $e^{m}_{t-1}$ is the corresponding memory error.
% We update all states at each step, irrespective of the true output or the predicted output.
% \hat{e}^{pcn}_{t-1}  
% I\{ y_{t-1} = \hat{y}^m_{t-1}\}, \\ I\{ y_{t-1} = \hat{y}^{pcn}_{t-1}\} $ \argmax_y \lmm_{ty}$ where $I$ is an indicator function, ${y_{t}}$ is the actual label at time $t$ and 

% \shiv{Define the memory/pcn ys.}
...
% Before deployment, we need to train the combiner network in the deployment setting.
...
% \begin{figure*}
% \begin{center}
%   \includegraphics[width=580pt]{combined.png}
% \end{center}
% \caption{Comparing Memory Networks and LMN}
% \label{fig:net_comparison}
% \end{figure*}
...
% \paragraph{Labeled memory writes}
...
% We are interpret memory updates as applying one step of gradient update on the training objective with respect to memory contents $M_t$
...
% The guiding principle behind our design of MANN was to write to memory only when the primary classification pipeline is deficient. This is useful for learning rare classes and when combining global and local patterns in online sequence learning.
% We interpret memory updates as applying one step of gradient update on the training objective with respect to memory contents $M_t$. However, in order to make sure that updates to memory are sparse, we design our updates based on hinge-loss rather than cross-entropy.  
%  The PCN parameters are updated via gradient descent. The updates to $M_t$ are also aligned with the gradient of above training objective as described below.
...
\end{center}
\end{figure}
% \shiv{How was margin and decay chosen?}
% \shiv{Did no optimization for that, margin is 0 for lm, and sequnces, and 0.1 for omniglot (same as kaiser). Decay too we just fixed as 0.99. Ideally we could optimize those but it requires re-running the training of combiner}
...
%\sunita{Here is pegasos analogy about $\alpha$ and SV's}
% \sunita{Justify this specific memory updates by relating to the online pegasos paper.  This is the place to perhaps include one theorem if you can contextualize it well.}
...
% ,SutskeverVL14,ChoMGBSB14,DanilukRWR17}. 
% In principle the state of a RNN can memorize relevant patterns from data. However, the success of attention in RNNs shows the benefits in practice of revisiting previous RNN states.
%Most common use is in encoder-decoder models like in translation~\cite{SutskeverVL14,ChoMGBSB14} where the attention is over the input sequences.  Our focus in this paper is online prediction, where a more relevant form of attention is self-attention. For language models, \cite{DanilukRWR17} shows that self-attention is not more effective than concatenation of the past five states. Their task is different from our task of online adaptation for next-token prediction. For us memory of all previous tokens is important, as it is not for summarizing the current context of the user but for recalling the following token from a similar context anywhere in the past.
% \paragraph{Memory Augmented Neural Networks (MANN)}
...
% and also attention cannot memorize rare facts across sequences.
...
% However, our experiments found the trained keys to be correlated leading to redundancy and poor memory utilization.
...
% At each step memory reads feeding into the RNN state (the PCN) and memory updated.
% The basic memory addressing scheme might seem similar to the D-NTM, which employs discrete attention. However it requires extensive training to create the set of addressing keys. In our experiments we found it both difficult and time-intensive to train D-NTM on large datasets. By relying on label information to directly condition memory addressing, we avoid the hard task of learning the addressing mechanism.

% It also allows us to handle new unseen labels, making our method conducive for few shot learning as well. 
...
% we do not need a separate memory loss over only the memory scores because we combine the label-wise memory scores with the primary neural network scores and create a global training objective.
% \sunita{Needs clean-up}

% We review the method of \cite{kaiser2017}.  Each memory cell stores a key-value-time triple where the key is a real vector, value is a class index, and time denotes when the cell was last correctly used.  The hidden vector $\vh_t$ from the last layer of the NN is used to retrieve the nearest cell based on the keys.  
% The output from the memory is then embedded in a real space, concatenated with the last layer from NN, transformed via a linear layer, and fed to a softmax layer to get the class prediction.  Memory is updated for every instance.  When the top-1 value matches the correct class $y_t$, the top-1 key is merged with $\vh_t$ and access time updated, else a least-recently used cell is replaced with the new key $\vh_t$ and value $y_t$.  In addition to the usual classification loss, the authors introduced a separate memory loss to promote the top-1 memory match to be correct.

% Our method also differs from meta-learning memory networks\cite{SantoroBBWL16}. Those methods follow the trend of generic memory networks where addressing is content-based and dependant upon the hidden vector $\vh_t$ derived from the input, and achieves meta-learning by encoding the output in the memory vectors. 
%
% Our model is both simpler to train and can be deployed in a plugin fashion to improve an existing PCN.
...
% %shalevshwartz:icml07
%  Our model spans the ideas of these works and that of kernel based on-line learning \cite{shalevshwartz:icml07}.One can see the working of the memory module as an on-line SVM learner, with the memory vectors $ \vh_m$ acting as the support vectors and the memory scalars $\alpha_m$ as the associated dual variables.  

% \paragraph*{Boosting}
% Our idea  determine the relative importance of Memory and PCN inspired from boosting, where each new stage has higher weights on previously misclassified instances, and its overall contribution is determined by its accuracy . In a similar vein memory is updated only on misclassified instances and keeps track of their relative counts, increasing its scoring of those labels. The combination of relative importance comes via the combining RNN, which keeps track of both the PCN and memory accuracy across labels.



\section{Experiments}
% Our labeled memory module can be used to augment any neural classification model be it a CNN for image labeling, a RNN for sequence prediction, or a regular feed forward network.
...
% For the data we use various datasets which include a synthetic dataset, a geo-temporal sequence dataset GeoLife[\cite], e-retailer user click data provided by Yoochoose[\cite], TSMC2014 and FSQTokyo are location based social network (LBSN) data [\cite].
\begin{itemize}
\item
...
% We first train the PCN, like in boosting without the memory, on the training sequence using a cross-entropy loss. In the second step, we add the memory and retrain the model for 3000 steps. 
% During test time, the output of the GRU is used to update the memory as described earlier. Each sequence has its own memory which is utilized for adaptation to sequence-specific patterns during testing. The mixing weights $\lambda$ are changing based on the accuracy of the two stages similar to Boosting, and are determined by the combining RNN. This models is called LMN.  In Section~\ref{sec-expt-online}, we present an experimental evaluation of different models on the datasets.
...
%Name &  LMN* &  Rei & LSTM & DNTM & MAML & LMN \\ \hline
%Brightkite & 3.88 (0.51) & 10.01 (0.18) &  10.7 (0.11) & 9.63 (0.13) & 4.27 (0.47) &\textbf{3.57 (0.55)}\\
%FSQNYC & 6.13 (0.25) & 8.72 (0.07) &  8.95 (0.03) & 9.01 (0.05) & 6.55 (0.16) &\textbf{5.54 (0.27)}\\ 
%FSQTokyo & 5.40 (0.26)& 6.95 (0.13)&  8.14 (0.08) & 7.25 (0.11) & 5.68 (0.23) &\textbf{5.32 (0.28)} \\ 
%Geolife & \textbf{1.05 } (0.83)  & 1.08 (0.83)&  1.13 \textbf{(0.84)} & 1.11 (0.82) & - &1.08 (0.83) \\ 
%Yoochoose & 5.01 (0.24) & 5.05 (0.23)&  5.01 (0.24) & 5.01 (0.23) & - &\textbf{4.96 (0.25)}\\ \hline
%%
\end{tabular}
\end{center}
...
% Note, the baseline LSTM is fed the true token and with ideal parameters should be able  to encode them as per-sequence state for better prediction.  But in practice we observe that more parameters of a larger LSTM state cannot substitute for focused adaptation. It is important to maintain more explicit state via external memory as in MANNs.
...
%This highlights the plug and play aspect of LMN. We can deploy LMN over existing models, with minimal re-training and achieve better online adaptation. 
...
% The overall training time for LMN is roughly the same as training the PCN, as we ran the retraining step for only 3000 steps.
\end{enumerate}
...
%A key problem with parameter retuning methods like MAML is the emphasis on using new information. During sequential meta-learning updates, the weights deteriorate as new input gets superimposed over top of the old input. This is evident by the fact that the accuracy of the meta-learner on all labels and on new labels is similar. LMN on the other hand is able to use information learned by the PCN much more effectively.
...
% Interestingly, for few-shot learning but our focus in this experiment is to compare memory organization strategies.
...
%%Neural cache (our implem.) & 72.3 &  - & 70.6 & 99.6\\ \hline
\end{tabular}
\end{center}
...
%
% Borrowing ideas from on-line kernel learning ,we use sparse memory updates effectively as subgradient of an implicit loss
