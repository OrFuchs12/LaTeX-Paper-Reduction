...
/Author Danushka Bollegala,  Alsuhaibani Mohammed, Takanori Maehara and Ken-Ichi Kawarabayashi
}
...
% There are work on joint learning and retrofitting. Retrofitting is not good when the corpus is small.
% Moreover, the training can be difficult when the corpus and lexicon are incompatible (legal corpus vs. WordNet)
% Our method learns word representations in an incremental manner. Stress on how humans read books
% We must compare against retrofitting and Yu 2014.
% Can we make our method consider multiple relations simultaneously and multi-hops on the ontology. This is very novel.

% methods that learn word representations from corpus only: skip-gram, glove. But not with lexicons. That is rare.
% Post-processing problems, one relation type only, ambiguity issues.
