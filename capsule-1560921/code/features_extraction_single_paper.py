...
def run_feature_extraction(latex_path, pdf_path, bib_path, path_to_save_df_dict,
                           path_to_save_analysis_dict, name, df):
    #they are some lines that we will still keep that can be used if we want to transpose the df and some lines that give example on how to add more features and so on.

    #define the summative features
    ...
    for key in summative_features_keys:
        Rows.append(key)
    #define each object his features.
    ...
    j = 0
    # as we know we can have several of the same object, so to represent the differences in the csv, we give a different index to each object (which is j). and also we will need to count how many columns we added with each object so we can add the next object features the same way, hence why we count.
    ...
    #here we can see that we give a range of 1, because in this file we perform feature extraction on 1 file
    for file_index in range(1):
        ...
        with open(path_to_save_df_dict, 'wb') as dct_file:
        ...
        counter_algo = 1
        i = 0
        #here to insert the features values we will need to know to which column to insert the data, so we use the same indexes that we defined at the start.
        while (i < len(Rows)):
            if (i < 43):
                if (i < 12):
...
def run_from_outside(latex_path, pdf_path, bib_path, path_to_save_df_dict,
                     path_to_save_analysis_dict, name, df):
    try:

        df = run_feature_extraction(latex_path, pdf_path, bib_path, path_to_save_df_dict,
                                    path_to_save_analysis_dict, name, df)
        return df
