...
%, where the distributions of arms may vary over time, 
...
%\cite{hartland2007change,yu2009piecewise,mellor2013thompson}. 
...
%with regret $O(\sqrt{\gamma_T T\log T})$.}
...
% but within a finite variation budget 
in the worst case.
...
%A variant of the non-stationary bandit problem is considered by~\cite{yu2009piecewise}, where the decision maker has side observations. They proposed a Windowed Mean-shift Detection UCB algorithm (WMD-UCB) and showed that the regret is at most $O\left(\gamma_T\log T\right)$ where $\gamma_T$ is the number of change points up to time $T$. This is in contrast to the case where side observations are not available, and where the regret is at least $\Omega(\sqrt{T})$~\cite{garivier2008upper}. {\blue Such a significant regret reduction mainly comes from free side observations that are not available in general.}
%\st{In this work, we show that such a significant regret reduction mainly comes from the performance improvement of a change detection algorithm due to the free side observations.} {\blue JH: I think we need to tone down this part since this is not the focus of our paper.}
...
%  \item We propose a change-detection based framework for a non-stationary bandit problem, which consists of a change detection algorithm and a bandit algorithm; we provide a regret upper bound for the class of policies, CD-UCB, when the UCB algorithm is chosen as the bandit algorithm. CD-UCB reduces the solution of non-stationary bandit problems to the design and analysis of change detection algorithms. 
%  \vspace{-5pt}
%  \item We propose a cumulative sum (CUSUM) type change detection algorithm tailored to the bandit setting, and provide an upper bound on the mean detection delay and a lower bound on the mean time between false alarms.
% %for the tailored CUSUM algorithm. 
% Then, we introduce CUSUM-UCB with the regret at most $O(\sqrt{T\gamma_T\log{\frac{T}{\gamma_T}}})$. To the best of our knowledge, this is the first regret bound for actively adaptive policies in the bandit feedback setting.
%  \vspace{-5pt}
%  \item 
% The performance of CUSUM-UCB, PHT-UCB (another policy we introduce) and existing policies are validated by both synthetic and real world datasets, and we show that our proposed algorithms are superior to other existing policies in terms of regret.
%\st{The performance of CUSUM-UCB, the existing policies and some heuristic policies generated by our change-detection based framework are validated by both synthetic data and real world data.}
 \end{enumerate}
%There are also some works deal with the full-information setting, where the decision maker can observe all the arms at each time slot~\cite{wei2016tracking}. Several algorithms and lower bounds have been proposed in~\cite{wei2016tracking}. However, our work focus on the bandit feedback and switching environment. They do not propose algorithms in such a setting.
...
%\begin{equation}
%\Delta_{\mu_T(i)}=\min\{\mu_t(*)-\mu_t(i):t\leq T, i\neq i^*_t\}.
%\end{equation} 

%\subsection{Notion of Regret}
...
%\subsection{Abruptly Changing Environment}
\subsection{Piecewise-stationary Environment}
%{\bf Abruptly changing environment.} 
...
%\begin{equation}
%\gamma_T=\sum_{t=1}^{T-1}\mathbbm{1}_{\{\exists i\in\mathcal{K}: \mu_t(i)\neq \mu_{t+1}(i)\}}.
%\end{equation}
...
%{\bf JH: can we change $3\epsilon$ to $\epsilon \in [0,3/4]$?}
...
%\begin{assumption}\label{ass:lambda}
%The distributions of all the arms are Bernoulli($u$), and $u\in(2\epsilon,1-2\epsilon)$ satisfies
%\begin{equation}
%(u-\epsilon)-{\lfloor (u-\epsilon)M\rfloor}/{M}>\lambda \text{~and~} {\lceil (u+\epsilon)M\rceil}/{M}-(u+\epsilon)>\lambda
%\end{equation}
%for some $\lambda\in(0,1/M)$. Note that $\lfloor\cdot\rfloor$ denotes the floor function and $\lceil\cdot\rceil$ denotes the ceiling function.
%\end{assumption}
%Assumption \ref{ass:lambda} restricts the distribution type of the arms and the value range of the expected reward. Bernoulli reward distributions have also been used in~\cite{besbes2014stochastic, mellor2013thompson, kaufmann2012thompson, agrawal2012analysis}. As $\lambda$ goes to $0$, the range of $u$ converges to the interval $(2\epsilon,1-2\epsilon)$. In addition, as shown in Section~\ref{sec:simul}, our policy CUSUM-UCB still performs well in practice even when Assumption \ref{ass:lambda} does not hold.
...
%For instance, part of Adapt-EvE~\cite{hartland2007change} can be viewed as using PHT as a change detection algorithm and the UCB algorithm as a bandit algorithm; CTS~\cite{mellor2013thompson} employs a bayesian online change detection algorithm and adapts TS algorithm as a bandit algorithm.
%A block-wise sample average comparison is constructed as a change detection algorithm to control the UCB algorithm in WMD-UCB policy when side observations are available. Note that we focus on the traditional bandit feedback setting, where the decision maker (both the change detection algorithm and the bandit algorithm) can only observe the reward of the chosen arm at each time. In terms of the semi-bandit feedback setting, we will discuss the benefit of more observations in Section~\ref{sec:analysis}.

%\subsection{Adapting Change Detection Algorithm}
...
literature.
%\st{(PHT is the type of change detection algorithm used in practice).}
...
%To address the issue of unknown priors, we propose a modified CUSUM algorithm in the following and provide theoretical guarantees for it. We force the bandit algorithm to choose some arms of poor performance sometimes in order to feed the samples for the change detection algorithms. Note that the cost of choosing suboptimal arms, to obtain a sufficient number of samples, is inevitable due to the non-stationary environment. 
...
%Let $\tau$ be the last time that UCB algorithm is restarted. 
...
%\begin{align}
%\label{eqn:update12}
%N_t(i)=\sum_{s=\tau_i}^t\mathbbm{1}_{\{I_s=i\}}, \,\,\,
%%\label{eqn:update2}
%n_t=\sum_{i=1}^K N_t(i).
%\end{align}
%For each arm $i$, we define the sample average $\bar{X}_t(i)$ and padding term $C_t(i)$ as
%\begin{align}
%\label{eqn:update34}
%\bar{X}_t(i) \!=\! \frac{1}{N_t(i)}\sum_{s=\tau_i}^t
%  X_s(i)\mathbbm{1}_{\{I_s=i\}}, \,\,
%%\label{eqn:update4}
%C_t(i) \!=\!  \sqrt{\frac{\xi\log{n_t}}{N_t(i)}},
%\end{align}
...
\end{cases}.
\end{eqnarray}
%where $i_t=\arg\max_{i\in\mathcal{K}}\left(\bar{X}_t(i)+C_t(i)\right)$.

%Note that we ignore the initialization step, where each arm is played once, in the traditional UCB algorithm. We handle the initialization step implicitly by extending $1/0$ to $\infty$, thus the algorithm chooses the arm without any valid observation $(N_t(i)=0)$ automatically. It is equivalent to the initialization step and keeps the algorithm as simple as possible in presentation.

Parameter $\alpha$ 
%is a tuning parameter we introduce in the CD-UCB policy. It 
...
%\label{alg:CUSUM}
%\begin{algorithmic}
%\REQUIRE parameters $\epsilon$, $M$, $h$ and sequence $\{y_k\}_{k\geq 1}$
%\STATE Initialize $g_0^+=0$ and $g_0^-=0$.
%\FOR{each $k$}
%\STATE Calculate $s_k^-$ and $s_k^+$ according to (\ref{twoCUSUM}).%(\ref{lowerCUSUM}) and (\ref{upperCUSUM}).
%\STATE Update $g_k^+$ and $g_k^-$ according to (\ref{eqn:g+-}). %(\ref{eqn:g+}) and (\ref{eqn:g-}).
%%\STATE $g_k^+=\max(0,g_{k-1}^++s_k^+).$
%%\STATE $g_k^-=\max(0,g_{k-1}^-+s_k^-).$
%\IF{$g_k^+\geq h$ or $g_k^-\geq h$}
%\STATE Return 1
%\ENDIF
...
%\st{The basic idea of CUSUM algorithm is to take the logarithm of likelihood ratio $\log\frac{p(y_k|\theta_1)}{p(y_k|\theta_0)}$ as the step of a random walk at time $k$.} {\blue [JH: limiting CUSUM to the log likelihood may distract readers. I found that CUSUM seems more general.]}
...
%\st{Then the property of this random walk is that it has negative (positive) drift before (after) the change point.}
...
%We propose a variant of CUSUM algorithm, which estimates the expectation of random variables before the change. 
To be specific, we use the first $M$ samples to calculate the average, $\hat{u}_0\triangleq({\sum_{k=1}^My_k})/{M}.$
%\begin{equation}
%\hat{u}_0=\frac{\sum_{k=1}^My_k}{M}.
%\end{equation}
...
\end{equation}
%\begin{eqnarray}\label{twoCUSUM}
%(s_{k}^+,s_{k}^-)=
%\begin{cases}
%(0,0), &k\leq M\cr
%(y_k-\hat{u}_0-\epsilon,\hat{u}_0-y_k-\epsilon), & k>M
%\end{cases}.
%\end{eqnarray}
%\begin{eqnarray}\label{lowerCUSUM}
%s_{k}^-=
%\begin{cases}
%0, &k\leq M\cr
%\hat{u}_0-y_k-\epsilon, & k>M
%\end{cases}
%\end{eqnarray}
%\begin{eqnarray}\label{upperCUSUM}
%s_{k}^+=
%\begin{cases}
%0, &k\leq M\cr
%y_k-\hat{u}_0-\epsilon, & k>M.
%\end{cases}
%\end{eqnarray}
...
%Note that due to the challenge of unknown priors, we have to compromise on the performance of the classical CUSUM algorithm. 
...
%\FOR{$t$ {\bfseries from} $1$ {\bfseries to} $T$}
%\STATE{Update according to equations (\ref{eqn:update12}-\ref{eqn:update34}).}
%\IF{$cnt(i)>0$ for some $i\in\mathcal{K}$}
%\STATE $I_t=i$; $cnt(i)=cnt(i)-1.$
%\ELSE
%\STATE{Update $I_t$ according to equation (\ref{eqn:It}).}
%%\STATE{$i_t=\arg\max_{i\in\mathcal{K}}\left(\bar{X}_t(i)+C_t(i)\right)$.}
%%\STATE{With probability $1-\alpha$ play arm $i_t$ and with probability
%%  $\alpha$ play a random arm. Let $I_t$ be the decision.}
%%\STATE{
%%%\begin{eqnarray*}
%%$\begin{displaystyle}
%%I_t=
%%\begin{cases}
%%i_t, & \text{with probability } 1-\alpha.\cr
%%i, & \forall i \in \mathcal{K}, \text{with probability } \frac{\alpha}{K}.
%%\end{cases}
%%\end{displaystyle}
%%$
%%%\end{eqnarray*}
...
%\ENDIF
%\STATE{{\blue Play arm $I_t$ and observe $X_t(I_t)$.}}
%%\STATE{Observe $X_t(I_t)$ for playing arm $I_t$.}
%\IF{CUSUM$(I_t,X_t(I_t))==1$}
%\STATE{$\tau_{I_t}=t+1$; $cnt(I_t)=M$; reset CUSUM$(I_t,\cdot)$.}
%\ENDIF
%\ENDFOR
%\end{algorithmic}
%\end{algorithm}
...
%(s_{k}^+,s_{k}^-)=(y_k-\hat{y}_k-\epsilon,\hat{y}_k-y_k-\epsilon),
%\end{equation}
where $\hat{y}_k=\frac{1}{k}\sum_{s=1}^ky_s$. 
\section{Performance Analysis}\label{sec:analysis}
%\st{In this section, we first present the regret upper bound result of CD-UCB, which is of independent interest in understanding the challenges of non-stationary environment. Then we provide the performance guarantees of our modified CUSUM algorithm. Finally, we conclude the above results to provide the regret upper bound of CUSUM-UCB.}
...
%under the Assumption~\ref{ass:piecewise} and \ref{ass:detectability}.
...
%\begin{eqnarray}
%\mathbb{E}[\tilde{N}^C_T(i)]\leq  
%&\left(\gamma_T+\frac{\alpha T}{K\exp(h)}\right)\cdot\left(\frac{4B^2\xi\log T}{(\Delta_{\mu_T(i)})^2}+\pi^2/3\right) \\\nonumber
%&+1+\frac{\pi^2}{3}
%+\gamma_T\cdot ChK/\alpha+\alpha T/K,
%\end{eqnarray}
...
\end{eqnarray}
%where $\mathbb{E}[F]$ is the expected number of false alarms in time horizon $T$, and $\mathbb{E}[D]$ is the expected detection delay. 
\end{theorem}

Recall that the regret of the CD-UCB policy is upper-bounded by
$\sum_{i=1}^K\mathbb{E}[\tilde{N}_T(i)]$. 
%\st{Then we have the regret upper bound of CD-UCB.}
...
bandit algorithm.}
%\st{In particular, we obtain the following result by letting $\alpha=0$.}
{\blue By letting $\alpha=0$, we obtain the following result.} 
%\st{by letting $\alpha=0$.}
...
\emph{(CUSUM$|\hat{u}_0$)}
%{The Algorithm \ref{alg:CUSUM} first estimates $\hat{u}_0$.}
%\st{The CUSUM runs in the case of $|\hat{u}_0-u_0|<\epsilon$ with probability at least $1-2e^{-2\epsilon^2M}$.}
%{\blue [JH: I think you can describe these in the main text or leave them in the proof.]}
%Then, 
...
%\textcolor{blue}{JH: how about adding a table for regret comparison
%  (standard UCB, Discounting-UCB, SW-UCB, CUSUM-UCB, EXP3 (fix
%  $V_T$)))?}
\begin{table*}[t]
%\vspace{-0.2cm}
%\scriptsize
...
%  \begin{tablenotes}
%%\vspace{-0.2cm}
%	\item[a] $\gamma_T$ is the total number of breakpoints. $V_T$ is the total variation budget.
%%    \item[a] $\sharp$: 
%%    \item[b] $\dag$: 
%%    \item[c] $\ddag$: 
%  \end{tablenotes}
...
\end{equation}
%Note that we omit the constants related to $\epsilon$, $M$ and $\lambda$.
\end{corollary}
{\green
\begin{remark}
...
in an order sense.
% {\bf JH: It may be misreading if $\gamma_T$ scales with $T$. If we have some more space, we may need to explain $\Omega(\sqrt{T \gamma_T} )$ bound we explained in the rebuttal.}
\end{remark}
...
We evaluate the
%{the policies presented in Table~\ref{table:regret} over three different non-stationary environments.}
...
%\st{Flipping environment and switching environment are based on synthetic data and Yahoo! dataset is collected from real world experiments.}
%As reported in~\cite{mellor2013thompson}, we {\blue were} unable to{\blue reproduce the results of} Adapt-EvE {\blue in}~\cite{hartland2007change}{\blue that exploits the PHT test, due to the lack of algorithm description.}
%Instead, we implement another instance of CD-UCB with the PHT algorithm~\cite{hinkley1971inference} running as thechange detection algorithm, named PHT-UCB. Note that PHT-UCB is
...
%PHT 
%\st{is} 
%{\blue can be viewed as}
%a variant of Algorithm~\ref{alg:CUSUM} by replacing
%(\ref{twoCUSUM}) with
%\begin{equation}\label{PHT}
%(s_{k}^+,s_{k}^-)=(y_k-\hat{y}_k-\epsilon,\hat{y}_k-y_k-\epsilon),
%\end{equation}
...
\end{cases}.
\end{eqnarray}
%In the simulation, we set $T=10^5$, $\gamma_T=2$, $\epsilon=0.1$ and $M=100$. 
%h=50 or 5 \log (T/\gamma_T) \alpha = 0.1\sqrt{\gamma_T/T \log (T/\gamma_T)} or 0.001
...
%{\bf JH: Explain why Rexp3 and PHT-UCB are excluded.}
...
%\begin{figure}[t]
%  \centering
%    \includegraphics[width=0.45\textwidth]{figs/delta}
%     \caption{Regret performances under the flipping environment}
%     \label{fig:flip}
%\end{figure}
...
%\begin{figure}[t]
%  \centering
%    \includegraphics[width=0.45\textwidth]{figs/SWEN_all}
%     \caption{Regret performances under the switching environment}
%     \label{fig:switch}
%\end{figure}
...
%{\blue The behavior of passively adaptive policies is almost like a random policy that plays each arm uniformly, since they cannot identify breakpoints and fail to estimate the changed reward distributions in frequently changing environments.}
...
%We show that CUSUM-UCB obtains the first known regret upper bound for
%actively adaptive policies in the bandit feedback setting.
...
\end{align}
%By the Lemma~\ref{lem:Chernoff}, for any $z<0$, we have that
%\begin{equation}
%\mathbb{P}\{\hat{u}_0-u_0\leq z\}\leq \exp(-2z^2M).
%\end{equation}
%Thus, it holds that
%\begin{equation}
%p_{\hat{u}_0-u_0}(z)\leq -4zM\exp(-2z^2M),~~\forall z<0.
%\end{equation}
We define
...
\leq&\frac{h+1}{\lambda}.
\end{align}
%where the last step holds by the Assumption \ref{ass:lambda}. 
...
\STATE Initialize $\tau_i=1$ and $cnt(i)=M$ for each $i\in\mathcal{K}$.
\FOR{$t$ {\bfseries from} $1$ {\bfseries to} $T$}
\STATE{Update according to equations (\ref{eqn:update}).}
\IF{$cnt(i)>0$ for some $i\in\mathcal{K}$}
\STATE $I_t=i$; $cnt(i)=cnt(i)-1.$
\ELSE
\STATE{Update $I_t$ according to equation (\ref{eqn:It}).}
