...
%
% In these human-AI teams, trust plays a key role. It was observed in previous work that the user's lack of trust in the predictions of the system has a negative impact on the performance of the human-AI team \cite{bansalpdates}.
...
Our  approach significantly outperformed the baseline approach \cite{bansalpdates} in all domains in  terms of the $AUTC$ metric, achieving results that are as much as 90\% better on average and up to 300\% for certain users.
...
\citet{bansalpdates} propose the following definition for the compatibility score of an update:
%
...
\citet{bansalpdates} propose a way of modifying a loss function $L$ (e.g., Cross-Entropy Loss) such that the amount of penalty given for the predicted label of an instance depends on whether it corresponds to a newly introduced error:
%
\begin{equation}
...
This is the model of \citet{bansalyond}, that we consider as our baseline.
...
% %The following scores are added as features in $F(i)$, where $m_0$ is the basethat our method tries to improve upon:
% $s(m_0,T_j)$, $s(m_0,V_j)$, $s(m^*(T_j),V_j)$ and $s(m^*(V_j),T_j)$,  where  $m_0$ is  the baseline  model  of~\citet{bansalpdates}. These features   are   relevant because the best performing model on $T_j$ or on $V_j$ has a high likelihood of performing well on $Y_j$ as well. For  example,
...
% considered a modified version of the algorithm, which changes line 4 to select a model for $Y_j$ from three candidate models: the base$m_0$ (no personalization), the best model on the validation set $m^*(V_j)$,  and
...
% \begin{itemize}
%     \item $A_B$: Always selects the base$m_0$.
...
%
In this section we present two use cases in which the personalized model differed from the baseof Bansal et al.~\shortcite{bansalyond} and provided a better performance-compatibility tradeoff.
...
% Both the personalized model and the basebegin by splitting on the `random section` node which
...
Much work has been done in the field of human-AI interactions. The compatibility of an update to an AI-system is closely related to the \nth{14} Guideline for Human-AI Interactions from Amershi et al.'s work \cite{amershiuidelines} described as \emph{``Update and adapt cautiously: Limit disruptive changes when updating and adapting the
AI-system’s behaviors''}. In our case, this means making sure that the predictions made by the updated model conform to the user's expectations that developed prior to the update. It is related also to the \nth{5} step in an article from Google Design \cite{lovejoy2017} that states the  user's model evolve in tandem. For more references to   work section in the paper of \citet{bansal2019updates}.
%
\section{}
%
