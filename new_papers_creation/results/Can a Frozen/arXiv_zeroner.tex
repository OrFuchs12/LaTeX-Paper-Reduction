...
\begin{table*}
	...
	\scalebox{0.65}{
		\begin{tabular}{llcccccccccc}
			\toprule
			Relations                                             & Question templates                      & \textbf{BM25}    & \textbf{DPR-NQ} & \textbf{DPR-multi} & \textbf{DPR-PAQ} & \textbf{Wiki} $\bf{\Lambda}$ & \textbf{PAQ} $\bf{\Lambda}$ & \textbf{SPAR-Wiki} & \textbf{SPAR-PAQ} & \textbf{Contriever} & \textbf{Zero-NeR (Ours)} \\ %\multicolumn{2}{l}{Relations}
			\midrule
...
% The biggest limitation of our method is the necessity for large memory/storage to store the retrieval key sets, that and high search latency.
% In our experiments, 21 million Wikipedia passages produced a total of 222 million retrieval keys.
% Because each key vector has 768 dimensions of 32-bit floating point numbers, the total size of the keys reached 637 GiB.
% Though the number of keys is large, using an approximate nearest-neighbor search (ANNS) library, such as FAISS \citep{johnson2019billion} or SCaNN \citep{avq_2020}, can decrease the search latency.
% Some previous studies \citep[e.g.,][]{subramanya2019diskann} showed that millisecond-order mean latency can be achieved when searching a database of a billion points on a disk.
% These techniques could help to decrease the search latency of the retrieval.

% Another possible limitation of our method comes from the encoder for named entities.
% \citet{match_your_words} showed that neural retrievers have difficulty in generalizing terms that are unseen during training.
% This suggests that the quality of encoder pretraining limits the retrieval generalizability \citep{tanzer-etal-2022-memorisation}.
% As described in Section~\ref{sec:method_ner}, the LUKE model was trained on Wikipedia entities.
% This implies that the LUKE model cannot be generalized outside the range of Wikipedia.
% As described in Section~\ref{sec:popularity_bias}, the LUKE model suffers from a popularity bias against long-tailed named entities.
% Future work should address the long-tail problem for named entities to see if neural retrievers have generalizability beyond BM25.
% Also, we only use LUKE model as the encoder, though experiments with the embeddings of RoBERTa model cannot be used for retrieval (Section~\ref{sec:ablation_contextualized_embedding}).
% Future work should discuss with other language models in various domains.

% Our implementation might not be applicable to the domain outside of Wikipedia because we use LUKE pre-trained within Wikipedia domain.
% However, with our method, we only need to pre-train an encoder like LUKE in the new target domain.
% It is efficient because we can remove contrastive learning with the corpus of the domain, which conventional dense retrievers requires.
% The generalizability of the proposed method using the LM trained on other domains should be investigated in future work.

% In this paper, the number of named entities contained in a question is limited to one.
% This restriction is in response to EQ dataset whose question contains one entity name.
% If there are multiple named entities in a question, queries can be generated for each of them.
% For each of those queries, the similarity score with keys can be calculated and single score for each passage can be calculated with pooling.
% Future work would verify how to pool the scores or ensemble the results with other retriever in such case.


% \begin{tabular}{llccccc}
%     \toprule
%     Relations                                             & Question templates                      & \textbf{BM25}    & \textbf{DPR-NQ}  & \textbf{Contriever} & \textbf{SPAR}    & \textbf{Zero-NeR (Ours)} \\ %\multicolumn{2}{l}{Relations}
%     \midrule
%     P17                                                   & Which country is [E] located in?        & 61.5             & 56.6             & \underline{65.4}    & 0                & \textbf{65.6}            \\
%     P19                                                   & Where was [E] born?                     & \textbf{75.3}    & 26.0             & 63.0                & 0                & \underline{71.6}         \\
%     P20                                                   & Where did [E] die?                      & \textbf{80.4}    & 32.8             & \underline{70.6}    & 0                & 69.3                     \\
%     P26                                                   & Who is [E] married to?                  & \textbf{89.7}    & 25.1             & \underline{73.2}    & 0                & 63.6                     \\
%     P36                                                   & What is the capital of [E]?             & \textbf{90.6}    & 74.9             & \underline{89.7}    & 0                & 81.8                     \\
%     P40                                                   & Who is [E]'s child?                     & \textbf{85.0}    & 16.5             & \underline{66.4}    & 0                & 52.5                     \\
%     P50                                                   & Who is the author of [E]?               & 73.0             & \underline{75.7} & 74.6                & 0                & \textbf{82.0}            \\
%     P69                                                   & Where was [E] educated?                 & \textbf{73.1}    & 19.9             & 56.0                & 0                & \underline{61.1}         \\
%     P106                                                  & What kind of work does [E] do?          & \underline{71.2} & 19.9             & 66.2                & 0                & \textbf{79.6}            \\
%     P112                                                  & Who founded [E]?                        & \textbf{81.2}    & 74.7             & \underline{77.1}    & 0                & 74.7                     \\
%     P127                                                  & Who owns [E]?                           & \textbf{78.4}    & 46.5             & \underline{69.1}    & 0                & 64.9                     \\
%     P131                                                  & Where is [E] located?                   & \textbf{63.1}    & 44.1             & \underline{52.5}    & 0                & 50.4                     \\
%     P136                                                  & What type of music does [E] play?       & \underline{48.7} & 34.7             & 45.6                & 0                & \textbf{57.0}            \\
%     P159                                                  & Where is the headquarter of [E]?        & \textbf{85.0}    & 69.0             & \underline{79.7}    & 0                & 78.1                     \\
%     P170                                                  & Who was [E] created by?                 & \textbf{72.6}    & 33.4             & 59.7                & 0                & \underline{72.3}         \\
%     P175                                                  & Who performed [E]?                      & \underline{56.6} & 41.6             & 47.5                & 0                & \textbf{63.9}            \\
%     P176                                                  & Which company is [E] produced by?       & \textbf{81.0}    & 43.0             & 71.7                & 0                & \underline{74.5}         \\
%     P264                                                  & What music label is [E] represented by? & \underline{45.6} & 27.6             & 37.2                & 0                & \textbf{50.4}            \\
%     P276                                                  & Where is [E] located?                   & \textbf{84.9}    & 71.4             & \underline{78.3}    & 0                & 77.8                     \\
%     P407                                                  & Which language was [E] written in?      & \textbf{86.2}    & 72.9             & \underline{81.7}    & 0                & 79.1                     \\
%     P413                                                  & What position does [E] play?            & \underline{74.3} & \textbf{75.7}    & 69.3                & 0                & 73.0                     \\
%     P495                                                  & Which country was [E] created in?       & \textbf{21.8}    & 19.4             & \underline{21.5}    & 0                & \underline{21.5}         \\
%     P740                                                  & Where was [E] founded?                  & \underline{74.4} & 57.0             & 73.1                & 0                & \textbf{77.5}            \\
%     P800                                                  & What is [E] famous for?                 & \textbf{74.7}    & 24.4             & 64.7                & 0                & \underline{69.2}         \\
%     \midrule
%     \multicolumn{2}{l}{\textit{Arithmetic macro average}} & \textbf{72.0}                           & 45.1             & 64.7             & 0                   & \underline{67.1}                            \\
%     \multicolumn{2}{l}{\textit{Arithmetic micro average}} & \textbf{71.4}                           & 44.6             & 64.1             & 0                   & \underline{66.6}                            \\
%     \multicolumn{2}{l}{\textit{Best on}}                  & 16                                      & 1                & 0                & 0                   & 7                                           \\
%     \bottomrule
% \end{tabular}
