...
/Author (William E. Macke, Reuth Mirsky, Peter Stone)}
...
With recent developments in deep learning, several works were proposed for a sub-area of multiagent systems, where agents share information using learned communication protocols~\cite{hernandez2019survey,mordatch2018emergence,foerster2016learning}. These works make several assumptions not used in this work: that the agents can learn new communication skills, and that an agent can train with its teammates before execution. An exception to that is the work of Van Zoelen et al. \shortcite{van2020learning} where an agent learns to communicate both beliefs and goals, and applies this knowledge within human-agent teams. Their work differs from ours in the type of communication they allow.
...
\DIFdelbegin \DIFdel{In case of }\DIFdelend \DIFaddbegin \DIFadd{To account for }\DIFaddend stochastic policies, where a policy will generally produce more than one potential trajectory from a state, we introduce the \emph{expected} divergence point that considers all possible trajectories constructed from $\pi_2$.
\begin{definition}
...
\noindent Computing \textsc{edp} directly from this equation is non trivial. We therefore rewrite \textsc{edp} as the following \DIFdelbegin \DIFdel{bellman }\DIFdelend \DIFaddbegin \DIFadd{Bellman }\DIFaddend update:
\begin{theorem}
...
\begin{description} 
\item [Zone of Information ($Z_I$)] given the location of the teammate and two of its possible goals \DIFaddbegin \DIFadd{$g_1,g_2$}\DIFaddend , $Z_I$ is the \DIFdelbegin \DIFdel{set of timesteps }\DIFdelend \DIFaddbegin \DIFadd{interval }\DIFaddend from the beginning of the plan \DIFdelbegin \DIFdel{and for as long as there may still be some ambiguity regarding these goals. }\DIFdelend \DIFaddbegin \DIFadd{up to the worst case distinctiveness of $g_1,g_2$ for recognizing the teammate's goal, as defined by Keren et al. }\shortcite{keren2014goal}\DIFadd{.
%DIF >  the interval during which there exists a shared prefix to minimal-cost plans to
}


%DIF >  set of timesteps from the beginning of the plan interval during which there might exist an action $a$ that the teammate can execute such that $a$ is the next action in a minimal-cost plans to both goals.

%DIF >  \textbf{OR: during which there is still ambiguity regarding these goals when the teammate is following the most ambiguous plan. } %Intuitively, these are all the timesteps in which the plan of the ad hoc agent can change from querying.
\DIFaddend Intuitively, these are the timesteps where the ad hoc agent \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{might }\DIFaddend gain information by querying about these goals.
\DIFaddbegin 

\DIFaddend \item [Zone of Branching ($Z_B$)] given the location of the ad hoc agent and two possible goals of the teammate, $Z_B$ is the \DIFdelbegin \DIFdel{set of timesteps starting when }\DIFdelend \DIFaddbegin \DIFadd{interval from the worst case distinctiveness of $g_1,g_2$ for recognizing }\DIFaddend the ad hoc agent\DIFdelbegin \DIFdel{is required to commit to a specific goal , which means it }\DIFdelend \DIFaddbegin \DIFadd{'s goal and up to the end of the episode.
Intuitively, these are the timesteps where the ad hoc agent }\DIFaddend might take a non-optimal action if it could \DIFaddbegin \DIFadd{not }\DIFaddend disambiguate between the two \DIFdelbegin \DIFdel{possible }\DIFdelend goals of its teammate.
%DIF < Intuitively, these are all the timesteps in which the plan of the ad hoc agent can change from querying.
\DIFaddbegin 

%DIF >  is the interval starting when the ad hoc agent is required to commit to a specific goal to the end of the episode, which means it might take a non-optimal action if it could not disambiguate between the two possible goals of its teammate.
...
%DIFDELCMD < %%%
\DIFdelend % Given these zones for each subset of goals $G'$, we can identify the \textbf{Critical Querying Point (CQP)} as the first timestep inside $Z_Q(G')$, and is the first timestep in which the ad hoc agent should consider whether to query ``Is your goal one of the stations in $G'$?''. If $Z_Q(G')$ is empty, then there is no time in which this query can be useful and $CQP(G')=-1$.  
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %In Figure~\ref{fig:toolFetchingDomain_a}, some of the critical querying points are $CQP(\{1\})=CQP(\{2\})=CQP(\{1,3\})=6$, as it takes the fetcher 5 timesteps to reach the toolbox and only then it enters $Z_B$ for goals 1 and 2. $CQP(\{3\})=-1$, as by the time the fetcher reaches the toolbox, the worker has already reached or passed station 3.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %In previous work, we introduced the concepts of zones of information, branching and querying, which were based on worst case scenarios of when an agent would not know optimal actions. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend These existing definitions of zones consider the \emph{worst case} for disambiguating goals. However, the worst case might be highly uncommon, and planning when to query accordingly can induce high costs. 
\DIFaddbegin 

\DIFadd{Additional useful notation used in the following definition is the }\emph{\DIFadd{Uniformly-Random-Optimal Policy}} \DIFadd{(or }\textsc{\DIFadd{urop}}\DIFadd{) $\hat{\pi}_{g}$, that denotes the policy that considers all plans that arrive at $g$ with minimal cost, samples uniformly at random from the set of these plans, and then chooses the first action of that plan. }\DIFaddend As we now have the requirements to compute the \emph{expected case} for disambiguation of goals, we introduce definitions for expected zones: %, or the average periods of time where an agent does not know an optimal action.
%Consider two potential goals $g_1, g_2$. We can define the following expected zones:
\begin{definition}
The \textbf{Expected Zone of Information $eZ_Q$} given the location of the teammate and two goals $g_1, g_2$, $eZ_I$ is the expected time period \DIFdelbegin \DIFdel{where a }\DIFdelend \DIFaddbegin \DIFadd{during which the teammate's }\DIFaddend plan is ambiguous between $g_1$ and $g_2$:
\begin{equation*}
    eZ_I(s, g_1 | g_2) = \{t| t \leq \textsc{edp}(s, \DIFdelbegin \DIFdel{\pi}\DIFdelend \DIFaddbegin \hat{\pi}\DIFaddend _{g_1} | \DIFdelbegin \DIFdel{\pi}\DIFdelend \DIFaddbegin \hat{\pi}\DIFaddend _{g_2})\}
\end{equation*}
where \DIFdelbegin \DIFdel{$\pi_{g_1}$, $\pi_{g_2}$ are the policies }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\pi}_{g_1},\hat{\pi}_{g_2}$ are the }\textsc{\DIFadd{urop}}\DIFadd{s }\DIFaddend of the teammate \DIFdelbegin \DIFdel{that consider all plans that arrive at }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $g_1$ and $g_2$\DIFdelbegin \DIFdel{respectively with minimal cost, and uniformly chooses the next action from them.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  and uniformly at random chooses the next action from among them.
}\DIFaddend % uniformly randomly selects from all plans that reach $g_1$, $g_2$ with minimal cost.
...
\end{equation*}
where \DIFdelbegin \DIFdel{$\pi_{g_1}$, $\pi_{g_2}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\pi}_{g_1}$, $\hat{\pi}_{g_2}$ }\DIFaddend are the \emph{ad hoc agent's} \DIFdelbegin \DIFdel{policies that uniformly produce all plans assuming that the teammate is reaching $g_1$, $g_2$ with minimal cost.%DIF < , and $\pi_{g_2}$ is the policy for reaching $g_2$.
}\DIFdelend \DIFaddbegin \textsc{\DIFadd{UROP}}\DIFadd{s.%DIF >  that produce all plans with uniform probability assuming that the teammate is reaching $g_1$, $g_2$ with minimal cost.%, and $\pi_{g_2}$ is the policy for reaching $g_2$.
...
\DIFaddend Given a \textsc{somali cat} \DIFdelbegin \DIFdel{problem }\DIFdelend \DIFaddbegin \DIFadd{domain }\DIFaddend $\langle S, A_A, A_T, T, C\rangle$, \DIFdelbegin \DIFdel{and a plan $p^*$ that has minimal cost in this domain: %DIF < , we can define the \emph{Marginal Cost} of a plan $p$
}%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{The Marginal Cost ($MC$) of a plan $p$ in a domain with a plan $p^*$ of minimal costis $MC(p) = cost(p) - cost(p^*)$.
}%DIFDELCMD < \label{def:mc}
%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Intuitively, the marginal cost of a plan $p$ estimates the difference in costs between an }\DIFdelend \DIFaddbegin \DIFadd{we want to 
quantify how much, in terms of expected plan cost, the }\DIFaddend ad hoc agent \DIFdelbegin \DIFdel{with full knowledge about the goal of its teammate, and an ad hoc agent that follows plan $p$. For example, if a plan had a cost of $50$ while the minimum cost achievable in a domain is $30$, then the marginal cost of this plan is $20$.
We aim to minimize the marginal cost of plans generated by the ad hoc agent, so }\DIFdelend \DIFaddbegin \DIFadd{can gain from asking a specific query.
%DIF > find the query that gives us the minimum expected cost. 
Therefore, }\DIFaddend we define the value of a query as follows:
\begin{definition}
\textbf{Value of a Query} Let $q$ be a query with possible responses \DIFdelbegin \DIFdel{$r\in R$, $P$ be the set of plans the ad hoc agent may take with no new information, and $P_r$ }\DIFdelend \DIFaddbegin \DIFadd{$R$, and $\mathcal{P}_g$ }\DIFaddend be the set of \DIFdelbegin \DIFdel{plans the ad hoc agent may take given response $r$}\DIFdelend \DIFaddbegin \DIFadd{possible plans of both agents that arrive at goal $g$}\DIFaddend . Then the value of query $q$ is
\DIFdelbegin \DIFdel{:
}\begin{displaymath}
    \DIFdel{V(q) = \mathbb{E}_{p\in P}[MC(p)] - \sum\limits_{r\in R}P(r)*\mathbb{E}_{p_r \in P_r}[MC(p_r)]
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
    \DIFadd{V(q) = \mathbb{E}_{p\in \mathcal{P}_g}[cost(p)] - \sum\limits_{r\in R}P(r)*\mathbb{E}_{p\in \mathcal{P}_g}[cost(p) | r]
\label{eq:value}
}\end{equation}\DIFaddend 
\end{definition}

\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{Computing the expected cost of this set of plans is non-trivial. We therefore define the following concept:
}\begin{definition}
\DIFadd{The }\emph{\DIFadd{Marginal Cost}} \DIFadd{of a plan $p$ in a }\DIFaddend \textsc{somali cat} \DIFdelbegin \DIFdel{, this value can be easily computed:
the expected marginal }\DIFdelend \DIFaddbegin \DIFadd{domain with a goal $g$ ($MC_g(p)$) is the difference between the cost of $p$ and the cost of a minimal-cost plan that arrives at $g$.
}\label{def:mc}
\end{definition}

\noindent \DIFadd{By using algebra we can replace $cost(p)$ in Equation~\ref{eq:value} with $MC_g(p)$ to yield an equivalent formula that is easier to compute:
}\begin{equation}
\DIFadd{\small
\begin{split}
    V & (q) = \\ 
...
\DIFadd{where $c^*_g$ is the minimal }\DIFaddend cost of a \DIFdelbegin \DIFdel{query-less plan }\DIFdelend \DIFaddbegin \DIFadd{plan that arrives at $g$.
}

\DIFadd{In }\textsc{\DIFadd{somali cat}}\DIFadd{,
%DIF > By using simple algebra, substituting \emph{Marginal Cost} for cost in Definition~\ref{def:value} yields an equivalent formula. Furthermore, in \textsc{somali cat}, the expected \emph{Marginal Cost} can be easily computed: 
$\mathbb{E}_{p\in\mathcal{P}_g}[MC_g(p)]$ }\DIFaddend at state $s$ is proportional to the \DIFdelbegin \DIFdel{amount of time }\DIFdelend \DIFaddbegin \DIFadd{number of timesteps in which }\DIFaddend the ad hoc agent doesn't know which ontic actions are optimal, or in other words \DIFdelbegin \DIFdel{$\mathbb{E}_{p\in P}\propto \sum\limits_{g\in G}P(g)*|\bigcup\limits_{g'\in G}eZ_Q(s, g' | g)|$.
%DIF < In the Tool Fetching Domain discussed below, we assume the ad hoc agent takes the \emph{NOOP} action when it doesn't know which ontic actions are optimal, so the proportionality constant for this domain is $1$ as the ad hoc agent incurs an additional cost of $1$ for each timestep it doesn't act.
}\DIFdelend \DIFaddbegin \DIFadd{$\mathbb{E}_{p\in \mathcal{P}_g}[MC_g(p)]\propto |\bigcup\limits_{g'\in G}eZ_Q(s, g' | g)|$.
}\DIFaddend 

\DIFaddbegin \DIFadd{In addition, notice that computing $V(q)$ using Equation~\ref{eq:v_q} assumes that the goal of the teammate, $g$, is known. However, the ad hoc agent does not know the true goal $g$ ahead of time, so we need to consider the expected value of a query given each of the possible goals being the teammate's true goal, or $\mathbb{E}_{g\in G}[V(q) | g]$.
}

%DIF >  Given a \textsc{somali cat} domain $\langle S, A_A, A_T, T, C\rangle$, and a plan $p^*$ that has minimal cost: %, we can define the \emph{Marginal Cost} of a plan $p$
%DIF >  \begin{definition}
%DIF >  The Marginal Cost ($MC$) of a plan $p$ in a domain with a plan $p^*$ of minimal cost is $MC(p) = cost(p) - cost(p^*)$.
%DIF >  \label{def:mc}
%DIF >  \end{definition}

%DIF >  Intuitively, the marginal cost of a plan $p$ estimates the difference in costs between an ad hoc agent with full knowledge about the goal of its teammate, and an ad hoc agent that follows plan $p$. For example, if a plan had a cost of $50$ while the minimum cost achievable in a domain is $30$, then the marginal cost of this plan is $20$. We aim to minimize the marginal cost of plans generated by the ad hoc agent, so we define the value of a query as follows:
%DIF >  \begin{definition}
%DIF >  \textbf{Value of a Query} Let $q$ be a query with possible responses $r\in R$, $P$ be the set of plans the ad hoc agent may take with no new information, and $P_r$ be the set of plans the ad hoc agent may take given response $r$. Then the value of query $q$ is:
%DIF >  \begin{equation*}
%DIF >      V(q) = \mathbb{E}_{p\in P}[MC(p)] - \sum\limits_{r\in R}P(r)*\mathbb{E}_{p_r \in P_r}[MC(p_r)]
%DIF >  \end{equation*}
%DIF >  \end{definition}

%DIF >  In \textsc{somali cat}, this value can be easily computed: the expected marginal cost of a query-less plan at state $s$ is proportional to the amount of time the ad hoc agent doesn't know which ontic actions are optimal, or in other words $\mathbb{E}_{p\in P}[MC(p)]\propto \sum\limits_{g\in G}P(g)*|\bigcup\limits_{g'\in G}eZ_Q(s, g' | g)|$. %In the Tool Fetching Domain discussed below, we assume the ad hoc agent takes the \emph{NOOP} action when it doesn't know which ontic actions are optimal, so the proportionality constant for this domain is $1$ as the ad hoc agent incurs an additional cost of $1$ for each timestep it doesn't act.


\DIFaddend % Given a \textsc{somali cat} problem $\langle S,A_T,A_A,T,C\rangle$, consider a query $q$ with potential answers $\{answer_1, answer_2, answer_3\}$. We define the value of a query to be as follows:
% \begin{equation*}
...
Our policy for determining whether or not to query at each timestep is shown in Algorithm~\ref{alg:query}. It takes as input the Zone of Querying for each pair of goals, $Z_Q$, the expected Zone of Querying $eZ_Q$, the current set of possible goals $G$, \DIFdelbegin \DIFdel{he }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend ad hoc agent’s current belief of the teammate’s goal $P$ and the current timestep $t$. First the algorithm checks if the current timestep is within a Zone of Querying of two goals or more. If not, then the agent knows of an optimal ontic action and no querying is required. Otherwise, we then find the best possible query given $eZ_Q$, and only ask this query if its value is greater than its cost.
...
\DIFdelbegin %DIFDELCMD < \If{$t \in Z_Q(g_1, g_2) \text{for any } g_1,g_2 \in G$}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \If{$t \in Z_Q(g_1, g_2) \text{ for any } g_1,g_2 \in G$}
\DIFaddbegin \If{$\exists g_1,g_2\in G \textit{  s.t.  } t\in Z_Q(g_1,g_2))$}
\DIFaddend \State $query \gets ChooseQuery(G, eZ_Q, P)$
...
\label{alg:query}
\end{algorithm}
To optimize the \DIFaddbegin \DIFadd{expected }\DIFaddend value of a chosen query, we define a binary vector $\textbf{x}$ for each possible query, such that $x_i$ is 1 if and only if $g_i$ is included in that query. We then optimize for the value of a query above over these vectors using a genetic algorithm. \DIFaddbegin \DIFadd{We use a population size of $50$ and optimize for $100$ generations. Members are selected using tournament selection, and then combined using crossover. Each bit in the two resulting members is mutated with probability $0.001$.
}\DIFaddend % \footnote{Implementation details of the genetic algorithm can be found in a code repository that is omitted for anonymity}. 
...
\DIFaddbegin \begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{Figures/ToolFetchingDomain_MultiTools.pdf}
    \caption{\DIFaddFL{Example instance of the tool fetching domain. Workstations are shown by the colored boxes, toolboxes are shown by the black boxes with a T, the fetcher is shown as the gray circle labeled with an F, while the worker is shown as a gray circle labeled with a W.}}
    \label{fig:domain_example}
\end{figure}

\DIFaddend Previous work in \textsc{somali cat} introduced an experimental domain known as the tool fetching domain~\cite{mirskypenny}. This domain consists of an ad hoc agent, the \emph{fetcher}, attempting to meet a teammate, the \emph{worker}, at some workstation with a tool. The worker needs a specific tool depending on which station is its goal, and the worker's goal and policy are unknown to the fetcher. It is the job of the fetcher to deduce the goal of the worker based on its actions, to pick up the correct tool from a toolbox and to bring it to the correct workstation. At each timestep, the agents can execute one ontic action each. Alternatively, the fetcher can query the worker with questions of the form ``Is your goal one of the stations $g_1,g_2...g_N$?", where $g_1, \ldots, g_N \subseteq G$ is a subset of all workstations, and the worker replies truthfully. All queries are assumed to have a cost identical to moving one step, regardless of the content of the query.
...
%DIFDELCMD < %%%
\DIFdelend We consider several goal distributions for the teammate's choice of a goal: a uniform distribution over all goals, the \DIFdelbegin \DIFdel{softmax of }\DIFdelend \DIFaddbegin \DIFadd{Boltzmann distribution over }\DIFaddend the negative of the worker's distances, and the \DIFdelbegin \DIFdel{softmax of }\DIFdelend \DIFaddbegin \DIFadd{Boltzmann distribution over }\DIFaddend the worker's positive distances. Intuitively, the latter two distributions respectively define workers that are most likely to prefer workstations that are closer or farther from their initial location. Results used a cost of $0.5$ when initiating a query and varied the per station cost of each query. All results are averaged over $100$ domain instances.
...
We compare the performance of Algorithm \ref{alg:query} (\emph{$eZ_Q$ Query}) against two baseline ad hoc agents: one baseline never queries, when it is uncertain of the teammate's goal, it waits for an optimal action to become clear (\emph{Never Query}). The other baseline is the algorithm \DIFdelbegin \DIFdel{used in previous work (}\emph{\DIFdel{BL}}%DIFAUXCMD
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{introduced by Mirsky et al. }\shortcite{mirskypenny}\DIFaddend , where the ad hoc agent chooses a random query once inside a $Z_Q$ \DIFaddbegin \DIFadd{(}\emph{\DIFadd{Baseline}}\DIFadd{)}\DIFaddend . In addition, we extended \DIFdelbegin \DIFdel{the }\DIFdelend \emph{\DIFdelbegin \DIFdel{BL}\DIFdelend \DIFaddbegin \DIFadd{Baseline}\DIFaddend } in two ways to make it choose queries in a more informed way\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. First }\DIFaddend by accounting for the changing cost of different queries, as well as for the probability distribution over the teammate's goals (\emph{\DIFaddbegin \DIFadd{BL:}\DIFaddend Cost+Prob}) \DIFdelbegin \DIFdel{and by choosing queries that reduce the ambiguity related to the ad hoc agent's next action -- }\DIFdelend \DIFaddbegin \DIFadd{The second method involves creating a set of stations that each ontic action would be optimal for, and then querying about the largest of these sets. This method first attempts to disambiguate }\DIFaddend which toolbox to reach first \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{and then attempts to disambiguate about the worker's station  (}\DIFaddend \emph{\DIFaddbegin \DIFadd{BL:}\DIFaddend Toolbox}). \DIFaddbegin \DIFadd{Additional details about this setup can be found in Appendix \ref{app:exp}.
}\DIFaddend 
...
%DIF <      \item Cost+Prob - Executes a query when in a $Z_Q$, but accounts for the changing cost of different queries, as well as for the probability distribution over the teammate's goals.
%DIF <      \item Toolbox - Executes a query when in a $Z_Q$, but chooses queries that reduce the ambiguity related to the ad hoc agent's next action -- which toolbox to reach first.
%DIF >      \item BL:Cost+Prob - Executes a query when in a $Z_Q$, but accounts for the changing cost of different queries, as well as for the probability distribution over the teammate's goals.
%DIF >      \item BL:Toolbox - Executes a query when in a $Z_Q$, but chooses queries that reduce the ambiguity related to the ad hoc agent's next action -- which toolbox to reach first.
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\linewidth]{Figures/ToolFetchingDomain_MultiTools.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Example instance of the tool fetching domain. Workstations are shown by the colored boxes, toolboxes are shown by the black boxes with a T, the fetcher is shown as the gray circle labeled with an F, while the worker is shown as a gray circle labeled with a W.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:domain_example}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %\subsection{Additional Domain?}

%\textbf{William TODO: Experimental setup, 50x50 grid, 20 goals, }

\section{Results}
We ran experiments in a $20\times 20$ grid, with $50$ workstations and $5$ toolboxes. Locations of the stations, toolboxes\DIFaddbegin \DIFadd{, }\DIFaddend and agents in each domain instance are chosen randomly.
\DIFaddbegin 

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figures/AAAI_Uniform.pdf}
    \caption{\DIFaddFL{Per-station cost vs. marginal cost of domains with uniform distribution over the goals of the worker.}}
    \label{fig:uniform}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{\DIFaddFL{Per station cost vs marginal cost of domain with softmax of the worker's distances to goals as distribution.}}
    \label{fig:dist}
\end{figure}

\DIFaddend Figure~\ref{fig:domain_example} shows an example of such a \DIFaddbegin \DIFadd{tool-fetching }\DIFaddend domain. We now compare the new algorithm with previous work and the heuristics described above. We demonstrate that \DIFdelbegin \DIFdel{our new algorithm }\DIFdelend \DIFaddbegin \emph{\DIFadd{$eZ_Q$ Query}} \DIFaddend is able to effectively leverage the additional information from our generalizations to obtain a better performance over previous work and the suggested heuristics, in terms of marginal cost (Definition \ref{def:mc}). 
\DIFaddbegin 

\DIFaddend Figure~\ref{fig:uniform} shows the marginal cost averaged over 100 domain instances 
% \commentr{domains, or instances? It's not clear how many instances in total each graph shows} 
with different per-station costs (x-axis) and a uniform probability distribution over the worker's goals. \emph{$eZ_Q$ Query} \DIFdelbegin \DIFdel{has an }\DIFdelend \DIFaddbegin \DIFadd{exhibits }\DIFaddend equivalent performance to other approaches when the per-station cost is $0$. As the per-station cost increases, the performance of the \emph{\DIFdelbegin \DIFdel{BL}\DIFdelend \DIFaddbegin \DIFadd{Baseline}\DIFaddend } \DIFdelbegin \DIFdel{-based }\DIFdelend methods dramatically worsen and cannot outperform \emph{Never Query} when the per-station cost is large enough. By contrast, the performance of \emph{$eZ_Q$ Query} continues to perform well and is robust to the varying query costs.

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=\linewidth]{Figures/AAAI_Uniform.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Per-station cost vs. marginal cost of domains with uniform distribution over the goals of the worker.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:uniform}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Figure~\ref{fig:dist} shows in a similar fashion the marginal cost averaged over 100 domain instances, but here the probability distribution of the goals of the worker is the \DIFdelbegin \DIFdel{softmax of }\DIFdelend \DIFaddbegin \DIFadd{Boltzmann distribution over }\DIFaddend the worker's distances each goal. This distribution means that the worker is more likely to choose goals that are farther from it. Again, \emph{$eZ_Q$ Query} performs similarly to the heuristics when the per-station cost is 0, but dramatically outperforms all other methods as this value increases.
\DIFdelbegin \DIFdel{Figure~\ref{fig:inv_dist} shows the average marginal cost with the softmax of the negative of the worker's distances to goals as the probability distribution. This means the worker is more likely to choose goals that are closer to it. In this scenario, }\emph{\DIFdel{Toolbox}} %DIFAUXCMD
\DIFdel{and }\emph{\DIFdel{Cost+Prob}} %DIFAUXCMD
\DIFdel{mildly outperform }\emph{\DIFdel{$eQ_Z$ Query}} %DIFAUXCMD
\DIFdel{when the per-station cost is 0. However, }\emph{\DIFdel{$eQ_Z$ Query}} %DIFAUXCMD
\DIFdel{is the only one to consistently outperform }\emph{\DIFdel{Never Query}} %DIFAUXCMD
\DIFdel{when the per-station cost increases. }\emph{\DIFdel{Never Query}} %DIFAUXCMD
\DIFdel{in this scenario performs extremely well since when the worker chooses a goal that's close to its current location, it reveals its true goal through its actions rather quickly. This dramatically reduces the overall need for querying in the first place. However, }\DIFdelend \DIFaddbegin \DIFadd{Additional results showing }\DIFaddend \emph{$eZ_Q$ Query} \DIFdelbegin \DIFdel{is still able to adapt and obtain performance that is consistently better than }\emph{\DIFdel{Never Query}}%DIFAUXCMD
\DIFdel{.
%DIF <  \begin{figure}
%DIF <      \centering
%DIF <      \includegraphics[width=\linewidth]{Figures/AAAI_WorkerDist.pdf}
%DIF <      \caption{Per station cost vs marginal cost of domain with softmax of the negative of worker's distances to goals as distribution.}
%DIF <      \label{fig:dist}
%DIF <  \end{figure}
}%DIFDELCMD < \begin{figure}
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=\linewidth]{Figures/AAAI_InverseWorkerDist.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Per station cost vs marginal cost of domain with softmax of the worker's distances to goals as distribution.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:dist}
%DIFDELCMD < \end{figure}
%DIFDELCMD <  %%%
\DIFdel{With two exceptions, our method performs significantly better than all other approaches with a p value $<0.05$. The first exception was }\emph{\DIFdel{Toolbox}} %DIFAUXCMD
\DIFdel{with a uniform distribution over goals and a per-station cost $\leq 0.1$ and the softmax of the negative of the worker's distance as the distribution over goals and a per-station cost of 0. The second exception was }\emph{\DIFdel{Cost+Prob}} %DIFAUXCMD
\DIFdel{with a uniform distribution over goals and per station cost $\leq 0.3$ and the softmax of the negative of the worker's distance as the distribution over goals and a per station cost of 0.  Our method failed to outperform the heuristics under these conditions mainly because there was less information for our method to leverage.
%DIF <   For instance, when the distribution over goals is uniform and the per-station cost is 0, asking ``is your goal one of stations 1, 2 or 3?" is effectively the same query as ``is your goal one of stations 1, 2, 3 or 4?". This makes it more difficult for our genetic algorithm to optimize, and can lead to worse performance.
}\DIFdelend \DIFaddbegin \DIFadd{outperforming the other approaches under an additional goal distribution can be seen in Appendix~\ref{app:res}.
}\DIFaddend 
...
Finally, there is an increased computational cost of using our new method over other approaches and the proposed heuristics. The following results are under the assumption that the \textsc{edp} computation is performed in advance, as these values are independent of the teammate's behavior and can be computed a priori regardless of the teammate's actions. Thus, the following time measurements do not include these offline computations. %However, this assumption is reasonable provided details of the domain are available ahead of time.
On average, all heuristics methods took $<0.1$ seconds to complete each simulation, while our method took on average $2.87$ seconds on an Ubuntu 16.04 LTS intel core i7 2.5 GHz. In practice, the increased time should not be a major detriment. If a robot is communicating with either a human or another robot, the major bottleneck is likely to be the communication channel (e.g. speech, network speed, decision making of the other agent) rather than this computation time. In addition, when using a genetic algorithm for optimization \DIFdelbegin \DIFdel{, our method }\DIFdelend \DIFaddbegin \DIFadd{as we do in this paper, the }\emph{\DIFadd{$eQ_Z$ Query}} \DIFadd{computation }\DIFaddend should only grow in terms of $O(|G|^2)$ with the number of goals (assuming that \textsc{edp} is precomputed ahead of time and more members/generations aren't used with larger numbers of goals).
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=\linewidth]{Figures/AAAI_WorkerDist.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
The scope of this work is still limited to a specific type of \DIFdelbegin \DIFdel{settings}\DIFdelend \DIFaddbegin \DIFadd{setting}\DIFaddend , namely \textsc{somali cat} problems. However, the \textsc{edp} formalization opens up new avenues for investigating other %can be beneficial to plan for other 
types of \textsc{cat} scenarios, such as an agent that can advise or share its beliefs with its teammates. We \DIFdelbegin \DIFdel{conjure }\DIFdelend \DIFaddbegin \DIFadd{conjecture }\DIFaddend that the $eZ_Q$ algorithm can be modified relatively \DIFdelbegin \DIFdel{easy }\DIFdelend \DIFaddbegin \DIFadd{easily }\DIFaddend to address such challenges, as long as the ad hoc agent remains the initiator of the communication. It might be more challenging to extend this work to domains in which the teammate is the one to initiate the communication, as other works have investigated in the context of reinforcement learning agents \cite{torrey2013teaching,cui2018active}. Nonetheless, this work provides \DIFaddbegin \DIFadd{the }\DIFaddend means to investigate collaborations in ad hoc settings in new contexts, while presenting concrete solutions for planning in \textsc{somali cat} settings.  
...
\appendix
\section{\DIFadd{Experimental Setup}}
\label{app:exp}
\DIFadd{This is where we will add more details about the experimental setup.
}

\section{\DIFadd{Additional Results}}
\label{app:res}\DIFadd{la
}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figures/AAAI_WorkerDist.pdf}
    \caption{\DIFaddFL{Per station cost vs marginal cost of domain with softmax of the negative of worker's distances to goals as distribution.}}
    \label{fig:inv_dist}
\end{figure}
\DIFadd{Figure~\ref{fig:inv_dist} shows the average marginal cost with the Boltzmann distribution over the negative of the worker's distances to goals as the probability distribution. This goal distribution means that the worker is more likely to choose goals that are closer to it. In this scenario, }\emph{\DIFadd{BL:Toolbox}} \DIFadd{and }\emph{\DIFadd{BL:Cost+Prob}} \DIFadd{mildly outperform }\emph{\DIFadd{$eQ_Z$ Query}} \DIFadd{when the per-station cost is 0. However, }\emph{\DIFadd{$eQ_Z$ Query}} \DIFadd{is the only one to consistently outperform }\emph{\DIFadd{Never Query}} \DIFadd{when the per-station cost increases. }\emph{\DIFadd{Never Query}} \DIFadd{in this scenario performs extremely well since when the worker chooses a goal that's close to its current location, it reveals its true goal through its actions rather quickly. This dramatically reduces the overall need for querying in the first place. However, }\emph{\DIFadd{$eZ_Q$ Query}} \DIFadd{is still able to adapt and obtain performance that is consistently better than }\emph{\DIFadd{Never Query}}\DIFadd{.
%DIF >  \begin{figure}
%DIF >      \centering
%DIF >      \includegraphics[width=\linewidth]{Figures/AAAI_WorkerDist.pdf}
%DIF >      \caption{Per station cost vs marginal cost of domain with softmax of the negative of worker's distances to goals as distribution.}
...
%DIF >  \end{figure}
 ...
 \DIFadd{With two exceptions, }\emph{\DIFadd{$eQ_Z$ Query}} \DIFadd{performs significantly better than all other approaches with a p value $<0.05$. The first exception was }\emph{\DIFadd{BL:Toolbox}} \DIFadd{with a uniform distribution over goals and a per-station cost $\leq 0.1$ and the Boltzmann distribution over the negative of the worker's distance as the distribution over goals and a per-station cost of 0. The second exception was }\emph{\DIFadd{BL:Cost+Prob}} \DIFadd{with a uniform distribution over goals and per station cost $\leq 0.3$ and the Boltzmann distribution over the negative of the worker's distance as the distribution over goals and a per station cost of 0.  Our method failed to outperform the heuristics under these conditions mainly because there was less information for our method to leverage. 
