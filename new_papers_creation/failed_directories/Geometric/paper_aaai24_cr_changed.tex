...
% Emerged as the widely adopted paradigm in diffusion models, denoising diffusion probabilistic models (DDPMs) \cite{ddpm_20_ho,dpm_15_jascha} have shown remarkable performance in various generation tasks. By gradually adding Guassian noises during the forward process, the input data is converted into predefined noisy distributions, which get iteratively denoised in the sampling phase. Such approach that trains diffusion models at each time steps enables better performance with respect to accuracy, effieciency, and training difficulty compared to end-to-end methods like variational autoencoders (VAEs) \cite{vae_13_kingma} and generative adversarial networks (GANs) \cite{gan_14_goodfellow}. Recent applications of diffusion models in computational chemistry \cite{edm_22_hoogeboom,targetdiff_23_guan} and biology \cite{diffab_22_luo,protseed_23_shi} have shown remarkable performances. In the context of molecule generation tasks, adjusting molecule conformations successively is highly consistent with the core principle of molecular dynamics.
...
% There are a number of attempts based on autoregressive models (ARs) \cite{gschnet_19_wallach,gshperenet_22_luo}, normalizing flows (NFs) \cite{moflow_20_zang,graphaf_20_shi}, variational autoencoders (VAEs) \cite{jtvae_18_jin,cgvae_18_liu}, energy-based models (EBMs) \cite{graphebm_21_liu}, and et al.
...
% To address the quest for equivariance of generated 3D conformations, several diffusion-based methods \cite{edm_22_hoogeboom,mdm_23_huang} model molecules indirectly through atomic distances. However, recent research \cite{moleformer_23_yuan} indicates that the contribution of bond angles on molecular learning is equivalent to pair-wise distances. Only a limited number of methods make full use of spatial information. Additionally, since molecular diffusion methods only act on points cloud, traditional graph convolutions are unable to discriminate the importance of different atoms. In light of these challenges, we design a novel dual track molecular learning framework named \textbf{Dual-track Transformer Network} (DTN). By integrating global transformer architecture, DTN serves as the denoising kernel to comprehensively capture spatial geometric information.
...
% MoDi \cite{modi_22_raab} utilizes structure-aware neural filters and 3D convolutions to promotes dedicated control over each joint.
...
% G-ShpereNet \cite{gshperenet_22_luo} is also an auto-regressive model based on SphereNet \cite{spherenet_22_liu} which is an equivariant network that learns 3D geometry via distances, bond angles, and torsion angles.
...
% ConfGF \cite{confgf_21_shi} regards gradients fields of atomic coordinates' log density as pseudo-force that act on atoms. GeoDiff \cite{geodiff_22_xu} systematically discuss the equivariance of Markov chain in diffusion process.
...
%  E-NFs designs a E(n) equivariant network named EGNN  EDM implement the same network to diffusion model and for one-shot generation. Another diffusion-based method MDM augments edge construction and uses dual SchNet for atom prediction.


% Though these models are capable to generate novel and valid 2D molecule graphs, abundant spatial information that is of great influence on molecular properties is left unexpored.
...
% In 3D moleuclar learning, equivariance is a fundamental condition that need to be satisfied. A model is considered E(n) equivariant if it is equivariant to rotations, translations, reflections, and permutations.
...
% Traditional graph neural networks, which are used to learn topology information of graph structured data, is simply permutaion equivariant. Though many efforts have been devoted to equipping GNNs with the above mentioned properties, we offer another perspective to tackle this issue.
...
\begin{table*}[t]
    ...
    % GFMDiff & \textbf{1.74} & \textbf{558} & \textbf{321} & \textbf{x} & \textbf{x} & \textbf{x} \\
...
% \subsection{Forward and denoising process}
% In the forward process of proposed GFMDiff, positions, atomic numbers, and atom valencies are gradually corrupted into a noisy prior distribution. The groud-truth conformation $G_0 = (P_0, X_0, A_0, V_0)$ is transformed by a Markov chain with a predefined variance schedule $\beta_t \in (0, 1), t=0, 1, ..., T$. The posterior distribution is defined as:
% \begin{eqnarray}
%     &q(G_{1:T}|G_0) = \prod^T_{t=1} q(G_t|G_{t-1}),& \\
...
% \end{eqnarray}
% where $h_t = {\rm concat}(x_t, a_t, v_t)$ represents atom features for convenience. To satisfy the quest for equivariance, atom coordinates cannot be directly introduced to diffusion before transformed to linear subspace where the center of gravity is zero \cite{enf_21_satorras,edm_22_hoogeboom,geodiff_22_xu}. Accordingly, both the noise distribution and the aforementioned posterior distribution are constrained to the same linear subspace. Though atom features are invariant to transformations, they are transformed to the same sacle as postions in the whole forward and reverse process.

% In the denoising process, we use the above mentioned denoising kernel to approximate the molecule conformation at each time step,
% \begin{eqnarray}
%     &p_\theta(G_{t-1}|G_t) = \mathcal{N}(G_{t-1}; \mu_\theta(G_t,t),\sigma_t^2 I),& \\
%     &\hat{G}_t = G_t / \alpha_t - \hat{\epsilon}_t * \sigma_t / \alpha_t ,&
% \end{eqnarray}
% where $\hat{\epsilon}_t$ is the output of the parameterized neural network.

% \subsection{Training obejective}
% For DDPMs, the typical target function is the variational lower bound of the log-likelihood of data. Following previous methods, we adopt the weighted square error of predictions and combine it with GFLoss with the weight of $\lambda$:
% \begin{eqnarray}
%     &\mathcal{L}_t = E_{\epsilon_t \sim \mathcal{N}(0, I)}[\frac{1}{2}\omega(t)(||\epsilon_t - \hat{\epsilon}_t||^2 + \lambda \mathcal{L}_{GF}(t))], & \\
%     &\mathcal{L}_{GF}(t) = ||\alpha_t(\hat{V}_{pred}(t) - V_t)||^2, &
% \end{eqnarray}
% where $\omega(t) = (1-{\rm SNR}(t)/{\rm SNR}(t-1))$.
...
% Whether DTN and GFLoss contribute to generating accurate molecule geometries needs to be find out.
...
% Results indicate that our method outperforms state-of-the-art (SOTA) models in multiple aspacts.
...
% Following the same setting from previous research, we split the QM9 training set into two halves, each consisting of 50K samples. The first half is used for training the EGNN \cite{egnn_21_satorras} classifier while the conditional generative method is trained on the later half.
...
% Given current performance on benchmark with large molecules, generating novel large samples needs further explorations.
