...
% \usepackage{fullpage} -- This package is specificy forbidden
% \usepackage{geometry} -- This package is specificy forbidden
% \usepackage{grffile} -- This package is specificy forbidden
% \usepackage{hyperref} -- This package is specificy forbidden
% \usepackage{navigator} -- This package is specificy forbidden
% (or any other package that embeds links such as navigator or hyperref)
% \indentfirst} -- This package is specificy forbidden
% \layout} -- This package is specificy forbidden
% \multicol} -- This package is specificy forbidden
% \nameref} -- This package is specificy forbidden
% \usepackage{savetrees} -- This package is specificy forbidden
% \usepackage{setspace} -- This package is specificy forbidden
% \usepackage{stfloats} -- This package is specificy forbidden
% \usepackage{tabu} -- This package is specificy forbidden
% \usepackage{titlesec} -- This package is specificy forbidden
% \usepackage{tocbibind} -- This package is specificy forbidden
% \usepackage{ulem} -- This package is specificy forbidden
% \usepackage{wrapfig} -- This package is specificy forbidden
% DISALLOWED COMMANDS
...
% That means  verbs (including short verbs like be, is, using,and go),
...
% to decrease the required replanning given new information. We then suggest \nir{Suggestion: We then suggest a local conflict resolution framework which defer conflict resolution until it occurs within a given threshold} an approximation that considers future collisions only when agents are in close proximity, thus owing us to scale to much larger problems. We provide experimental results illustrating the scala of our method with regard to both the number of agents and the number of inaccurate edges.
...
% The runtime of finding optimal plans for CMAPF is exponential in the number of agents. Therefore, the largest group dominates the planning time. Had the largest group size were smer than $k$, we sh obtain an optimal solution faster.  [roni: claims about runtime should be more careful. We can rephrase this a bit but I'm not sure it is needed]
...
Given any MAPF-IM problem $G_s=\langle V, \eko, \ekb, \eao, \eab \rangle$, the resulting snapshot is $snap(G_s)=\langle V, \eko \cup \eao, \ekb \cup \eab \rangle$, that is, we assume that  edges that are assumed to be open, are indeed open, and  edges that are assumed to be closed, are indeed closed.
...
% In MAPFUN a set of agents must navigate in a graph $G=\langle V, E \rangle$. However, the agents are given as input a graph specification $G_s=\langle V, E_k, E_u \rangle$. $G$ denotes the true world graph, while $G'$ denotes the current knowledge that the agents have about $G$, ced the {\em snapshot}.
% The set of vertices $V$ is identical in $G$ and $G_s$. $E_k$ denotes the set of known edges, that is $\for e \in E_k, e \in E$. $E_u$ denotes the of unknown edges, that is, there may be some edges $e\in E$, $e\notin E_u$, and there may be some edges $e' \in E_u, e' \notin E$.

% \guy{Suggesting below a symmetric definition.}

% In MAPFUN a set of agents must navigate in a graph $G=\langle V, E \rangle$. The agents are given as input a graph specification $G_s=\langle V, E_k,E_{\overline{k}}, E_u,E_{\overline{u}} \rangle$. $G$ denotes the true world graph, while $G'$ denotes the current knowledge that the agents have about $G$, ced the {\em snapshot}.
% The set of vertices $V$ is identical in $G$ and $G_s$. $E_k$ denotes the set of known edges, that is $\for e \in E_k, e \in E$. $E_{\overline{k}}$ denotes the set of edges that are known to be missing, that is $\for e \in E_{\overline{k}}, e \notin E$.
% $E_u$ and $E_{\overline{u}}$ denote uncertain edges. There may be some edges $e\in E_u$, $e\notin E$, and there may be some edges $e' \in E_{\overline{u}}, e' \in E$. That is, $E_u$ contains edges that the agents believe may exist, but may actuy be missing from the graph $G$, while $E_{\overline{u}}$ may contain edges that the agents believe are missing, but may exist in $G$.
...
% \plan{Implicit observations - observation relation $O(v,e)$ between vertex and edge. When reaching a vertex you observe  edges that satisfy the relation. After observing you get a new snapshot $G_{s'}$ where some edges move from $E_u$ to $E_k$.}
...
% \begin{itemize}
%     \item $E'_k = E_k \cup \{ \langle v_j,v_l \rangle | \exists v_i \in V_c, O(v_i,\langle v_j,v_l \rangle),  \langle v_j,v_l \rangle \in E\}$ -- the set of  edges that were detected from the new positions of the agents.
%     \item $E'_{\overline{k}} = E_{\overline{k}} \cup \{ \langle v_j,v_l \rangle | \exists v_i \in V_c, O(v_i,\langle v_j,v_l \rangle),  \langle v_j,v_l \rangle \notin E\}$ -- the set of  non existing edges that were detected from the new positions of the agents.
%     \item $E'_u = E_u \setminus \{ \langle v_j,v_l \rangle \in E_u | \exists v_i \in V_c, O(v_i,\langle v_j,v_l \rangle),  \langle v_j,v_l \rangle \notin E\}$ -- the set of  edges that the agents assumed to exist but were detected to be missing.
%     \item $E'_{\overline{u}} = E_{\overline{u}} \cup \{ \langle v_j,v_l \rangle \in E_{\overline{u}} | \exists v_i \in V_c, O(v_i,\langle v_j,v_l \rangle),  \langle v_j,v_l \rangle \in E\}$ -- the set of  edges that the agents assumed were missing, but were detected to exist.
% \end{itemize}


% \plan{Solution - policy $\pi(G_s,\langle v_1,...,v_n \rangle)$ specifies which action each agent should take given the current snapshot and  agent locations.}
...
% We now describe an extension to CMAPF that we c \emph{multi-agent path finding under uncertain maps (MAPFUM)}
...
% \guy{I think that we now need a definition of sensing. I suggest to define a distance metric $d(v,v')$, and say that if $d(v,v')<c$ then an agent in $v$ can sense  outgoing edges from $v'$. Alternatively, maybe we just want a relation and not a distance metric, because I do not think that line of sight respects the triangle inequality.}
...
% In MAPFUM, a subset of edges might be added or removed. We refer to these subset as \emph{uncertain edges (UEs)}. Initiy, the agents are not aware whether an edge is traversable. Thus, the UEs refers to as the \emph{online} part of the problem, whereas the map and set of agents refers to as the \emph{offline} part of the problem. \guy{This is not what online and offline usuy refer to. Typicy, offline is solving before acting, and online means interleaving solving and acting. Better not to overload terms.}
...
% An online MAPFUM starts by solving an offline MAPF problem. While the agents following their destined trajectories, they \emph{observe} which edges are traversable. An \emph{observation} is a triplet, $\langle e, s, t\rangle$, where $e$ is an uncertain edge, $s$ is the edge \emph{status}, $t$ is the timestep. Upon perceiving a new observation, a replanning is required, take into account the new knowledge about the map. Formy, MAPFUM is a quintuplets $\langle G, A, UE, F, O \rangle$, where G, A are defined as in CMAPF. $UE \subset E$ is the set of uncertain edges.
% \plan{Sensing obstacles - within given range - we see  blocked and unblocked cells}
...
% The set of actions in MAPFUM contains move and wait actions, as well as the observe action, which precedes both of them \guy{we need to discuss this - do we want implicit or explicit observations. I think that implicit observations make more sense here. Your definition seems implicit, but then there is not need for an "observe" action - it is enough to say that the agent observes  edges within range.}. That is, an action starts with observing adjacent edges. If new observation is received, new paths are generated, considering the current knowledge about the map. Otherwise, the previously planned trajectories are followed.
...
% $$\for e \in UE, \for t_{1}, t_{2} \in \mathbb{N}^+: F(e, t_{1}) = F(e, t_{2})$$
...
% $\Pi^{t_{\infty}}$ represents the last generated plan afterwards  agents attained their goals.
...
% It is the sum over the partial plans until  agents reached their goals.
...
% $$\for t_{i}: SOC(\Pi^{t_{i}})=SOC(OPT^{t_{i}})$$
...
\Input{Snapshot $G_s=\langle V, \eko, \ekb, \eao, \eab \rangle$, Observation relation $O$, Set of agents $A$}
    ...
    \While{not  agents reached their goals}{
...
Whenever new information becomes available to one agent, it communicates the information to  agents. Then, the agents may replan given the new information.
...
Below, we describe our online algorithms. We begin with a generic replanning algorithm, emphasizing the main components of  replanning methods. Algorithm~\ref{alg:generic} shows this generic algorithm. The algorithm begins with initializing the multi-agent plan (line 3), and then the agents start acting.

At each step the agent update the current snapshot given the observation function $O$, that is,  edges that are observable from the current positions of the agents are observed, and the agents communicate their observations to form a centralized new snapshot. If the snapshot is identical to the previous one, then the existing plan is still valid, and the agents can continue to act.
    ...
    \Input{new snapshot $G_s$, ongoing policy $\pi$,  set of agents $A$, current timestep $t$}
        ...
        \While{$ \pi' = \emptyset \wedge |A_r| < |A|$}{
            ...
            \If{$\pi'=\emptyset$}{
                ...
                $A_r \leftarrow A_r \cup \{a_j \in A_{\overline{r}} \mid \mbox{P}(j)>\hat{p} \}$ \\\tcp{add  unaffected agents with priority higher than the lowest priority affected agent}
            }
        }
        \If{$\pi' = \emptyset$}{
            $\pi' \leftarrow PP(G, A)$
...
%%%%%%%%%%%%%%% COMMENNNNNTED OUT
% Algorithm~\ref{alg:local} describes this approach. The algorithm has 3 main subroutines : initialization (line 3), where an initial plan is computed, replanning after new information has arrived (line 7), and collision resolution (line 10). We need to check for proximate collisions (line 9) and resolve them, because the computed plans ignore future collisions. Again, as with the approach that resolves  conflicts (Algorithm~\ref{alg:generic}), there can be various implementations for its methods.

% It is possible to use a CMAPF solver, such as CBS or PP (with or without ID), for  3 subroutines. All these algorithms can be adapted such that they resolve only collisions that occur within at most $r$ steps, ignoring farther collisions~\cite{li2021lifelong}.
...
\begin{figure*}[tbhp]
    ...
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      ...
      {Figures/maze/mixed_sm_figures/Cactus-and-legend.png}
      % {Figures/maze/mixed_sm_figures/Cactus.png}
      % \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/DeltaSOC(UEs).png}
      % \caption{Maze grids results.}
      % \label{fig:rt-soc-maze}
    \end{subfigure}
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      \includegraphics[width=\columnwidth]{Figures/warehouse/mixed_sm_figures/Cactus.png}
      % \includegraphics[scale=.2]{Figures/warehouse/mixed_sm_figures/DeltaSOC(UEs).png}
      % \caption{Warehouse grids results.}
      % \label{fig:rt-soc-warehouse}
    \end{subfigure}
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      \includegraphics[width=\columnwidth]{Figures/room/mixed_sm_figures/Cactus.png}
      % \includegraphics[scale=.2]{Figures/room/mixed_sm_figures/DeltaSOC(UEs).png}
...
\begin{figure*}[tbhp]
    ...
    \centering
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      \includegraphics[width=\columnwidth]{Figures/maze/mixed_sm_figures/cactus_with_dashed_lines.png}
      % {Figures/maze/mixed_sm_figures/Cactus.png}
      % \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/DeltaSOC(UEs).png}
      % \caption{Maze grids results.}
      % \label{fig:rt-soc-maze}
    \end{subfigure}
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      \includegraphics[width=\columnwidth]{Figures/warehouse/mixed_sm_figures/cactus_with_dashed_lines.png}
      % \includegraphics[scale=.2]{Figures/warehouse/mixed_sm_figures/DeltaSOC(UEs).png}
      % \caption{Warehouse grids results.}
      % \label{fig:rt-soc-warehouse}
    \end{subfigure}
    \begin{subfigure}[b]{0.69\columnwidth}\centering
      \includegraphics[width=\columnwidth]{Figures/room/mixed_sm_figures/cactus_with_dashed_lines.png}
      % \includegraphics[scale=.2]{Figures/room/mixed_sm_figures/DeltaSOC(UEs).png}
      % \caption{Room grids results.}
      % \label{fig:rt-soc-room}
    \end{subfigure}
...
%       \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/Cactus.png}
%       \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/DeltaSOC(UEs).png}
%       \caption{Runtime and $\Delta$ SOC for Maze grids.}
%       \label{fig:rt-soc-maze}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.95\columnwidth}\centering
%       \includegraphics[scale=.2]{Figures/warehouse/mixed_sm_figures/Cactus.png}
%       \includegraphics[scale=.2]{Figures/warehouse/mixed_sm_figures/DeltaSOC(UEs).png}
%       \caption{Runtime and $\Delta$ SOC for Warehouse grids.}
%       \label{fig:rt-soc-warehouse}
%     \end{subfigure}\\
%     \begin{subfigure}[b]{0.95\columnwidth}\centering
%       \includegraphics[scale=.2]{Figures/room/mixed_sm_figures/Cactus.png}
%       \includegraphics[scale=.2]{Figures/room/mixed_sm_figures/DeltaSOC(UEs).png}
...
%       \includegraphics[width=\columnwidth]{Figures/maze/mixed_sm_figures/Cactus.png}
%       % \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/DeltaSOC(UEs).png}
%       \caption{Maze grids results.}
%       \label{fig:rt-soc-maze}
%     \end{subfigure}
...
% The first trend holds for  grids: Local+CBS yields lower solution cost than the PP-based algorithm, which is reasonable since CBS is geared toward finding lowest-cost solutions.
...
\begin{figure}[tbh]
    ...
    \end{subfigure}\\
      ...
      \includegraphics[width=0.47\columnwidth]{Figures/maze/mixed_sm_figures/Success-Rate(k)_pos=100_ID_ONLY.png}
      \includegraphics[width=0.47\columnwidth]{Figures/maze/mixed_sm_figures/Success-Rate(UEs)_k=50_ID_ONLY.png}
      % \caption{Success-Rate for Maze grids.}
      % \label{fig:mazesrk}
    \end{subfigure}
    \begin{subfigure}[b]{\columnwidth}\centering
      \includegraphics[width=0.47\columnwidth]{Figures/warehouse/mixed_sm_figures/Success-Rate(k)_pos=100_ID_ONLY.png}
      \includegraphics[width=0.47\columnwidth]{Figures/warehouse/mixed_sm_figures/Success-Rate(UEs)_k=40_ID_ONLY.png}
      % \caption{Success-Rate for Warehouse grids.}
      % \label{fig:G}
    \end{subfigure}
    \begin{subfigure}[b]{\columnwidth}\centering
      \includegraphics[width=0.47\columnwidth]{Figures/room/mixed_sm_figures/Success-Rate(k)_pos=100_ID_ONLY.png}
      \includegraphics[width=0.47\columnwidth]{Figures/room/mixed_sm_figures/Success-Rate(UEs)_k=30_ID_ONLY.png}
...
\begin{figure}[tbh]
    ...
    \end{subfigure}\\
        \begin{subfigure}[b]{\columnwidth}\centering
      \includegraphics[width=0.47\columnwidth]{Figures/maze/mixed_sm_figures/Success-Rate(k)_pos=100.png}
      \includegraphics[width=0.47\columnwidth]{Figures/maze/mixed_sm_figures/Success-Rate(UEs)_k=50.png}
      % \caption{Success-Rate for Maze grids.}
      % \label{fig:mazesrk}
    \end{subfigure}
    \begin{subfigure}[b]{\columnwidth}\centering
      \includegraphics[width=0.47\columnwidth]{Figures/warehouse/mixed_sm_figures/Success-Rate(k)_pos=100.png}
      \includegraphics[width=0.47\columnwidth]{Figures/warehouse/mixed_sm_figures/Success-Rate(UEs)_k=40.png}
      % \caption{Success-Rate for Warehouse grids.}
      % \label{fig:G}
    \end{subfigure}
    \begin{subfigure}[b]{\columnwidth}\centering
      \includegraphics[width=0.47\columnwidth]{Figures/room/mixed_sm_figures/Success-Rate(k)_pos=100.png}
      \includegraphics[width=0.47\columnwidth]{Figures/room/mixed_sm_figures/Success-Rate(UEs)_k=30.png}
      % \caption{Success-Rate for Room grids.}
      % \label{fig:Gs0}
    \end{subfigure}
...
Figure~\ref{fig:success-rate}(left) plots the success rate ($y$-axis) of each algorithm for an increasing number of agents ($x$-axis),  to 100. As the effect of ID is clear from Figure~\ref{fig:cactus-w-dashed-lines}, we report only the variants with ID.
The same trends ob PP  significantly reduce the algorithms' runtime and hence increase the success rate.  The same trends are also observed in Figure~\ref{fig:success-rate}(right), where success rate is 
...
% First, consider the results for Maze maps, given in figure~\ref{fig:maze_mixed_}.
% \plan{SuccessRate(k)}
...
%         \centering
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=0.2]{Figures/maze/mixed_sm_figures/Success-Rate(k)_pos=100.png}
%       \caption{Success-Rate(k)}
%       \label{fig:mazesrk}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=.2]{Figures/maze/mixed_sm_figures/Success-Rate(UEs)_k=50.png}
...
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=0.2]{Figures/warehouse/mixed_sm_figures/Success-Rate(k)_pos=100.png}
%       \caption{Success-Rate(k)}
%       \label{fig:G}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=.2]{Figures/warehouse/mixed_sm_figures/Success-Rate(UEs)_k=40.png}
%       \caption{Success-Rate(UEs)}
%       \label{fig:Gs0}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
...
%         \centering
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=0.2]{Figures/room/mixed_sm_figures/Success-Rate(k)_pos=100.png}
%       \caption{Success-Rate(k)}
%       \label{fig:G}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
%       \includegraphics[scale=.2]{Figures/room/mixed_sm_figures/Success-Rate(UEs)_k=30.png}
%       \caption{Success-Rate(UEs)}
%       \label{fig:Gs0}
%     \end{subfigure}
%     \begin{subfigure}[b]{0.45\columnwidth}\centering
...
%1) Tomer Shahar, Shashank Shekhar, Dor Atzmon, Abdah Saffidine, Brendan Juba, Roni Stern:
...
For example, in MAPF-TU~\cite{shahar2021safe}  edges are known but there are lower and upper bounds on the time it takes to traverse each edge, while in MAPF-DP agents' actions may be delayed with some proba~\cite{honig2016multi, wagner2017path, atzmon2020probabilistic}.
...
discuss MAPF in partiy known environments.
These papers tackle different uncertainty than we do. %, and avoid using plan trees.
Closest to our MAPF-IM problem is recent work on MAPF-OU \cite{shofer2023multi}, where a relatively sm subset of edges may be blocked.
%They assume a-priori knowledge of a relatively sm set of possibly blocked edges.
MAPF-IM extends this problem setting in two ways. First, we ow both for edges that are considered   
...
% \citet{queffelec2021planning,queffelec2023complexity}  discuss MAPF in partiy known environments. They focus on complexity analysis, given constraints such as disowing collisions, communication, and finite horizon.

% They suggest two offline algorithms that construct plan trees for  agents, branching on the possibly blocked edges. Given the relatively sm set of possibly blocked edges, these plan trees can have manageable size.
...
% In stochastic MAPF \cite{levy2022online}, an agent may stochasticy move in a different direction than intended. Aside from the difference in uncertainty, stochastic problems can be optimized for minimizing expected costs, which does not hold for the non-stochastic case.
...
%They studied a different online variant of MAPF ced lifelong MAPF (LMAPF), where the set of %agents is fixed but an agent reaching its goal will receive a new goal to go to.
...
%3) Our work with Bar.
%There they focused offline planning, and require knowledge of  possible obstacle locations. We %plan online, and do not require any a-priori knowledge of where obstacles may appear.
...
% Third, our sensing action is more flexible, owing sensing edges not only from their immediate vertices. [[Roni: risky]]
 in MAPF-IM they are key constraints.
%they attempt to find a policy that with high proba minimizes cost, focusing on the exploration strategy, while we attempt to provably minimize costs when the assumptions over the edges are accurate. Also, research in CTP does not consider the problem of collisions, which pose a major chenge for us.
...
%They also typicy limit their attention to physical environments and sensors, while we study a more abstract problem setting. \roni{Last sentence should be commented out in my opinion as it is not clear. }
...
% The setting and objective in MR-SLAM is typicy different: they assume a mostly unknown environment and the objective is to map the environment, while we focus on a mostly known environment and the objective is to navigate the agents towards their goals.
% %They also typicy limit their attention to physical environments and sensors, while we study a more abstract problem setting. \roni{Last sentence should be commented out in my opinion as it is not clear. }
% MR-SLAM research mostly focuses on synchronizing the information collected from the noisy sensors of agents into a single map. Subsequently they assume that navigation is less important. Our work can hence be considered as complementing work on MR-SLAM, assuming perfect sensors and simple map synchronization, and focusing instead on the navigation problem.
...
%             Replan for  the agents in Affected\\
%             Update $\pi$
%             break
%         }
%     }
...
% If new open edge:  are affected
% Else
%     If PP, highest priority agent that planned to path in the blocked cell and  lower priority agents
...
%     \While{not  agents reached their goals}{
%         If change in snapshot -- c \textbf{AdaptPlan( agents, change description)} to update $\pi$ \\
...
%     Option 1: replan  according to new snapshot
...
%             If change in snapshot -- c \textbf{AdaptPlan(partition change description)} to update $\pi$ \\
%         Execute next step in $\pi$ \\
%     }
% \end{algorithm}
...
% 3. Other comparisons #1: is there a benefit in snapshopt optimal in terms of SOC:  solvers
% Expected: (a) CBS-based better than PP (b) Full finds better local, (c) IA and RA are the same
% Also expected: Full and Local are practicy the same.
...
% 5. Other comparison #3: impact of type of changes (only open, only closed, idey: graph with middle grounds)
