...
Second, if the heuristic is \emph{consistent} then \astar is guaranteed to never expand a node more than once~\cite{hartNR68Astar}. As we show later, \kastar does not share this property. % Added for reviewer R2.1. 
...
Such \emph{node re-expansions} can negatively affects the runtime of \kastar. For the single-goal shortest path problem, there are even cases where the number of node re-expansions done by \astar is exponential in the number of unique states in the graph~\cite{martelli1977complexity}. % R2.2
...
and Lemma 1 in Holte and Zilles more recent paper on the optimality of cost-algebraic \astar~\cite{holte2019optimal}.  % R2.3
...
Consistency of heuristic aggregation function, as we defined it, and consistency of heuristic function, serve different purposes. 
The former are is used to allow \kastar to be admissible given a set of consistency heuristic functions, while the latter is used to guarantee that \astar does not need to re-expand nodes. We discuss this in detail below. % R2.4


% the standard definition of consistency in heuristic search in two ways. First, it is a property of a heuristic aggregation function and not of an individual heuristic function. 
% Second, as we show later, using an \axiomcons heuristic aggregation function does not guarantee that node re-expansions will be avoided. 
...
% The difference between Definitions~\ref{def:axiomcon} and~\ref{def:reexp} are small, but are important. As we discussed above, the former is a necessary and sufficient condition for \kastar to be admissible given a set of consistent heuristic functions. 
% But, it does not guarantee that \kastar will not need to re-expand nodes. 
% The latter definition fills this gap, as prove below. % R2.5
...
\begin{figure}
    \centering
    \subfloat{\includegraphics[width=0.45\textwidth]{min-vs-max-left.png}}\hfill
    \subfloat{\includegraphics[width=0.45\textwidth]{min-vs-max-right.png}}
...
%   \max(\vect{h}(n) - \vect{h}(m)) &\leq d(n, m)
%     \label{eq:const-proof-step1}
% %    \Phi(\vect{h}(n)) - \Phi(\vect{h}(t_i)) & \leq d(n, t_i) & \text{($\Phi$ is \axiomcons and $h_i(t_i) = 0$)}\\
% \end{align}

% % aggregation consistency:
% % $ \Phi(\vect{v}) \leq \Phi(\vect{u}) + \max(\vect{v}-\vect{u})$

% % $ \Phi(\vect{v}) - \Phi(\vect{u}) \leq \max(\vect{v}-\vect{u})$


% % [[RONI: I'M HERE]]
% % What we want is a constraint on $\Phi$ that gives us the following.


% %  \begin{align*}
% %  \Phi(\vect{v}) - \Phi(\vect{u}) \leq SOMETHING \leq d(n,m) \\
% %  \end{align*}
% %  where $\vect{v}=\vect{h}(n)$ and $\vect{u}=\vect{h}(m)$
 
% %  OPTION 1: => works, <= false
% %  SOMETHING = 0
 
% %  OPTION 1': => works, <= stuck
% %  SOMETHING = $\max(\vect{v}-\vect{u})$
 
% %  OPTION 1'': => works, <= stuck
% %  if max(v-u) >= 0 then Phi(v) - Phi(u) <= $\max(\vect{v}-\vect{u})$
% %  (otherwise, we have max(u-v) >= min(u - v) > 0 and then Phi(u) - Phi(v) <= max(u-v)) $\max(\vect{v}-\vect{u})$
 
 
% %  OPTION 2
% % $SOMETHING(\vect{h}(n), \vect{h}(m)) \leq d(n,m)$
% %  we know that since the h-functions are consistent, this should help us
% %  $\forall t  h_t(n)-h_t(m) \leq d(n,m) $

% %  OPTION 3: => stuck, <= probably works
% %  SOMETHING = $|\max(\vect{v}-\vect{u})|$
 
% %  a+Qv - c-Qu = 1/6(Qv-Qu) -1/6m(v-u) +Qv - 5/6(Qv-Qu)-1/6m(v-u)-Qu
% %              = 2/6Qv-2/6Qu-2/6m(v-u)
             
% % if 1/2(Qv-Qu)+1/2m(v-u) < 0 then m(v-u) <Qv-Qu < -m(v-u), so v-u < 0
...
% %  We need:
% %  (1) d(s,n) + Q(\vect{h}(n)) <= d(s,m) + Q(\vect{h}(m))
% %  We know:
% %  (2) Q(\vect{h}(n)) - Q(\vect{h}(m)) > max_i(h_i(n)-h_i(m))   [[Q is not consistent]]
% %  %(3) h(n)-h(m) <= d(n,m)                                  [[local h are consistent]     
% %  (4) h(m)-h(n) <= d(m,n)                                  [[local h are consistent]
% %  (5) d(s,n) > d(s,m) + d(m,n)                             [[Reexpansion is needed to find optimality]]
% %  (6) d(s,n) > 0, d(s,m) > 0, d(m,n) > 0

% % Challenge:
% % We want to find quantities such that (2)-(6) hold and (1) holds as well.
 
% %  [From(4)]
% %   (4.5) max_i(h_i(m)-h_i(n)) <= d(m,n)  
% %  [From (5)]
% %   (5.5) - d(m,n) > d(s,m) - d(s,n)                              
% %   (5.75) 

% % %  (7) Q(\vect{h}(n)) - Q(\vect{h}(m)) <= d(s,m) -d(s,n) 
% %  [side change for (1) gives us]
% %  (7) Q(\vect{h}(n)) - Q(\vect{h}(m)) <= d(s,m) - d(s,n)

 
% %  Can it happen that 
% %   (8) d(s,m) - d(s,n) >= max_i(h_i(n)-h_i(m))
 
% %  It can if this can happen:
% %   (9) - d(m,n) >= max_i(h_i(n)-h_i(m))
% %   (9) 0 >= max_i(h_i(n)-h_i(m)) + d(m,n)
 
 
 
% %  A problem:
% %  can it happen that:
% %  (9) Q(\vect{h}(m))-Q(\vect{h}(n)) < 0   ???
% %   == Q(\vect{h}(n)) > Q(\vect{h}(m))
 
 
% %  \begin{align*}
% %  someFunction\vect{h}(n)-\vect{h}(m)\leq d(n,m)
% %  \end{align*}

% \noindent Now, since $\Phi$ is \axiomcons, then 
% \begin{align}
% %    \Phi(\vect{h}(n))\leq & \max(\vect{h}(n)-\vect{h}(t_i))+\Phi(\vect{h}(t_i)) & \\
%     \Phi(\vect{h}(n))-\Phi(\vect{h}(m))\leq & \max(\vect{h}(n)-\vect{h}(m)) & \\
%     \Phi(\vect{h}(n))-\Phi(\vect{h}(m))  \leq & d(n, m)  & \text{ (due to (\ref{eq:const-proof-step1}))}\\
%         \Phi(\vect{h}(n))  \leq &d(n, m) + \Phi(\vect{h}(m)) & \\
...
%   Second, we assume that $\Phi$ is not \axiomcons, and show that there exists an \omspp instance, a tuple of consistent heuristics $\vect{h}$, and a node $n$ such that when \kastarphi expands $n$ while $g(n) > d(s,n)$.
%   Since $\Phi$ is not \axiomcons, there exists vectors $\vect{v}$ and $\vect{u}$ such that $\Phi(\vect{v}) - \Phi(\vect{u}) > \max (\vect{v} - \vect{u})$.
% %  We observe that $\Phi(\vect{v}) > \Phi(\vect{u}) + \delta$.\\ \roni{Observe where? is this pointing to some figure?}
%   Let $a = \frac{1}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) - \frac{1}{6}\max (\vect{v} - \vect{u})$ and $b = \frac{1}{2}(\Phi(\vect{v}) - \Phi(\vect{u})) + \frac{1}{2}\max (\vect{v} - \vect{u})$ and $c = \frac{5}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) + \frac{1}{6}\max (\vect{v} - \vect{u})$ and make the following observations:
%   \begin{align}
%  a &> 0 \text{ and } b > 0 \text{ and } c > 0\\
%  c-(a+b) &= \frac{1}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) - \frac{1}{6}\max (\vect{v} - \vect{u}) > 0\\
%  a+b &< c\\
%  b &>  \max(\vect{v} - \vect{u}) \\
%  c &<  \Phi(\vect{v}) - \Phi(\vect{u}) \\
%  (a + \Phi(\vect{v})) - (c + \Phi(\vect{u})) &= \frac{2}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) - \frac{2}{6}\max (\vect{v} - \vect{u})\\
%  c + \Phi(\vect{u}) &< a + \Phi(\vect{v}) \\
% %  and $c <
%  \end{align}
% Consider the graph in Fig.~\ref{fig:kstarphi-bad-non-target}.
% The heuristics involved are consistent because for all $i$, we have $h_{i}(m) - h_{i}(n) = v_{i} - u_{i} \leq \max (\vect{v} - \vect{u}) < b = d(m, n)$ and therefore $h_{i}(m) \leq d(m, n) + h_{i}(n)$.
...
%   \begin{tikzpicture}
%     \node[source] (s) at (0, 0) {$s$};
%     \node[other]  (a) at (3, 1.5) {$m$};
%     \node[other] (b) at (6, 0) {$n$};

%     \draw[->] (s) -- (a) node[midway] (sa) {\phantom{0}};
%     \draw[->] (s) -- (b) node[midway] (sb) {\phantom{0}};
%     \draw[->] (a) -- (b) node[midway] (ab) {\phantom{0}};

%     \node[above left = -5mm of sa,edgeweight] {$a$};
%     \node[below = -2mm of sb,edgeweight] {$c$};
%     \node[above right = -5mm of ab,edgeweight] {$b$};

%     \node[left  = 2mm of  s,infobox] {$\vect{h}=\vect{0}$};
%     \node[right = 2mm of  a,infobox] {$\vect{h}=\vect{v}$};
%     \node[right = 2mm of  b,infobox] {$\vect{h}=\vect{u}$};
%   \end{tikzpicture}
%   \caption[Premature expansion for \kastarphi]{A counter-example where \kastarphi expand a node with the wrong $g$ value.
%   The start is $s$. For any $\vect{v}$, $\vect{u}$, and positive $a$, $b$, and $c$ such that $a+b < c$ and $a+\Phi(\vect{v}) > c+\Phi(\vect{u})$, running \kastarphi will expand $n$ with $g(n) = c$ while $d(s,m) = a+b$.
%   If $\Phi(v)-\Phi(u) < b$ then the heuristics are consistent.}
%   \label{fig:kstarphi-bad-non-target}
% \end{figure}
% }
...
% \begin{align}
%     \Phi(\vect{h}(n)) \leq  & \max(|\vect{h}(n)-\vect{h}(m)|) + \Phi(\vect{h}(m)) & \\
%     \Phi(\vect{h}(n))  \leq & d(n, m) + \Phi(\vect{h}(m)) & \text{ (due to (\ref{eq:const-proof-step1}))}\\
%         \Phi(\vect{h}(n))  \leq &d(n, m) + \Phi(\vect{h}(m)) & \\
%     g(n) + \Phi(\vect{h}(n))           \leq & g(n)+ d(n, m) + \Phi(\vect{h}(m)) & \\
%     F_\Phi(n)           \leq & d(s, m) + \Phi(\vect{h}(m)) &
%     \text{(by definition of $F_\Phi$ and $n$)}\\
...
%   Second, we assume that $\Phi$ is not \axiomcons, and show that there exists an \omspp instance, a tuple of consistent heuristics $\vect{h}$, and a node $n$ such that when \kastarphi expands $n$ while $g(n) > d(s,n)$.
%   Since $\Phi$ is not \axiomcons, there exists vectors $\vect{v}$ and $\vect{u}$ such that $\Phi(\vect{v}) - \Phi(\vect{u}) > \max (\vect{v} - \vect{u})$.  we observe that $\Phi(\vect{v}) > \Phi(\vect{u}) + \delta$.\\ \roni{Observe where? is this pointing to some figure?}
%   Let $a = \frac{1}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) - \frac{1}{6}\max (\vect{v} - \vect{u})$ and $b = \frac{1}{2}(\Phi(\vect{v}) - \Phi(\vect{u})) + \frac{1}{2}\max (\vect{v} - \vect{u})$ and $c = \frac{5}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) + \frac{1}{6}\max (\vect{v} - \vect{u})$ and make the following observations:
%   \begin{align}
%  a &> 0 \text{ and } b > 0 \text{ and } c > 0\\
%  c-(a+b) &= \frac{1}{6}(\Phi(\vect{v}) - \Phi(\vect{u})) - \frac{1}{6}\max (\vect{v} - \vect{u}) > 0\\
...
\begin{lemma}[Generic example of inadmissible \kastar]
For the \omspp instance defined by the graph depicted in Figure~\ref{fig:kstarphi-bad}, a start vertex $s$, and goals $t_i$ and $t_j$, 
if (1) $u_i = 0$, (2) $\delta > 0$, (3) $\Phi(\vect{v}) > \Phi(\vect{u}) + \delta$, and (4) $\epsilon = \frac{\Phi(\vect{v}) - \Phi(\vect{u}) - \delta}{3} > 0$, then running \kastarphi will find a suboptimal path to $t_i$.
\label{lem:genericExample}
\end{lemma}
\begin{proof}
  In the first iteration of \kastarphi, the vertex $s$ will be expanded, adding to \open the vertices $n$, $t_i$, and $t_j$, with $g$ values 
  $\epsilon$, 
  $\delta+2\epsilon$, 
  and $\Phi(\vect{v})$, respectively, and 
  $F_\Phi$ values 
  $\epsilon+\Phi(\vect{v})$, 
  $\delta+2\epsilon+\Phi(\vect{u})$, 
  and $\Phi(\vect{v})$, respectively. 
  To prove Lemma~\ref{lem:genericExample}, we show below that 
  \kastarphi will expand $t_i$ first and then $t_j$, halting afterwards without expanding $n$. The optimal path to $t_i$ goes through $n$, and thus it will not be found by \kastarphi, as required. 
  To show that $t_i$ is expanded first, we express $F_\Phi(t_i)$ in terms of $\epsilon$ and $\Phi(\vect{v})$ as follows: 
  \begin{align}
      F_\Phi(t_i)=&\delta+2\epsilon+\Phi(\vect{u}) \\
                 =&\delta+3\epsilon-\epsilon+\Phi(\vect{u}) \\
                 =&\delta+\Phi(\vect{v})-\Phi(\vect{u})-\delta -\epsilon+\Phi(\vect{u}) \\                 
                 =&\Phi(\vect{v})-\epsilon                                 
  \end{align}
  The table in the righthand side of Figure~\ref{fig:kstarphi-bad} lists the $F_\Phi$ values of $n$, $t_i$, and $t_j$ expressing only with $\Phi(\vect{v})$ and $\epsilon$, showing that indeed \kastarphi will halt before expanding $n$, as required. $\qed$
\end{proof}
\begin{figure}
  \centering
  \subfloat{
  \begin{tikzpicture}
    \node[source] (s) at (0, 2) {$s$};
    \node[other]  (a) at (3, 2) {$n$};
    \node[target1] (b) at (3, 0) {$t_i$};
  ...
  The start is $s$, nodes $t_i$ and $t_j$ are goals where $i\neq j$. For any $\vect{v}$, $\vect{u}$, $i$, $\delta$, and $\epsilon$ such that (1) $u_i = 0$, (2) $\delta > 0$, (3) $\Phi(\vect{v}) > \Phi(\vect{u}) + \delta$, and (4) $\epsilon = \frac{\Phi(\vect{v}) - \Phi(\vect{u}) - \delta}{3} > 0$, running \kastarphi will return a suboptimal path to $t_i$.   To see this, the table on the right shows the $g$, $\Phi(\vect{h})$, and $F_\Phi$ values for $n$, $t_i$, and $t_j$ after expanding $s$. The optimal path to $t_i$ is through $n$ and costs $\delta+\epsilon$, but \kastarphi will expand $t_i$ before $n$, returning a path of length $\delta+2\epsilon$ to $t_i$, which is suboptimal.}
...
%Theorem~\ref{thm:admissible} shows that with admissible heuristics, \axiomadm is a sufficient condition for proving that \kastarphi is admissible.
...
\begin{figure}
	...
	\begin{tikzpicture}
	\node[source]  (s)  at (0, 2) {$S$};
	\node[other]   (a)  at (3.8, 0) {$A$};
	\node[target1] (t1) at (3.8, 2) {$t_1$};
	\node[target2] (t2) at (0, 0) {$t_2$};
	...
	\node[above right = -5mm of sa,edgeweight] {$v_2$};
	\node[above = -2mm of st1,edgeweight] {$v_1 + v_2 +\frac{\epsilon}{3}$};
	\node[left  = -2mm of st2,edgeweight] {$v_1 + v_2 +\frac{2\epsilon}{3}$};

	\node[above = 1mm of s,infobox] {$\vect{0}$};
	\node[below = 0mm of a,infobox] {$\vect{v}$};
	\node[above = 1mm of t1,infobox] {$\vect{0}$};
	\node[below = 0mm of t2,infobox] {$\vect{0}$};
	\end{tikzpicture}
	} \\
	\subfloat{
	\begin{tabular}[b]{l@{\,}*{7}{c}}
	\toprule
	\multirow{2}{*}{Node} &     & \multicolumn{2}{c}{\astari{t_1}} & \multicolumn{2}{c}{\astari{t_2}} &  \multicolumn{2}{c}{\kastarphi} \\
	\cmidrule(r){3-4} \cmidrule(lr){5-6} \cmidrule(l){7-8}
	& $g$ & $h_1$ & $f_1$ & $h_2$ & $f_2$ & $\Phi(\vect{h})$ & $F$ \\
	\midrule
	$S$   & $0$ & $0$ & $0$ & $0$ & $0$ & $0$ & $0$ \\
	...
	$A$   & $v_2$ & $v_1$ & $v_1 + v_2$ & $v_2$ & $2v_2$  & $\Phi(\vect{v})$ & $v_1 + v_2 + \epsilon$  \\
	\bottomrule
	\end{tabular}
	}
	\caption{Generic example with consistent heuristics. %where \kastarphi does not expand a surely expanded node, $A$
    We assume that $v_1 \leq v_2$ and $\epsilon$ is defined as $\epsilon = \Phi(\vect{v}) - v_1$.
	The vector of heuristic values is displayed next to each node.
	%The table indicates which nodes are surely expanded ($S$) for \astari{t_1}, \astari{t_2}, and \kastarphi.
	}
	\label{fig:kastarphi-surely-skipped-generic}
\end{figure}
	
Second, we prove that if there exists $\vect{v}$ such that $\Phi(\vect{v}) < \min \vect{v}$, then there is an \omspp instance, a tuple of consistent heuristics, and a surplus node $A$ such that \kastarphi explores $A$.
Let $\vect{v}$ such that $\Phi(\vect{v}) < \min \vect{v}$.
Assume without loss of generality that $v_1 = \min \vect{v}$ and $v_2 = \max\vect{v}$.
Let $\epsilon = \Phi(\vect{v}) - v_1 <0$.
Consider the \omspp on Figure~\ref{fig:kastarphi-surely-skipped-generic}.
Since $\epsilon < 0$, we have $F(A) < F(t_2) < F(t_1)$.
Therefore, \kastarphi expands $A$.
On the other hand, $f_1(A) > f_1(t_1)$ and $f_2(A) > f_2(t_2)$, so $A$ is a surplus node.
Thus \kastarphi expands a surplus node.
...
\begin{figure}
	...
	\begin{tikzpicture}
	\node[source]  (s)  at (0, 1.5) {$S$};
	\node[other]   (a)  at (2.5, 0) {$A$};
	\node[target1] (t1) at (2.5, 1.5) {$t_1$};
	\node[target2] (t2) at (0, 0) {$t_2$};
	...
	\node[above right = -5mm of sa,edgeweight] {$2$};
	\node[above = -2mm of st1,edgeweight] {$1$};
	\node[left  = -2mm of st2,edgeweight] {$1$};

	\node[above = 1mm of s,infobox] {$\vect{0}$};
	\node[below = 0mm of a,infobox] {$\vect{0}$};
	\node[above = 1mm of t1,infobox] {$\tuple{0, 2}$};
	\node[below = 0mm of t2,infobox] {$\tuple{2, 0}$};
	\end{tikzpicture}
	}\hfill
	\subfloat{
	\begin{tabular}[b]{l@{\,}*{7}{c}}
	\toprule
	\multirow{2}{*}{Node} &     & \multicolumn{2}{c}{\astari{t_1}} & \multicolumn{2}{c}{\astari{t_2}} &  \multicolumn{2}{c}{\kastarmax} \\
	\cmidrule(r){3-4} \cmidrule(lr){5-6} \cmidrule(l){7-8}
	& $g$ & $h_1$ & $f_1$ & $h_2$ & $f_2$ & $\max(\tuple{h_1, h_2})$ & $F$ \\
	\midrule
	$S$   & $0$ & $0$ & $0$ & $0$ & $0$ & $0$ & $0$ \\
	$t_1$ & $1$ & $0$ & $1$ & $2$ & $3$ & $2$ & $3$ \\
	$t_2$ & $1$ & $2$ & $3$ & $0$ & $1$ & $2$ & $3$ \\
	...
	\caption{Max example with consistent heuristics where \kastarmax expands a surplus node.
	}
	\label{fig:kastarphi-surely-skipped-max}
\end{figure}

Third, if $\Phi = \max$ then we provide an \omspp instance, a tuple of consistent heuristics, and a surplus node $A$ such that \kastarphi explores $A$.
Consider the instance in Figure~\ref{fig:kastarphi-surely-skipped-max}.$\qed$


% Third, we prove that if there exists $\vect{v}$ such that $\Phi(\vect{v}) > \min \vect{v}$ and $\Phi$ is \axiomcons, then there is an \omspp instance, a tuple of consistent heuristics, and a surplus node $A$ such that \kastarphi explores $A$.
% Let $\vect{v}$ such that $\Phi(\vect{v}) > \min \vect{v}$.
% Assume without loss of generality that $v_1 = \min \vect{v}$ and $v_2 = \max\vect{v}$.
% Let $\epsilon = \Phi(\vect{v}) - v_1 >0$.
% Since $\Phi$ is \axiomcons, we have $\Phi(\vect{v}) - \Phi(\tuple{0, v_2}) \leq \max(\vect{v} - \tuple{0, v_2}) = v_1$.
% Therefore, $\epsilon = \Phi(\vect{v}) -v_1 \leq \Phi(\tuple{0, v_2})$.
% \abda{To be continued}
% %Consider the \omspp on Figure~\ref{fig:kastarphi-surplus-wasted}.
% %Since $\epsilon < 0$, we have $F(A) < F(t_2) < F(t_1)$.
% %Therefore, \kastarphi expands $A$.
% %On the other hand, $f_1(A) > f_1(t_1)$ and $f_2(A) > f_2(t_2)$, so $A$ is a surplus node.
% %Thus \kastarphi expands a surplus node.

% Fourth, we prove that if $\Phi$ is not \axiomcons, then there is an \omspp instance, a tuple of consistent heuristics, and a surplus node $A$ such that \kastarphi explores $A$.
% Let $\vect{u}$ and $\vect{v}$ such that $\Phi(\vect{v}) - \Phi(\vect{u}) > \max \vect{v} - \vect{u}$ and $u_i = 0$ for some $i$.
% Without loss of generality, assume $u_1=0$ and define $\epsilon = \Phi(\vect{v}) - \Phi(\vect{u}) - \max \vect{v} - \vect{u}$. $\qed$
\end{proof}
...
%$h_1(A) = v_1 \leq v_2 + h_1(S) = v_2$ so $h_1$ is consistent, and $h_2(A) = v_2 \leq v_2 + h_2(S) = v_2$ so $h_2$ is consistent as well.
...
%However, the $F$-value for $A$ is $v_2 + h(A) = v_2 + \Phi(\vect{v}) = v_2 + v_1 + \epsilon$ which is larger than the $F$ value for $t_1$ and $t_2$.
