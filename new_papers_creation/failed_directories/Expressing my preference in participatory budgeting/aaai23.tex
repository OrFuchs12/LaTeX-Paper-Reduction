...
%\usepackage{aaai23-nocopyright}  % for personal de-anonymized copies
...
% Possible options for titles:
% How to use participatory budgeting in the real world
% Improving real world participatory budgeting
% Methods to organize participatory budgeting in the real world
% Organizing participatory budgeting in the real world
% Elicitation and aggregation methods in real world participatory budgeting
\author{
    %Authors
...
\affiliations {
    % Affiliations
    \textsuperscript{\rm 1} Ben-Gurion University of the Negev, Israel\\
    \textsuperscript{\rm 2} Boston University, USA \\
    \textsuperscript{\rm 3} University of Edinburgh, UK\\
    royfa@post.bgu.ac.il, benade@bu.edu, kobig@bgu.ac.il
}
...
With respect to the aggregation rule, greedy aggregation leads to outcomes that are  highly sensitive to   the input format used and the fraction of the population that participates. The method of equal shares, in contrast, leads to outcomes that are not sensitive to the type of voting format used, and these outcomes are remarkably stable even when the majority of the population does not participate in the election.  These results carry valuable insights for PB practitioners and social choice researchers.
 ...
 \end{abstract}
%  \begin{abstract}
%  Participatory budgeting is an influential paradigm that engages     residents in the process of allocating a city’s budget.
% Implementations of PB  vary in the input format they use to
%  elicit participants’ preferences on potential projects.  Our goal is to find the  combination of input format   and aggregation method that satisfy objective  measures of   stability and welfare, as well as easy for people to use.
%  We conduct an extensive empirical study, in which more
% than 1800 voters used different methods in a controlled setting.   We measured the outcomes achieved by PB 
% and users' experience  for different input formats and  aggregation methods.
% Our results show that  the greedy aggregation method  that is currently used in PB settings  
%  is highly sensitive to the type of input format used. 
%  A new aggregation method that guarantees proportional outcomes is minimally dependent on the input format, while producing outcomes that are more stable than greedy aggregation.
%  We argue for combining this  aggregation rule with the popular K-approval input format  which  scores high on the usability scale.
%  \end{abstract}
