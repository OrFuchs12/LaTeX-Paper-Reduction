...
% We conjecture that finding an optimal solution to a given \samd problem is NP-hard. We base this conjecture on the fact that computing the probability that a \emph{task network} satisfies a deadline is NP-hard \cite[Theorem 5]{cohen2015estimating}. A task network consists of a set of tasks that must be performed, where some of these tasks should be performed in sequence and some in parallel. ~\cite[Lemma 5]{cohen2015estimating}
% proved that even for task networks that consist of a single sequence of tasks, computing the probability that it satisfies a deadline is  NP-hard. A given supplier assignment $\varphi$ in an \samd problem is equivalent to a task network that consists solely of a sequence of tasks. [[Roni: all this is said in one sentence below: we think it is NP hard because M() is hard.]
...
%However, this is not a formal proof since it migh
...
%and for each assignment we need to compute $M(T, \varphi, d)$. 
...
%Alternatively, there is a polynomial time algorithm that approximates $M(T, \varphi, d)$. Thus, we have two brute-force algorithms, one that computed an optimal solution in time that is exponential in two times the number of tasks $(2|T|)$, and an approximation that is exponential in the number of tasks $(|T|)$. 
...
%In the previous sections a theoretical analysis of the algorithm to solve the \samd problem was presented. Here we add experimental evaluation results to show this algorithm performance in practice. Using the \astar algorithm we compute the probability of the project to meet a deadline and perform this computation with other methods.
...
%In same cases, mostly when we analyze the resulted error, with a higher magnitude. \Roni{Bad English. What do you mean?}
%The first results we present shows how the deadline effects the error. In 
Figure~\ref{allDDRealError} shows how the deadline ($x$-axis) effects the error ($y$-axis). 
%we can see that for a very small deadlines, if the probability to meet the deadline is 0, the error is 0. \Roni{We cannot see this in the graph!} 
...
% Same for very high deadlines, if the probability to meet the deadline is 1, the error is 0. Between those two extreme deadline cases, the error increases till some pick and then decreases. We identify these trend both in \sampling and \expectation heuristic algorithms. For different deadlines, even on the same instance, the error my change dramatically. [[Roni: the above is just a list of observations without an explanation. Not good.]] For \astar heuristic algorithm, the resulted error is 0 for any deadline. [[Redundant: this is by definiiton]]
