...
% The bounded concealment assumption

% Sample complexity of PI-SAM
...
For any sub-trajectory $T'$ of a trajectory in $\mathcal{T}$ that ends in a state where literal $l$ is observed (i.e., $T'[-1].s(l)= \true$) 
    ...
    % \begin{itemize}
    %     \item Rule 1 [not an effect]. If $l=\true$ in $T'[-1].s$ and $|T'|=1$, then $\neg l$ is not an effect of $T'[-1].a$
    %     \item Rule 2 [an effect]. If $l=\false$ in $T'[0].s$ and $l=\true$ in $T'[-1].s$ then $\exists a\in T'.a$ that has $l$ as an effect.
    %     \item Rule 3 [must not delete]. If $l=\true$ in $T'[-1].s$ and $|T'|>1$, then if $\neg l$ is an effect of $T'[i].a$ for some $i$, then there must be $i'>i$ where $T'[i'].a$ has $l$ as an effect.
    % \end{itemize}
\end{observation}
