...
%\begin{table}[htb]
%      \begin{tabular}{l}
%        \emph{Be certain of what you know and be aware what you don't. That is wisdom.} \\
...
%A mature MRC system should be able to determine whether the retrieved context contains the answer before actually predicting the answer, and
...
%Recently, a new dataset called SQuAD2.0 \cite{Rajpurkar2018Know} was released, containing instances with unanswerable questions for a given context, as an example shown in Table \ref{table01}. It has become a flagship challenge, which

%requires the system not only to answer questions when possible but also to determine when no answer is supported by the passage and thus abstain from answering.





%Besides, the comprehension process should be as interpretable as possible. Consequently, the advance on MRC tasks can be achieved by two aspects,
...
%From the perspective of MRC system design,
...
%This work starts from this viewpoint and focuses on exploring useful applying patterns of language models instead of presenting better language models to enhance state-of-the-art MRC models further. We will show the manner to use a pre-trained language model may still have a massive impact on MRC performance, no matter how strong the language model itself is.
...
\begin{enumerate}
	%\item We surveyed and compared various decoder mechanisms based pre-trained LMs for MRC task, including the answer verifiers to assess if the question is answerable given the passage, and attention mechanisms to model the interactions of passage and question.
	%\item We proposed a new reading comprehension model based on the two-stage verification mechanism of synchronous verification and post verification.
	...
	% 		\item For the first time, we define the significance test for the concerned MRC task and show that our models are significantly better than the baselines.
\end{enumerate}
...
%Recently, Deep contextual language models have been shown effective for learning universal language representations by leveraging large amounts of unlabeled data, achieving many state-of-the-art results in a series of NLU benchmarks. Some prominent examples are
...
%, Generative Pre-trained Transformer (OpenAI GPT)  \cite{radford2018improving}
...
%Different from the recent focus on training new PrLMs as the encoder,
...
%To determine whether a question is answerable according to the given passage,
...
%In contrast, our model benefits from joint training of the span and no-answer losses, which is simple, effective, and easy to implement. Besides, we do not use any
%Our model also excludes extra feature or input like plausible answers\footnote{We empirically find that using plausible answers overfitting issues.}, but achieves dominant performance.
%\textcolor{blue}{Our work is different from previous in that we survey the verification designs and employ two stages of verification, motivated by the human reading and comprehension pattern, which is sufficient and effective. Besides, our method is built on the strong PrLMs as the backbone, which can be used as the strong baseline and easily adapted to other MRC models.}
...
% The sketchy reader first makes a preliminary judgment, whether the question is answerable. Then, the intensive reader is applied to produce candidate answer spans, verify the answerability, and give the final answer prediction. The outputs of both of the modules are aggregated for the final decision.
...
%Then, a multi-head self-attention \cite{vaswani2017attention} is used to encode the internal dependency of the tokens.
...
%, where the embedding features of each element are transformed layer-by-layer in the fashion of aggregating features from the other elements with adaptive attention weights.

% Let $X^{l} = \{x^{l}_{1}, \dots, x^{l}_{n} \}$ be the features of the $l$-th layer, where the initial input $X^{0}=X$. The features of the $l+1$-th layer, $x^{l+1}$ is computed by
% \begin{align}
% \tilde{h}_{i}^{l+1} &= \sum_{m=1}^{M}W_{m}^{l+1}\left \{ \sum_{j=1}^{n}A_{i,j}^{m}\cdot V_{m}^{l+1}x_{j}^{l} \right \},\label{mha} \\
% h_{i}^{l+1} &= \textup{LayerNorm}(x_{i}^{l}+\tilde{h}_{i}^{l+1}), \\
% \tilde{x}_{i}^{l+1} &= W_{2}^{l+1}\cdot \textup{GELU}(W_{1}^{l+1}h_{i}^{l+1}+ b_{1}^{l+1}) + b_{2}^{l+1}, \label{ffn}\\
% x_{i}^{l+1} &= \textup{LayerNorm}(h_{i}^{l+1} + \tilde{x}_{i}^{l+1}),
% \end{align}
% where $m$ %in Eq. (\ref{mha})
% is the index of the attention heads, and $A_{i,j}^{m}\propto \mathrm{exp}[(Q_{m}^{l+1}x_{i}^{l})^{\top }(K_{m}^{l+1}x_{j}^{l})]$ denotes the attention weights between elements $i$ and $j$ in the $m$-th head, which is normalized, $\sum_{j=1}^{n}A_{i,j}^{m}=1$. $W_{m}^{l+1}, Q_{m}^{l+1}, K_{m}^{l+1}$ and $V_{m}^{l+1}$ are learnable weights for the $m$-th attention head, $W_{1}^{l+1}, W_{2}^{l+1}$ and $b_{1}^{l+1},b_{2}^{l+1}$ %in Eq. (\ref{ffn})
% are learnable weights and biases, respectively.
...
%     There are two ways to incorporate the verifier in our MRC reader: 1) merging it as a joint loss, called \textit{synchronous verification (SV)}, 2) assisting as an independent verifier, like the idea of re-reading, namely \textit{post verification (PV)}.
...
%The answer verification can be regarded as classification and regression task\footnote{The binary classification problem can be formed as the regression following \url{https://nlp.stanford.edu/seminar/details/jdevlin.pdf}.}, where
...
%     \paragraph{Post Verification}
%     PV is a parallel verifier that shares the same architecture as the reader. The only difference is the loss function where only answer verification loss is used, without the answer span prediction objective. We find this PV method can work well as an individual part, with the joint one, SV.
...
%\subsection{Reader Settings}
%The baseline reader can be any MRC model.
...
%For the SQuAD2.0 challenge, systems must not only answer questions when possible but also abstain from answering when no answer is supported by the paragraph.
...
%                 \textbf{ALBERT}: Great Lakes Megalopolis and the Northeastern megalopolis. \\
%                 \textbf{Ours:} $\langle\textup{no answer}\rangle$   \quad $\mid$ \quad
%                 \textbf{Best Span Score ($score_{has}$)}: 0.03, \textbf{Verification Score ($score_{na}$)}: 1.73; \textbf{Threshold ($\lambda$):} -0.98 \\
%                 \hline

%                 \hline
%             \end{tabular}
%         \end{center}
%         \caption{\label{com_exps} \textcolor{blue}{Answer prediction examples from the ALBERT baseline and Retro-Reader.}}
%     \end{table*}
...
\begin{table}
	...
	\setlength{\tabcolsep}{5pt}
	{
		\begin{tabular}{l c c c c }
			...
			% \midrule
			% \multicolumn{5}{c}{\emph{Top results on the leaderboard}} \\
			Human& -  & - & 86.8&    89.5 \\
			\midrule
...
% 			UPM$\dagger$& -  & -  & 87.2 & 89.9 \\
			SG-Net \cite{zhang2019sg} & -  & - & 87.2 &    90.1 \\
			% XLNet \cite{yang2019xlnet}& 87.9  & 90.6  & 87.9 &    90.6   \\
			ALBERT \cite{Lan2020ALBERT}& 87.4  & 90.2 & 88.1 &    90.9 \\
% 			ALBERT+ DA Verifier$\dagger$& -  & - & 87.8 & 91.3\\
% 			albert+verifier$\dagger$& -  & - & 88.4 & 91.0 \\
			ELECTRA \cite{clark2019electra} & 88.0 & 90.6 & 88.7 & 91.4 \\
% 			SA-Net on Electra$\dagger$ & - & - & 88.9 & 91.5 \\
% 			ELECTRA+RL+EV$\dagger$ & - & - & 89.0 &	91.8 \\
% 			albert+KD+transfer+twopass$\dagger$ & - & - & 89.1 &	91.9 \\
			\midrule
...
% 	$\dagger$ refers to the results without a published literature citation.
	TAV: threshold-based answerable verification (\S\ref{sec:ap}).
% 	Due to the SQuAD submission policy to avoid tuning against the test set, the leaderboard holder only allows us to submit our final model for test evaluation. Therefore, we did not submit the baseline models but report the Dev results and the public Dev\&Test results for reference.
	}
\end{table}
...
% It is applied to a $2\times2$ contingency table, as shown in Figure \ref{fig:mc}, which tabulates the outcomes of two models on all the evaluated examples. The null hypothesis for this test states that the marginal probability for each outcome (label one or label two) is the same for both algorithms. In other words, when applying both algorithms on the same data, we would expect them to be correct/incorrect, on the same proportion of items. \begin{figure}[htb]
% 	\centering
% 	\includegraphics[width=0.3\textwidth]{mc_sig.pdf}
% 	\caption{\label{fig:mc} Contingency table.}
% \end{figure}




% Under the null hypothesis, with a sufficiently large number of disagreements between the algorithms, the test statistic $\chi^2$ has a chi-squared distribution with one degree of freedom.
% \begin{equation}
% \chi^2 = \frac{(\left | b-c \right | -\gamma)^2}{b+c}
% \end{equation}
% where $\gamma$ is the correction factor.
...
% 			FastQA \cite{DBLP:conf/conll/WeissenbornWS17} & 43.7& 56.4 &41.9 &55.7  \\
% 			FastQAExt \cite{DBLP:conf/conll/WeissenbornWS17} &43.7& 56.1& 42.8 &56.1  \\
...
% \footnote{We only focus on single models with single-task setting for fair comparison in our paper. Though our ensemble models also achieved the state-of-the-art results, the ensemble models commonly benefit from heuristic stacking of models and some tactic augmentations, which makes it hard to distinguish the actual model contribution. }
...
%We can see that the performance of ALBERT is powerful. However, our model is more powerful, boosting the baseline essentially. It also outperforms all the latest works and achieves the first place on the leaderboard.
...
%The results except ours are from \citeauthor{tay2018densely} \shortcite{tay2018densely} and \citeauthor{back2020neurquri} \shortcite{back2020neurquri}
	...
	\begin{center}
		...
		\setlength{\tabcolsep}{5pt}
		{
			\begin{tabular}{lllllll}
				...
				\midrule
				%     XLNet  & 81.0 & 86.6 & 90.3 & 90.3 & 85.6 & 88.4 \\
				%     \quad     + E-FV &  &   &  &   &  &  \\
				%     \quad     + I-FV (Class.)  &  &  &  &   &  &  \\
				%     \quad     + I-FV (Reg.) &  &  &  &  &  &  \\
				%     \quad     + RV  &  &   &  &  &  & \\
...
%     Besides the two alternative matching layers described in \S\ref{sec:intensive}, we further add an extra self-attention layer from the PrLM encoders for comparisons with similar parameters.
