...
\begin{itemize}
	...
	% Then this sequence is projected by a linear layer to map with model dimension $d_{model}$, which is taken as the final input sequence $\mathrm{s}=[s_1,\dots,s_N]$. 
...
% But the model does not show the same capability on less similar language pairs, such as En-De.
...
% In our experiments, as $k$ increases, the translation result is sometimes prone to terminating during waiting without generating any translation words.
...
% We thank anonymous reviewers for helpful feedbacks and thank our colleagues Junnan Zhu, Yining Wang for their invaluable contributions. 
