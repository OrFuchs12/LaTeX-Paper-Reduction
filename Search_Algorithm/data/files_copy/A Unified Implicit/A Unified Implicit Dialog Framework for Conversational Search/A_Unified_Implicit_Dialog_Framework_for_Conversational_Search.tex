...
\begin{abstract}
%Enabling conversational interactions with data without explicitly encoding the rules of the interaction is referred to as \textit{implicit dialog} systems.
...
%To mitigate the problems, we propose a unified development framework for goal-oriented, information seeking conversation systems. 
%We focus on facilitating a domain-agnostic, fast prototyping of the interactive search of a domain; also enabling the developers to identify and share common building blocks across various domains.
...
%Previous sentence needs revisiting. 
...
%that are present in the commercial website 
...
% Natural Language Understanding for CS 
...
%By comparing the frames associated with consecutive utterances, the inherent operation associated with the utterances were identified. % For a test set of $251$ utterances extracted from same data-set, 
