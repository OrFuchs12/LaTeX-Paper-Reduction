...
In this section, we assume the linear Gaussian problem (LGP) of Definition \ref{def LGP}, the overparameterized regime $k=p>n$ and the min-norm model $\hat\betab$ of \eqref{eq:min_norm}. We prove Theorem \ref{thm:master_W2} that derives the asymptotic DC of $\hat\betab$ and we show how this leads to sharp formulae for the risk of the Magnitude- and Hessian-pruned models.
...
%
\vp
\textbf{(A2)[Boundedness and empirical distribution].}~~There exist constants $\Sigma_{\min},\Sigma_{\max}\in(0,\infty)$ such that:
$
...
\noindent We remark that Assumption (A2) above implies (see \cite[Lem.~4]{bayati2011dynamics} and \cite[Lem.~A3]{javanmard2013state}) that for any pseudo-Lipschitz function $\psi:\R^2\rightarrow\R$ of order $4$, i.e., $\psi\in\rm{PL}(4)$:
$$
...
In this section, we prove the following main result, which is a special case of Theorem \ref{thm:master_W2}, tailored to our specific need in this paper, that is, characterizing the risk of pruned solutions.

Let $\bth^P=\Pc(\bth)$ be a pruned version of the min-norm solution $\bth$. Recall from Section \ref{sec:risk}, that the first crucial step in characterizing the risk $\Lc(\bth^P)$ is studying the risk of a threshold-based pruned vector $\Lc(\bth^\Tc_t)$. Theorem \ref{thm:app_W2} below shows how this is possible. 
...
\Lc(\bth^g) &= \E_{\Dc}[(\x^T(\bt^\star-\bth^g) + \sigma z)^2] = \sigma^2 + (\bt^\star-\bth^g)^T\Sigmab(\bt^\star-\bth^g) \nn \\
  &= \sigma^2 + \frac{1}{p}\sum_{i=1}^p\Sigmab_{i,i}\big(\sqrt{p}\betas_i-g(\sqrt{p}\hat\betab_i)\big)^2\nn
  \\
...
Let random variables $(\Lambda,B)\sim \mu$ (where $\mu$ is defined in Assumption (A2)) and fix $\kappa>1$. Define parameter $\xi$ as the unique positive solution to the following equation
...
\cts{
\begin{theorem}[Asymptotic DC -- Overparameterized LGP]\label{thm:app_W2} %Fix $\kappa=p/n>1$.
Fix $\kappa>1$. Let Assumptions (A1) and (A2) in Section \ref{sec:ass_app} hold for some $k\geq 3$. Consider $\hat{\bt}$ as in \eqref{eq:min_norm} and $\hat\Pi_n(\y,\X,\betas,\Sigmab):=\frac{1}{p}\sum_{i=1}^{p}\delta_{\sqrt{p}\hat{\bt}_i,\sqrt{p}\betas_i,\Sigmab_{i,i}}$, the joint empirical distribution of $(\sqrt{p}\hat\betab,\sqrt{p}\betas,\Sigmab)$. Let $f:\R^3\rightarrow\R$ be any of the following two: (a) $f\in\rm{PL}(2)$, or, (b) $f=f_\Lc$ defined in \eqref{eq:fdef} \fx{where $g$ is a Lipschitz function}. In either case, it holds that
...
Before we prove the theorem, let us show how it immediately leads to a sharp prediction of the risk behavior. Indeed, a direct application of \eqref{eq:thm_app_f} for $f=f_\Lc$ to \eqref{eq:risk_app_f} shows that
\begin{align}
...
We further remark on the following two consequences of Theorem \ref{thm:app_W2}. 

First, since \eqref{eq:thm_app_f} holds for any $\rm{PL}(2)$ function, we have equivalently that $\hat\Pi_n(\y,\X,\betas,\Sigmab)$ converges in Wasserstein-2 distance to $\Pi_{\kappa,\sigma^2}\otimes\mu$, where recall that $\Pi_{\kappa,\sigma^2}$ is the distribution of the random variable $X_{\kappa,\sigma^2}$.

Second, the theorem implies that the empirical distribution of $\sqrt{p}\betab_n$ converges weakly to $\Pi_{\kappa,\sigma^2}$. To see this, apply \eqref{eq:thm_app_f} for the $\rm{PL}(2)$ function $f(x,y,z) = \psi(x)$ where  $\psi:\R\rightarrow\R$ is a bounded Lipschitz test function. 

\subsection{Proof of Theorem \ref{thm:app_W2}}
\vspace{5pt}
...
\end{lemma}
\begin{proof}
First, we show that the min-norm solution $\hat\betab=\X^T(\X\X^T)^{-1}\y$ of \eqref{eq:PO_beta} is bounded. Note that $\kappa>1$, thus $\X\X^T$ is invertible wpa 1. We
have,
\begin{align}
...
$$
where we used the facts that wpa 1: $\|z\|_2/\sqrt{n}\rP 1$, $\sigma_{\max}(\Xb)<\sqrt{2n}(\sqrt{\kappa}+1)$ and \cts{by Assumption (A2)}:
$$
\|\betas\|_2^2 = \frac{1}{p}\sum_{i=1}^{p}(\sqrt{p}\betas_i)^2 \rP \E\left[B^2\right].
$$
Put together in \eqref{eq:Ubb}, shows that
\begin{align}
\tn{\hat\betab_n} < \frac{2(\sqrt{\kappa}+1)\sqrt{\Sigma_{\max}} \sqrt{\E\left[B^2\right]} + 2\sigma}{\sqrt{\Sigma_{\min}}(\sqrt{\kappa}-1)/2} =: \tilde{B}_+.\label{eq:bd_beta}
\end{align}
Recalling that $\hat\w_n= \sqrt{\Sigmab}\betab - \sqrt{\Sigmab}\betas$, we conclude, as desired, that wpa 1,
$
\tn{\hat\w_n} \leq \sqrt{\Sigma_{\max}}\tilde{B}_+ + \sqrt{\Sigma_{\max}} \sqrt{\E[B^2]} \leq B_+.
$
...
\noindent{(iv).}~Let $f:\R^3\rightarrow\R$ be a $\rm{PL}({k})$ function. Let $\betab_n=\Sigmab^{-1/2}\w_n + \betas$. Then,
$$
\frac{1}{p}\sum_{i=1}^{p}f\left(\sqrt{p}\betab_n(\g,\h),\sqrt{p}\betas,\Sigmab\right) \rP \E_{(B,\Lambda,H)\sim\mu\otimes	\Nn(0,1)}\left[f\left(X_{\kappa,\sigma^2}(B,\Lambda,H),B,\Lambda\right) \right].
$$


\noindent{(v).}~\cts{The empirical distribution of $\betab_n$ converges weakly to the measure of $X_{\kappa,\sigma^2}$, and also,  for large enough absolute constant $C>0$:
\begin{align}\label{eq:k_AO}
\frac{1}{p}\sum_{i=1}^{p}|\sqrt{p}\betab_n(\g,\h)|^{2} < C.
...
\end{align}
It suffices to prove that the solution $\hat\betab_n$ of the PO in \eqref{eq:PO_beta} satisfies $\hat\betab_n\not\in\Sc$ wpa 1. To see that this is sufficient, note the following. On the one, setting $f=f_\Lc$, directly proves \eqref{eq:thm_app_f}. On the other hand, recall that $W_2$-convergence is equivalent to convergence of any $\rm{PL}(2)$ test function $f$. 
...
\end{align}%\tn{\h}
Recall here, that $\Xb=\X\Sigmab^{-1/2}\distas\Nn(0,1)$, $\g\sim\Nn(0,\Iden_n)$, $\h\sim\Nn(0,\Iden_p)$, $h\sim \Nn(0,1)$ and we have used the change of variables $\w:=\sqrt{\Sigmab}(\betab-\betas)$ for convenience. 
...
Let $\w_n=\w_n(\g,\h)$ be the unique minimizer of \eqref{eq:AO_con} as per Lemma \ref{lem:AO}(i), i.e., $\frac{1}{2}\tn{\Sigmab^{-1/2}\w_n+\betab}^2 = \phi(\g,\h)$. Again from Lemma \ref{lem:AO}, the minimization in \eqref{eq:AO_con} is $1/\Sigma_{\max}$-strongly convex in $\w$. Here, $\Sigma_{\max}$ is the upper bound on the eigenvalues of $\Sigmab$ as per Assumption (A2). Thus, for any $\tilde\eps>0$ and any feasible $\w$ the following holds (deterministically):
\begin{align}\label{eq:sc}
\frac{1}{2}\tn{\Sigmab^{-1/2}\w+\betab}^2 \geq \phi(\g,\h) + \frac{\tilde\epsilon^2}{2C},~\text{provided that}~ \|\w-\w_n(\g,\h)\|_2 \geq \tilde\epsilon.
\end{align}

Now, we argue that 
\begin{align}\label{eq:dev_arg}
\betab\in\Sc \text{ implies that } \|\w-\w_n(\g,\h)\|_2\geq \tilde\eps \text{ wpa 1, }
\end{align}
for 
appropriate value of $\tilde\eps$ and $\w=\sqrt{\Sigmab}(\betab-\betas)$. 
Consider any $\betab\in\Sc$. 
...
\end{align*}
In the last line above, we recalled that $\betab=\Sigmab^{-1/2}\w+\betas$ and $\Sigmab_{i,i}\geq\Sigma_{\min},~i\in[p]$ by Assumption (A2). This proves \eqref{eq:dev_arg}.
...
%\end{align}
%\cts{The terms $\frac{1}{p}\sum_{i=1}^p |\sqrt{p}\betas_{i}|^{2k-2} $ and $ \frac{1}{p}\sum_{i=1}^p |\Sigmab_{i,i}|^{2k-2}$ are bounded by Assumption (A2).}
...
\begin{align}
|F_n(\betab_n(\g,\h),\betas,\Sigmab) - F_n(\betab,\betas,\Sigmab)| &=
 \frac{1}{p}\sum_{i=1}^p|\lai|\left| (\sqrt{p}\betas_i-g(\sqrt{p}\betab_{n,i}))^2-(\sqrt{p}\betas_i-g(\sqrt{p}\betab_{i}))^2 \right|\nn\\
 &\leq
\Sigma_{\max} \frac{1}{p}\sum_{i=1}^p \left| (\sqrt{p}\betas_i-g(\sqrt{p}\betab_{n,i}))^2-(\sqrt{p}\betas_i-g(\sqrt{p}\betab_{i}))^2 \right|\nn\\
 &\leq
\Sigma_{\max} L \frac{1}{p}\sum_{i=1}^p (1+ \|\sqrt{p}[\betas_i,\betab_{n,i}]\|_2 +  \|\sqrt{p}[\betas_i,\betab_{i}]\|_2) \sqrt{p}|\betab_{n,i}-\betab_{i}|\nn\\
 &\leq
\Sigma_{\max} L  \Big(1+ \frac{1}{\sqrt{p}}\big(\sum_{i=1}^{p}\|\sqrt{p}[\betas_i,\betab_{n,i}]\|_2^2\big)^{1/2} + \frac{1}{\sqrt{p}}\big(\sum_{i=1}^p\|\sqrt{p}[\betas_i,\betab_{i}]\|_2^2\big)^{1/2}\Big) \|\betab_{n}-\betab\|_2\nn\\
 &\leq
%C  \left(1+ \|\betas\|_2^2+ \|\betab_{n}\|_2^2 + \|\betab\|_2^2\right) \|\betab_{n}-\betab\|_2.
C  \left(1+ \max\{\|\betas\|_2^2,\|\betab_{n}\|_2^2,\|\betab\|_2^2\}^{1/2} \right) \|\betab_{n}-\betab\|_2.\label{eq:fcase1}
 \end{align}
 In the second line above, we used boundedness of $\lai$ as per Assumption (A2). In the third line, we used the fact that the function $\psi(a,b) = (a-g(b))^2$ is $\rm{PL}(2)$. The fourth line follows by Cauchy-Schwartz  inequality. Finally, in the last line, we used the elementary fact that $a+b+c\leq 3\max\{a,b,c\}$ for $a=2\sum_{i=1}^p(\betas_i)^2$ and $b=\sum_{i=1}^p\betab_{n,i}^2$ and $c=\sum_{i=1}^p\betab_{i}^2$.
 
Second, consider the case $f\in\rm{PL}(2)$. Let $\ab_i=(\sqrt{p}\betab_{n,i}(\g,\h),\sqrt{p}\betas_i,\Sigmab_{i,i})$ and $\bb_i=(\sqrt{p}\betab_i,\sqrt{p}\betas_i,\Sigmab_{i,i})$, for $i\in[p]$. A similar chain of inequality holds:
\begin{align}
|F_n(\betab_n(\g,\h),\betas,\Sigmab) - F_n(\betab,\betas,\Sigmab)| &\leq \frac{L}{p}\sum_{i=1}^p (1+\max\{\|\ab_i\|_2,\|\bb_i\|_2\})\|\ab_i-\bb_i\|_2\nn \\
&= \frac{L}{p}\sum_{i=1}^p \left(1+\max\{\|\ab_i\|_2,\|\bb_i\|_2\}\right)\cdot|\sqrt{p}\betab_{n,i}(\g,\h) - \sqrt{p}\betab_i|\nn\\
&\leq L\left(1+\max\{\frac{1}{p}\sum_{i=1}^p \|\ab_i\|_2^{2},\frac{1}{p}\sum_{i=1}^p\|\bb_i\|_2^{2}\} \right)^{1/2}{\|\betab_{n}(\g,\h) - \betab\|_2}\nn\\
&\leq C\left(1+\max\{\|\betab_{n}\|_2^2,\|\betas\|_2^2,\|\betab\|_2^2,\frac{1}{p}\sum_{i=1}^p\lai^2\} \right)^{1/2}{\|\betab_{n}(\g,\h) - \betab\|_2}\nn\\
&\leq C\Sigma_{\max}^2\left(1+\max\{\|\betab_{n}\|_2^2,\|\betas\|_2^2,\|\betab\|_2^2\} \right)^{1/2}{\|\betab_{n}(\g,\h) - \betab\|_2}.\label{eq:fcase2}
\end{align}
The first inequality uses the fact that $f\in\rm{PL}(2)$. The second inequality in the third line follows by Cauchy-Schwartz. The last inequality used Assumption (A2) on boundedness of $\lai$.


It follows from either \eqref{eq:fcase1} or \eqref{eq:fcase2} that in order to prove \eqref{eq:dev2show}, we need to show boundedness of the following terms: $\|\betab_n\|_2$, $\|\betas\|_2$ and $\|\betab\|_2$. By feasibility of $\betab_n$ and $\betab$, we know that $\betab_n,\betab\in\Bc$. Thus, the desired $\|\betab\|_2<\infty$ and $\|\betab_n\|_2<\infty$ follow directly by Lemma \ref{lem:bd_PO}. (Alternatively, for $\betab_n$ we conclude the desired by directly applying Lemma \ref{lem:AO}(v)). Finally, to prove $\|\betas\|_2<\infty$, note that
$$
\|\betas\|_2^2 =\frac{1}{p}\sum_{i=1}^p(\sqrt{p}\betas_i)^2,
$$
\cts{which is bounded wpa 1 by Assumption (A2), which implies bounded second moments of $\sqrt{p}\betas$. } This completes the proof of \eqref{eq:dev2show}, as desired.
%And everything is easy now since $\|\betas\|_2^2$ is bounded as long as second moments of empirical distribution as bounded, $\|\betab_n\|_2^2$ is bounded if $\|\betas\|_2^2$ is bounded, and, $\|\betab\|_2^2$ is bounded, since $\betab\in\Bc$ by Lemma 1. To prove 
...
\begin{align}
\left(\w_n,u_n,\tau_n\right) = \max_{u\geq 0}\min_{\tau\in\Tc} ~ \frac{u\tau\tn{\gba}^2}{2} + \frac{u\sigma^2}{2\tau} + \frac{u\sigma h}{\sqrt{n}} + \min_{\w\in\Bc} \left\{ \frac{1}{2}\tn{\Sigmab^{-1/2}\w+\betas}^2 + \frac{u}{2\tau}\tn{\w}^2 - u \sqrt{\kappa}\,{\hba}^T\w \right\} .\label{eq:AO_3}
\end{align}
...
\end{align}
In Lemma \ref{lem:AO}(v) we will prove that wpa 1, in the limit of $p\rightarrow\infty$, $\|\w_n\|_2\leq C$ for sufficiently large absolute constant $C>0$. Thus, by choosing the upper bound in the definition of $\Bc$ in Lemma \ref{lem:bd_PO} strictly larger than C, guarantees that the unconstrained $\w_n$ in \eqref{eq:w_n} is feasible in \eqref{eq:AO_3}.
...
\end{align}
Here, $\Lambda,B$ are random variables as in Definition \ref{def:Xi} and \cts{we also used Assumption (A2) together with the fact that the function $(x_1,x_2)\mapsto x_1^2x_2^{-1}(x_2^{-1}+u/\tau)^{-1}$ is $\rm{PL}(2)$ assuming $x_2$ is bounded.}
...
\vspace{5pt}
\subsubsection{Proof of (iii).}  Next, we compute the saddle point $(u_*,\tau_*)$ by studying the first-order optimality conditions of the srtictly concave-convex $\Dc(u,\tau)$. Specifically, we consider unconstrained minimization over $\tau$ and we will show that the minimum is achieved in the strict interior of $\Tc$. Direct differentiation gives
\begin{subequations}
\begin{align}
...
\Longrightarrow\betab_{n,i}&=\frac{{\Sigmab_{i,i}^{-1/2}}}{1+(\ksi_n\Sigmab_{i,i})^{-1}}\sqrt{\kappa}\tau_n\hba_i +\left(1-\frac{1}{1+\ksi_n\Sigmab_{i,i}}\right)\betas_i.\label{eq:betan}
\end{align}
For $i\in[p]$, define 
\begin{align}
...
\vspace{2pt}
\noindent\underline{Proof of \eqref{eq:ivstep1}.}~~Fix some $\eps\in(0,\ksi_*/2)$. From \eqref{eq:Dc}, we know that w.p.a. 1 $|\xi_n-\xi_*|\leq \eps$ and $|u_n-u_*|\leq \eps$. Thus, $\w_n$ is close to $\vb_n$. Specifically, in this event, for every $i\in[p]$, it holds that:
\begin{align}
...
Denote, $\ab_i=(\sqrt{p}\betab_{n,i},\sqrt{p}\betas_i,\Sigmab_{i,i})$ and $\bb_i=(\sqrt{p}\vb_{n,i},\sqrt{p}\betas_i,\Sigmab_{i,i})$. Following the exact same argument as in \eqref{eq:dev2show} (just substitute $\betab\leftrightarrow\vb_n$ in the derivation), we have that for some absolute constant $C>0$ wpa 1:
\begin{align}
|F_n(\betab_n(\g,\h),\betas,\Sigmab) - F_n(\vb_n,\betas,\Sigmab)| &\leq C \|\betab_n-\vb_n\|_2.
\end{align}
%\begin{align}
...
\begin{align}
|F_n(\betab_n(\g,\h),\betas,\Sigmab) - F_n(\vb_n,\betas,\Sigmab)|
&\leq  
...
But, \cts{recall that $\|\betas\|_2^2 = \frac{1}{p}\sum_{i=1}^p(\sqrt{p}\betas_i)^2<\infty$, as $p\rightarrow \infty$ by Assumption (A2).}
...
\vspace{2pt}
\noindent\underline{Proof of \eqref{eq:ivstep2}.}~~Next, we will use (A2) to show that 
\begin{align}
...
\end{align}
\cts{We will prove that $h\in\rm{PL}(4)$. Indeed, if that were the case, then Assumption (A2) gives}
\begin{align}
...
$$
\frac{1}{p}\sum_{i=1}^p{\psi(\sqrt{p}\betab_{n,i}(\g,\h))} \rP \E\left[\psi(X_{\kappa,\sigma^2})\right].
$$
Since this holds for any bounded Lipschitz function, we have shown that the empirical convergence of $\betab_n$ converges weakly to the distribution of $X_{\kappa,\sigma^2}$. It remains to prove boundedness of the $2$nd moment as advertised in \eqref{eq:k_AO}. Recall  from
\eqref{eq:betan} that
\begin{align}
\sqrt{p}\betab_{n,i}&=\frac{{\Sigmab_{i,i}^{-1/2}}}{1+(\ksi_n\Sigmab_{i,i})^{-1}}\sqrt{\kappa}\tau_n\h_i +\left(1-\frac{1}{1+\ksi_n\Sigmab_{i,i}}\right)(\sqrt{p}\betas_i).\nn
\end{align}
Using this, boundedness of $\Sigmab_{i,i}$ from Assumption (A2), and the fact that $\tau_n\rP\tau_\star, \ksi_n\rP\ksi_\star$, there exists constant $C=C(\Sigma_{\max},\Sigma_{\min},k,\tau_\star,\ksi_\star)$ such that wpa 1,
\begin{align}
\frac{1}{p}\sum_{i=1}^p|\sqrt{p}\betab_{n,i}|^{2} \leq C\left(\frac{1}{p}\sum_{i=1}^p|\h_{i}|^{2}+\frac{1}{p}\sum_{i=1}^p|\sqrt{p}\betas_{i}|^{2}\right).\nn
\end{align}
But the two summands in the expression above are finite in the limit of $p\rightarrow\infty$. Specifically, (i) from Assumption (A2), $\frac{1}{p}\sum_{i=1}^p|\sqrt{p}\betas_{i}|^{2}\rP\E[B^{2}]<\infty$; (ii) $\frac{1}{p}\sum_{i=1}^p|\h_{i}|^{2}\rP\E[H^{2}]=1$, using the facts that $\h_i\simiid\Nn(0,1)$ and $H\sim\Nn(0,1)$. This proves \eqref{eq:k_AO}, as desired.
...
Using the above properties, we prove the following two technical lemmas used in the proof of Theorem \ref{thm:app_W2}
