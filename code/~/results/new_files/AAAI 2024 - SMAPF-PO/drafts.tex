...
\sloppy{A classical multi-agent path finding (MAPF) problem is defined by a tuple $\tuple{k, G, \{s_1,\ldots,s_k\},
\{g_1,\ldots,g_k\}}$, 
%where $i\in \{1,\ldots, k\}$ is an agent index;
where $k$ is the number of agents, 
$G=(V,E)$ is an undirected graph where $V$ represents the physical positions agents may occupy over time;
$s_i\in V$ is the initial position of agent $i$; 
and $g_i\in V$ is the target position of $i$. 
Time is discretized into time steps. At every time step, 
each agent can either stay in its current position (vertex) or move to a neighbouring one along an edge in $E$. 
A single-agent plan $\pi_i$ for agent $i$ in a MAPF problem is a path in $G$ that starts in $s_i$ and ends in $g_i$, 
representing the sequence of positions the agent occupies at every time step.  
...
The cost of a MAPF solution $\Pi$ is the sum of steps in $\Pi$'s constituent single-agent plans. 
Finding cost-optimal solutions to MAPF is NP-Hard. 
...
In this paper, we focus on finite state, action, and observation spaces. % redundant sentence
...
If there are potential conflicts that involve more than two agents, we repeat the above process but search for a policy that is safe with respect to all other agents. We continue this process until all conflicts have been resolved or we have failed to find a safe policy for one of the agents in a conflict. 
...
% \begin{figure}[ht]
%     \subfigure[]{
%         \includegraphics[width=0.47\columnwidth]{P1.png}
%         \label{subfig:P1}} 
%     \subfigure[]{
%         \includegraphics[width=0.47\columnwidth]{P2.png}
%         \label{subfig:P2}} 
%     \subfigure[]{
%         \includegraphics[width=0.36\columnwidth]{P3.png}
%         \label{subfig:P3}}
%     \subfigure[]{
%         \includegraphics[width=0.58\columnwidth]{P4.png}
%         \label{subfig:P4}}
%     \subfigure[]{
%         \includegraphics[width=0.45\columnwidth]{P5.png}
%         \label{subfig:P5}}
...
\begin{figure*}[ht]
    \subfigure[P1s]{
        \includegraphics[width=0.1\textwidth]{P1s.png}
        \label{subfig:P1s}} 
    \subfigure[P2s]{
        \includegraphics[width=0.11\textwidth]{P2s.png}
        \label{subfig:P2s}} 
    \subfigure[P3s]{
        \includegraphics[width=0.13\textwidth]{P3s.png}
        \label{subfig:P3s}}
    \subfigure[P4s]{
        \includegraphics[width=0.13\textwidth]{P4s.png}
        \label{subfig:P4s}}
...
% \begin{figure}[ht]
\subfigure[P1]{
        \includegraphics[width=0.12\textwidth]{P1.png}
        \label{subfig:P1}} 
    \subfigure[P2]{
        \includegraphics[width=0.12\textwidth]{P2.png}
        \label{subfig:P2}} 
    \subfigure[P3]{
        \includegraphics[width=0.13\textwidth]{P3.png}
        \label{subfig:P3}}
    \subfigure[P4]{
        \includegraphics[width=0.245\textwidth]{P4.png}
        \label{subfig:P4}}
    \subfigure[P5]{
        \includegraphics[width=0.115\textwidth]{P5.png}
        \label{subfig:P5}}
    % \caption{2 agents' benchmarks used in the empirical evaluation. Each color represents an agent, where the filled cells are the agents' initial states and the bordered cells are the agents' goal states. Black cells are blocked. The numbers within cells represent beacons with respective influence range, located in those cells.}
    % \label{fig:big-problems}
     \subfigure[P2-3a]{
        \includegraphics[width=0.1\textwidth]{P2-3a.png}
        \label{subfig:P2-3a}}
    \subfigure[P3-4a]{
        \includegraphics[width=0.1425\textwidth]{P3-4a.png}
        \label{subfig:P3-4a}}
    \subfigure[P2-4a]{
        \includegraphics[width=0.27\textwidth]{P2-4a.png}
        \label{subfig:P2-4a}} 
...
% 1. Offline joint POMDP
% 2. Online, solve window with joint
% 3. Online, solve window with blocking all belief
% -->?<-- 4. Online, solve window with blocking all belief, add sensing to FSVI 
% 5. The Best! 
% Online, solve window with blocking all belief, add sensing to FSVI, sense when stuck
...
To provide deeper insight into the algorithms' behaviors, we also report the number of potential collisions detected and the number of times an agent replanned, denoted as ``\#C'' and ``\#R'', respectively. Note that since \emph{Offline} generates a policy offline that avoids conflicts, it does not check for potential conflicts and never replans. So, $\#C$ and $\#R$ for Offline are always zero. % and is not reported. 
