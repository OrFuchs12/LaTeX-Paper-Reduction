...
def computeRMSE(dataset, userVectors, itemVectors, itemVectors4Types, data):
    rmse = 0
    rmseOld = 0
    oldCount = 0
    rmseNew = 0
...
def computePrecision(test4Rank, userVectors, itemVectors, itemVectors4Types, itemNum, data):
    precision5 = 0.0
    precision10 = 0.0
    recall5 = 0.0
    recall10 = 0.0
...
def initItemVectors4Types(cityUsers, userNum, itemNum, featureK):
    ...
    # check users not in any city, init seperately
#    for user in range(userNum):
 #       if not itemVectors4Users.has_key(user):
#	    print user
 #           itemVectors4Users[user] = np.random.randn(itemNum, featureK) / math.sqrt(featureK)
...
#        trainRMSE, rmseNew, rmseOld, count, newCount, oldCount = computeRMSE(trainSet, userVectors, itemVectors, itemVectors4Types, data)
        testRMSE, testRmseNew, testRmseOld, count, newCount, oldCount = computeRMSE(testSet, userVectors, itemVectors, itemVectors4Types, data)
        end = time.time()

        if epoch % 5 == 0:
            p5, r5, p10, r10 = computePrecision(testRank, userVectors, itemVectors, itemVectors4Types, itemNum, data)
            end = time.time()
