...
%We accomplish this by proposing an event detector algorithm that can identify and annotate at which time this deviation occurs. The algorithm analyzes raw bearing data in the frequency domain and compares the Kullback-Leibler (KL) divergence and standard deviation between a bearings' frequency characteristics in the normal operating state and near failure. When a certain threshold is reached, we say that failure has occurred. The algorithm quantifies said divergence, but cannot forecast the risk of failure. To this end, we subsequently train several survival models using the annotated data that allows us to forecast the probability of failure as a progressive estimate over time, contrary to traditional RUL regression, that only offer a mere point estimate of the failure time. This method also enables us to quantify survival probabilities between groups of bearings by their features (covariates). In our work, feature information is extracted from the time domain, as is common in literature \cite{randall_vibration_2021}, and event information is extracted from the frequency domain \cite{randall_rolling_2011}.
...
%As a rule of thumb, a good quality bearing with proper lubrication may obtain a lifetime given by the $L_{10}$ parameter.
% The $L_{10}$ life of a bearing is a commonly used term in the industry and is a statistical measure that predicts the number of rotations at which 10\% of a population of identical bearings will have failed due to surface fatigue, when operating under the same conditions~\cite{ISO281}. The $L_{10h}$ is derived from $L_{10}$ and describes the expected life in hours, using the formula:

% \begin{equation}
% L_{10h} = \frac{16666}{RPM} \left(\frac{C}{P}\right)^3 [h]
% \end{equation}

% where:
% \begin{itemize}
%     \item $RPM$ is revolutions per minute in [1/60 Hz]
%     \item $C$ is the dynamic load capacity of the bearing in [kN]
%     \item $P$ is the equivalent dynamic bearing load in [kN]
%     \item $p$ is the life exponent which is 3 for ball bearings (dimensionless) 
% \end{itemize}
...
%TODO: Write half page on the fundamentals of rolling bearings analysis
% Resp: Fernando/Morten
% How does a bearing operate, perhaps a photo and a timeline showing degradation
% Include 1-2 pictures of a bearing (idea)
...
%This section concerns extracting information from the physical world and domain to refined information which will feed the survival analysis. One of the first step phases of the project is to understand the domain of the project and progressively connect the needs and characteristics to logic. When confronting distinctive duo signals/domains, such as vibrations and bearings, a comprehensive grasp of mechanical principles becomes imperative to propagate quality and efficacy throughout the entirety of the data lifecycle, thereby serving as the project's fundamental cornerstone.

%The frequency-domain analysis is generally applied for the processing of bearing vibration signals. This is because the changes in the health status of the bearing appear in the frequency characteristics. For example, rolling body defects and raceway spalling cause periodic shocks, which lead to a change in the frequency. The composition and magnitude of these frequency components facilitate the analysis of machine operating conditions. Different frequency-domain indicators reflect different aspects of the bearings operating condition, so several frequency-domain features are extracted, as shown in Table 2. In this article, we not only extract the frequency-domain features but also the energy features of the first five intrinsic mode function (IMF) components based on empirical mode decomposition (EMD).

%From domain experts' knowledge and previous works \cite{ran}, it is possible to identify specific characteristics to pay attention to and in turn, transform them into features that will {\color{red}robust the models other than the Parametric group (KM) (Should be rephrased a bit)}. With this basis, several signal-based statistical features have been engineered to give possible robustness to the models that we are going to architect; below are described the definitions and formulas of the features that can support each model {\color{red} "Each model"?}.  %Given a signal frame $\bm{x} = (x)_{i=1}^N$ 
%$\sum p \cdot log(1/p)\;\;\;  p(x) =  \sum_{i=1}^n [s_i=x]$
...
%In each of them will create an event at the end of these slices and the covariates are created taking the average of the proper \emph{time-slices} that represent the status, e.g. for the first event created the \emph{time-slices} are going to be only one from the bottom of the lifetime as we are representing an imminent event, for the second event we will have the last two \emph{time-slices} of data average to represent an event with feature characteristics as less \emph{imminency} of the previous one event and so on. This technique preserves the information of the features tight and correlated with the \emph{time-to-event} information through successive steps.

%The \emph{XJTU-SY and PRONOSTIA Datasets} even though has an optimal matching with the project as "life degradation" data type, suffers from a problem of lack of samples (or \emph{subjects}) to study in this case. This brings problems in the assessment of the parameters of the models and the solution's convergence.

%The approach to cover this problem is to create a process of data-augmenting that in this case produces a \emph{synthetized} data; A multitude of the event is constantly created that has a \emph{likelihood} from the parent data point, starting from the start of the \emph{study} to the end of \emph{real failures}.

%The first task consist on use the signal of $X axis$, $Y axis$ as an independent value for another bearing: In other words, the amount of signal or bearings in the dataset of the study becomes doubled. Starting from that, with a bootstrapping method that randomly adds samples within a range around the value of the parent and calibrates the features accordingly ending with a tenfolded of the single signal. In the last passing there will be an ulterior twentyfoldering where the data points are created by dividing the entire lifetime of a single data point into \emph{time-slices}: In each of them will create an event at the end of these slices and the covariates are created taking the average of the proper \emph{time-slices} that represent the status, e.g. for the first event created the \emph{time-slices} are going to be only one from the bottom of the lifetime as we are representing an imminent event, for the second event we will have the last two \emph{time-slices} of data average to represent an event with feature characteristics as less \emph{imminency} of the previous one event and so on. This technique preserves the information of the features tight and correlated with the \emph{time-to-event} information through successive steps.
...
%CoxPH, DeepSurv, DSM and WeibullAFT generally obtain mediocre performance in both ranking and accuracy, and especially the two neural network architectures, DeepSurv and DSM, tend to perform significantly worse than their ensemble-based counterparts. We attribute this to a lack of training data and to the limited variance in the dataset; due to the lack of actual bearing data, we re-sample the original events to create more observations. This helps the models learn but, in turn, reduces the richness of information carried by each individual bearing, and consequently, some models may exhibit an excessively optimistic performance. In PRONOSTIA (see Tab. \ref{subtab:pronostia}), RSF and CoxBoost similarly perform well, and the performance, both in terms of ranking and accuracy, is generally better across all models. Note that PRONOSTIA has less than half the number of samples that XJTU has.
...
% \begin{center}
% \caption{XJTU ($N=1000$, $C=20\%$, $P=12$).}
% \label{subtab:xjtu}
% \begin{tabular}{llllll} %\toprule
%     {Model} & {T\textsubscript{train}} & {CI $\uparrow$} & {CI\textsubscript{td} $\uparrow$} & {IBS $\downarrow$} \\ \midrule
%     CoxPH & \textbf{0.056} & 0.584 & 0.591 & 0.209 \\
%     RSF & 5.754 & 0.817 & 0.810 & \textbf{0.069} \\
%     CoxBoost & 4.581 & \textbf{0.880} & \textbf{0.876} & 0.092\\
%     DeepSurv & 14.158 & 0.659 & 0.659 & 0.468\\
%     DSM & 28.323 & 0.589 & 0.600 & 0.493\\
%     WeibullAFT & 0.586 & 0.598 & 0.508 & 0.172\\ \bottomrule
% \end{tabular}
% \end{center}
% \end{subtable}%
...
% \begin{subtable}{.45\textwidth}
% \begin{center}
% \caption{PRONOSTIA ($N=400$, $C=20\%$, $P=12$).}
% \label{subtab:pronostia}
% \begin{tabular}{llllll} %\toprule
%     {Model} & {T\textsubscript{train}} & {CI $\uparrow$} & {CI\textsubscript{td} $\uparrow$} & {IBS $\downarrow$} \\ \midrule
%     CoxPH & \textbf{0.048} & 0.602 & 0.607 & 0.175 \\
%     RSF & 1.621 & 0.867 & 0.856 & \textbf{0.052} \\
%     CoxBoost & 1.686 & \textbf{0.901} & \textbf{0.883} & 0.058\\
%     DeepSurv & 2.299 & 0.634 & 0.618 & 0.456\\
%     DSM & 15.043 & 0.605 & 0.593 & 0.466\\
%     WeibullAFT & 0.646 & 0.635 & 0.582 & 0.164\\ \bottomrule
% \end{tabular}
% \end{center}
% \end{subtable}
% \caption{Performance metrics on the test sets. $N$: total sample size, $C$: pct. of censored data, $P$: number of covariates.}
% \label{tab:results}
% \end{table}
