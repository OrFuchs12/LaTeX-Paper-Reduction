...
%In our daily learning, reviewing not only deepens our memory of old knowledge, but also more importantly, inspires us to learn the new knowledge. As the fundamental design intent of neural networks is to mimic the information processing mechanisms of the human brain, we believe that the learning of networks also has an inheritance relationship in the temporal dimension. 
...
%The attention of neural network reflects the attention level on the different parts of the input\cite{zagoruyko2016paying}. Fig \ref{fig1} shows the changes of a convolutional network's spatial attention maps as training goes on. We can observe that there exists a pattern : At the beginning of training, the attention of network is disorganized. The background and the main body received equal attention. As epoch increases, the network will gradually focus on the main body of images, which is more relevant to the classification task. Motivated by this observation, we come up with two insightful questions: (1) Can network utilize the old knowledge to assist current learning like human? and (2) How to apply positive supervision to the temporal learning process of network? 
...
%Only transfer the spatial information in features is not enough to train a better student. 
...
%The mainstream research is designing new transformation and loss functions to distill knowledge in the transformation space. AT\cite{zagoruyko2016paying} used multiple layer attention maps to transfer spatial information. CRD\cite{tian2019contrastive} formulated the pair-wise distillation as contrastive learning. OFD\cite{heo2019comprehensive} designed a new distance function to distill major knowledge between teacher and student using marginal ReLU. 
%Another branch is optimizing the matching relationship between teacher and student feature candidates. ReviewKD\cite{chen2021distilling} used multi-level information of the teacher to guide one-level learning of the student network. SAD\cite{ji2021show} utilized an attention-based meta-network that learns the relative similarities to identify the possible links between teacher features and student features.
...
%Let $N_{0},N_{1},\dots,N_{t-1},N_{t}$ denote the network in different training phases. Our goal is exploiting the knowledge in previous states to assist in the training of the current network $N_{t}$. Given the same input $X$, we use $F_{i}^{l} \in \mathbb{R}^{C^{l}\times H^{l}\times W^{l}}$ to represent the activation mapping of the $l$th layer in $N_{i}$, where $C^{l},H^{l},W^{l}$ denote the channels, height and width, respectively.
...
%Under this framework, the student can acquire knowledge from both the teacher network and data simultaneously.
\begin{figure*}[htbp]
	\centering
...
%More intuitively, Markov Chain (MC) can be used to tell the difference between our method and others. The training of the existing feature-based KD methods is a one-order MC while the current training is only based on the last state. 
%In our framework, the current training will be effected by $k$ previous states and the one-order MC becomes a k-order MC. (shown in eq \ref{eq:3}).
%\begin{align}
%	\label{eq:3}
%	P(S_{t}|S_{t-1}) \Rightarrow P(S_{t}|S_{t-1},\dots,S_{t-k})
%\end{align} 
...
%The main insight of the proposed method is extracting the temporal information in the progressive training. 
...
%where $AT()$ is the transformation function that transforms the feature map to an attention map. The details of $AT()$ will be discussed in Section \ref{subsec3.4}. 
...
% Given the input sequence $(\Delta^{t-k\delta,t-(k-1)\delta}_{s_{l}},\dots,\Delta^{t-\delta,t}_{s_{l}})$, taking LSTM neuron $t$ as an example,the input gate $i_{t}$,forget gate $f_{t}$, cell $\mathcal{C}_{t}$, output gate $o_{t}$ and hidden states $\mathcal{H}_{t}$ will be calculated as:
% \begin{align}
%     i_{t} & = \sigma (W_{\Delta i} * \Delta^{t-\delta,t}_{s_{l}} + W_{h i} * \mathcal{H}_{t-1}+W_{ci}\circ \mathcal{C}_{t-1}+b_{i})\nonumber,\\
%      f_{t} & = \sigma (W_{\Delta f} * \Delta^{t-\delta,t}_{s_{l}} + W_{h f} * \mathcal{H}_{t-1}+W_{cf}\circ \mathcal{C}_{t-1}+b_{f})\nonumber,\\
%      \mathcal{C}_{t} & =f_{t} \circ \mathcal{C}_{t-1} + i_{t} \circ tanh (W_{\Delta c} * \Delta^{t-\delta,t}_{s_{l}} + W_{h c} * \mathcal{H}_{t-1}+b_{c}),\\
%      o_{t} &= \sigma (W_{xo} * \Delta^{t-\delta,t}_{s_{l}} + W_{ho} * \mathcal{H}_{t-1} + W_{co} \circ \mathcal{C}_{t} + b_{o})\nonumber,\\
%      \mathcal{H}_{t} &= o_{t} \circ tanh(\mathcal{C}_{t})\nonumber
% \end{align}

% Similar to \cite{shi2015convolutional}, we design the Conv-LSTM network in encoding-forecasting style by stacking the Conv-LSTM layers. The initial states and cell outputs of the forecaster are copied from the last state of the encoder. The final prediction is gained by concatenating all the states in the forecaster and feeding them into a 1$\times $1 convolutional layer. However, unlike general spatiotemporal sequence forecasting problems, knowledge increment sequence has a smaller scale and simpler features. Thus, our Conv-LSTM is a simplified version that has only one layer in the encoder and one layer in the forecaster. %More different structures of Conv-LSTM are explored in the ablation study.
...
\end{table*}
 ...
 %We conducted experiments on various tasks. Firstly, we compared our method with other knowledge distillation techniques in the context of classification. We varied the architecture and datasets in different settings. Additionally, we applied our method to object detection task. Our approach consistently produced significant improvements over the other model.
...
%All the experiments are implemented by Pytorch\cite{vanhoucke2011improving} using an A6000 GPU.
...
\label{tab5}
\end{table}
%\textbf{Exploration of structures of Conv-LSTM}
