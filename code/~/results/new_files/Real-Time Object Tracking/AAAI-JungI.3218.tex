...
/Author (Ilchae Jung, Kihyun You, Hyeonwoo Noh, Minsu Cho, Bohyung Han)
} %Leave this	
% /Title ()
...
%\newcolumntype{L}[1]{>{\raggedright\let\newline\\\arraybackslash\hspace{3pt}}p{#1}}
...
%By learning strong representations of target and background using convolutional neural networks, many algorithms~\cite{nam2016learningmulti,luca2016fullycovolutional,jack2017endtoend,gundogdu2018good,lijun2015visual,hong2015online,nam2016modeling,martin2016beyond,martin2017eco,fan2017sanet,zhu2017robust,han2017branchout,jung2018realtime} have achieved a significant performance gain.
...
%Based on the success, recent methods~\cite{hamed2017learning,huang2017learning,fan2017parallel,zheng2018distractor,jung2018realtime} have achieved outstanding accuracy and speed by further developing network architectures and training techniques.
...
%and it is not straightforward to extend the framework to the rest of the frames.
%However, in this approach, meta-learning is limited to initializing the deep neural network for target appearance modeling at the first frame, and it is not straightforward to extend the framework to the rest of the frames.
\iffalse
{\color{blue}
...
%meta-learned model generalized better. model realistic tracking scenarios and allows better generalization.
%Such a fine-grained simulation requires additional hyperparameters, but the model adaptation during tracking can be more efficient and more accurate.
...
%\ilchae{(Replace figure1 with this) We observe that the pruning algorithm makes 1.2$\times$ speed-up with minimizing the loss of accuraccy compared to our algorithm withouht this pruning algorithm.}
%Figure~\ref{fig:speedup} illustrates the pipeline of our tracking algorithm via meta-learning for efficient model adaptation and one-shot channel pruning.
%
...
\begin{itemize}[label=$\bullet$]
  ...
  %\item We propose a novel meta-learning strategy for real-time tracking with two-phase simulated episode for model adaptation and one-shot network pruning.
  %\item We propose meta-validation strategy based on hard instances with a triplet loss, which facilitates generalizing meta-learned algorithms better to real-world tracking scenarios.
...
%  \item The proposed algorithm demonstrates a competitive performance in multiple benchmark datasets including OTB2015~\cite{otb2015} and TempleColor~\cite{templecolor}. 
\end{itemize}
%
...
%\hyeonwoo{Real time tracking}
%Real-time object tracking~\cite{luca2016fullycovolutional,hamed2017learning,fan2017parallel,huang2017learning,jung2018realtime,zheng2018distractor} has been an active research topic in computer vision.
...
%\hyeonwoo{Offline learning in tracking?}
...
%, which does not require online model update during tracking.
...
%\hyeonwoo{Meta-learning}
%Meta-learning is a framework to learn a learning algorithm under a certain probability distribution~\cite{thrun1998learning,schmidhuber1987evolutionary,hochreiter2001learning,andrychowicz2016learning}.
...
%This approach has been explored to expedite the learning procedure in few-shot classification~\cite{li2017metaSGD,ravi2016optimization,vinyals2016matchingnet,finn2017model_agnostic}, reinforcement learning~\cite{finn2017model_agnostic,al2018continuous,ritter2018been} and imitation learning~\cite{duan2017one}.
...
% \iffalse
% \begin{algorithm}[t]
% 	\caption{Meta-Learning with Simulated Episodes}\label{pre_train_algorithm}
% 	\begin{algorithmic}[1]x
% 		\Procedure{Meta-Learning Fast Adaptation}{}\newline
% 		\textbf{Pre-requres:} The number of iterations $K_\mathrm{s}$, $K_\mathrm{w}$ \newline
% 		%Sequence batch size, $N_{bat}$ \newline
% 		\textbf{Input:} Meta-parameters $\mathcal{M}$, tracking sequences $\{\mathcal{D}_n\}$ \newline
% 		\textbf{Output:}  Learned meta-parameters $\mathcal{M}^*$  \newline
% 		\EndProcedure
% 		\vspace{-0.3cm}
% 		\While{not converged}
% 			\State{Sample batch of episodes $\mathcal{E}_i\sim p(\mathcal{E})$}
% 			\For{\textbf{all} $\mathcal{E}_i$}
% 				\State{Sample a sequence $\mathcal{D}_n$ from $\{\mathcal{D}_n\}_{n=1...N}$ }
% 				\State{Draw $\mathcal{D}_s \sim p(\mathcal{D}_s|\mathcal{D}_{n})$}
% 				\State{$\theta_\mathrm{s}^{0} = \theta_\mathrm{init} \in \mathcal{M}$}
% 				\For{\textbf{all} $k \in {1,...,K_\mathrm{s}}$} 
% 				\State{// Strong adaptation based on Eq.~\eqref{eq:strong_update}}
% 				\State{$\theta^k_s = \theta_s^{k-1} - \alpha^k_s \odot \nabla_{\theta_s^{k-1}}\mathcal{L}(\mathcal{D}_s;\theta_s^{k-1})$}
% 				\EndFor
				
% 				\State{Draw $\mathcal{D}_w \sim p(\mathcal{D}_w|\mathcal{D}_{n}, \theta_\mathrm{s}^{K_{s}})$ by tracking}
% 				\State{$\theta_\mathrm{w}^{0} = \theta_\mathrm{s}^{K_{s}}$}
				
% 				\For{\textbf{all} $k \in {1,...,K_\mathrm{w}}$} 
% 				\State{// Weak update based on Eq.~\eqref{eq:weak_update}}
% 				\State{$\theta^k_w = \theta_w^{k-1} - \alpha^k_w \odot \nabla_{\theta_w^{k-1}}\mathcal{L}(\mathcal{D}_w;\theta_w^{k-1})$}
% 				\EndFor
				
% 				\State{Draw $\mathcal{D}_v \sim p(\mathcal{D}_v|\mathcal{D}_n,\{\mathcal{D}_n\}_n)$}
				
% 			\EndFor{\textbf{end for}}
% 			\State{// Update meta-parameters based on Eq.~\eqref{eq:meta_parameters_update}}
% 			\State{$\mathcal{M} \leftarrow \text{optimize}(\mathcal{M}, \nabla_\mathcal{M}\mathcal{L}_v)$}
...
\begin{figure*}[t]
	\begin{center}
		%\includegraphics[width=\linewidth]{training_framework.png}%
		%\includegraphics[width=\linewidth]{training_framework2.png}%
...
%We first define the objective of model adaptation and discuss each component of the proposed meta-learning approach in details.


%We meta-learn hyperparameters for gradient-based fast model adaptation during object tracking.
%We propose simulated episode based on two-phase model adaptation and meta-validation with hard instance to simulate important characteristics of real tracking scenarios within simulated episode.

\subsection{Objective}

%Let $\mathcal{D}=\{(\x_i, y_i)\}_{i=1}^N$ be a dataset for target appearance learning, which is collected by tracking targets 
...
%
\begin{equation}
%\mathcal{L}(\mathcal{D};\thetageneral)= -\sum_{i=1}^N  \y_{i}^\top \log \Big[\mathbf{softmax}\big[\f_{\thetageneral}(\x_i)\big]\Big],
\mathcal{L}(\mathcal{D};\thetageneral)= -\mathbb{E}_{p_{\mathcal{D}}(\x, \y)} \Big[  \y^\top \log \big[\text{Softmax}\big(\f(\x;\thetageneral)\big)\big] \Big],
\label{eq:adaptation}
\end{equation}
%\begin{equation}
%\mathcal{L}(\mathcal{D};\thetageneral) = -\sum_{i=1}^N  y_{i}\mathrm{log}(\hat{y}_{i}^{+})+(1-y_{i})\mathrm{log}(\hat{y}_{i}^{-}),
%\end{equation}
...
% $\y \in \{[1, 0]^\top, [0, 1]^\top\}$ denotes the binary label of $\x$; it is $[1, 0]^\top$ if $\x$ is a positive sample of the target and it is $[0, 1]^\top$ if $\x$ is a negative sample of the target.
The softmax activation function $\text{Softmax}(\cdot)$ normalizes the output into a probability-like vector.

%\iffalse
%\subsection{Objective}
%%Similarly to typical tracking-by-detection~\cite{RTMDNet,MDNET}, both strong and weak model adaptations are performed by using classification loss given positive and negative samples.
%The loss function of our tracking network, $\mathcal{L}_\mathrm{cls}(\cdot ; \cdot)$, is based on the standard cross-entropy as follows:
%%
%\begin{equation}
%\label{bin_loss}
%%\mathcal{L}_\mathrm{cls}(\mathcal{D};\theta) = -\mathbb{E}_\mathcal{D}\big[ y_{i}\mathrm{log}(\hat{y}_{i}^{+})+(1-y_{i})\mathrm{log}(\hat{y}_{i}^{-}) \big],
%\mathcal{L}(\mathcal{D};\thetageneral) = -\sum_{i=1}^N  y_{i}\mathrm{log}(\hat{y}_{i}^{+})+(1-y_{i})\mathrm{log}(\hat{y}_{i}^{-}),
%\label{eq:adaptation}
%\end{equation}
%where $\mathcal{D}=\{(\x_i, y_i)\}_{i=1}^N$ is a dataset
%%
%where $\mathcal{D}=\{(\x_i,y_i)\}_{i=1}^{N}$ is a set of candidate bounding boxes $\x_i$ and their labels $y_i \in \{0,1\}$, and $\hat{y}_i^+$ and $\hat{y}_i^-$ denote the positive and negative output probability of the model parameterized by $\theta$, respectively.
%The $I_i$ and $b_i$ denotes an image and a bounding box coordinates.
%%$I_i$ denotes an image and $R_i$ denotes a bounding box position.
%%$y_i$ is a label denoting positive ($1$) and negative ($0$) samples, and $N_\mathrm{m}$ is the number of samples in a dataset.
%Note that the label of each bounding box sampled from a frame is determined by the IoU measure with respect to the ground-truth or the estimated target.
%%Samples whose IoU with the ground-truth or estimated target is greater than the positive threshold is considered as positive samples, and Samples whose IoU with target is smaller than the negative threshold is considered as negative samples.
%\fi


%\subsection{Tracking Episode Simulation}
\subsection{Tracking Simulation}

%\iffalse
%We define \textit{a tracking episode} as visual tracking for a single video.
%%A tracking episode starts with an initial model and a tracking algorithm alternates model adaptation and target estimation in the subsequent frames until every frame of the video has an estimated target position.
%A tracking episode starts with an initial model and a \textit{tracking algorithm} alternates model adaptation and target estimation in the subsequent frames.
%The tracking episode ends if every frame of the video has an estimated target position.
%Because the execution of a tracking episode is computationally expensive, we develop our meta-learning approach based on \textit{simulate tracking episodes}.
%We define \textit{a simulated tracking episode} as a sequence of model adaptation, target estimation and model validation for a single target on artificially generated data.
%%Although a simulated tracking episode does not perform visual tracking on real video, we design it to simulate realistic model adaptation similar to a tracking episode.
%\fi
%
%\iffalse
%Since the hyperparameters are meta-learned based on the simulated tracking episodes, creating realistic episodes at training time is crucial for the generalization capability of the learning algorithms for real tracking scenarios.
%The typical visual tracking algorithms consider two different types of model adaptation. 
%One is performed at the $1^\mathrm{st}$ frame based on the target ground-truth and while the other is based on the estimated targets obtained from online tracking process in the rest of the frames. 
%We call the former as {\em initial adaptation} and the latter as {\em online adaptation}.
%The major difference between the initial and online adaptations is the reliability of labels used for learning. 
%We consider this aspect in the episode simulation and use separate data for each adaptation during meta-learning.
%Note that the separation of the initial and online adaptations in the simulated episode is one of important differences from MetaTracker~\cite{park2018metatracker}, which only has the adaptation based on the ground-truth during meta-learning.
%\fi
...
%online tracking process in the rest of the frames. 
...
%{\color{blue} Note that the separation of the initial and online adaptations in the simulated tracking is one of the important differences from MetaTracker~\cite{park2018metatracker}, which only has the adaptation based on the ground-truth during meta-learning.}

%Let $\Gamma=\{\mathcal{V}_i\}_{i=1}^M$ be a training video set, where $\mathcal{V}_i$ is an annotated video with tracking ground-truths for a single moving target.
...
%Similarly, we name a standard object tracking for a single video as \textit{a tracking episode}
%We use the term \textit{a simulated tracking} and \textit{a simulation} interchangeably.
...
%use used to construct the online dataset $\dataonline$
%Starting from an initial parameter $\thetainit$, this step results in an adapted parameter $\thetainitadapt$.
%The online adaptation uses $\dataonline$ to adapt the model parameters based on Eq.~\eqref{eq:adaptation}.
%for optimization of model parameters based on Eq.~\eqref{eq:adaptation}. 
%The online adaptation uses Eq.~\eqref{eq:adaptation} and $\dataonline$ to optimize model parameters.
%This step simulates model adaptations based on noisy datasets 
%The online adaptation uses $\dataonline$ to simulate model adaptations based on noisy datasets.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		%\Procedure{Meta-Learning Fast Adaptation}{}\newline
		\State
		%\textbf{Pre-requres:} The number of iterations $K_1$, $K_2$ \newline
		%Sequence batch size, $N_{bat}$ \newline
		%\textbf{Input:} A Training video set $\Gamma=\{\mathcal{V}_i\}_{i=1}^M$,
		\textbf{Input:} A Training video set $\Gamma$,
		\newline
		Meta-parameters $\mathcal{M}=\{\thetainit, \Ainit, \Aonline\}$,
		\newline
		%Meta-parameters $\mathcal{M}=\{\thetainit, \{\alphainit^k\}_{k=1}^{K_1}, \{\alphaonline\}_{k=1}^{K_2}\}\}$, \newline
		\textbf{Output:}  Learned meta-parameters $\mathcal{M}^*$  \newline
		%\EndProcedure
		%\vspace{-0.3cm}
		\While{not converged}
			%\State{Sample a mini-batch of annotated videos $\mathcal{V}$ from $ \Gamma$}
			%\State{Sample a mini-batch of annotated videos $\mathcal{V}_i$ from $ \Gamma$}
			\State{Sample a mini-batch of annotated videos from $\Gamma$}
			%\For{\textbf{all} $\mathcal{V}_i$}
			\For{\textbf{all} video $\mathcal{V} $ in a mini-batch}
				...
				\EndFor{\textbf{end for}}
				%\State{Set $\thetainitadapt=\thetainitadapt^{K_1}$ ~// initial adapted model}
				%\newline
				...
				%\State{Set $\thetaonlineadapt = \thetaonlineadapt^{K_2}$ ~// online adapted model}
				%\newline
				%\State{Collect $\dataval^\text{std}$ based on $\mathcal{V}_i$}
				%\State{Collect $\dataval^\text{hard}$ based on $\Gamma - \{\mathcal{V}_i\}$}
				%\State{$\dataval = \dataval ^\text{std} \cup  \dataval^\text{hard}$}
			...
			\EndFor{\textbf{end for}}
			%\State{Compute $\mathcal{L}_\mathrm{v}$ over a mini-batch of $\mathcal{E}_i$~// Eq.~\eqref{eq:meta_validation}}
...
%\subsubsection{Simulated Adaptations}
...
%Simulated strong adaptation during meta-learning is identical to the strong adaptation during real tracking.
...
%After the initial adaptation, the model parameter $\thetainitadapt$ is set to $\thetainitadapt^{K_1}$.
The model parameter after the initial adaptation is given by $\thetainitadapt^{\Kinit}$.


%\vspace{-0.3cm}
\paragraph{Online Adaptation}
%Contrary to the strong update, weak update in real tracking is conducted with estimated target, which might contains some noisy or mislocalized samples.
%Using ground-truth target appearance to simulate weak update is problematic because it might lead to over-confident learning algorithm that cannot distinguish unreliable training samples, which might cause drastic failure in real tracking and results in problems such as draft.
The online adaptation uses the online dataset $\dataonline$, which includes the examples sampled from the estimated targets.
%The online adaptation is performed based on the dataset $\dataonline$, which includes the examples sampled from the estimated targets and the background in the past few frames.
%The simulated tracking episode approximates the procedure to estimate the target starting from $\thetainitadapt$ using the examples in $\dataonline$.
%{\color{blue}
%The dataset $\dataonline$ is constructed based on $4$ frames that are uniformly sampled from $\mathcal{V}$.
...
%Therefore, we use targets estimates based on a strong updated model $\theta^*_1$ to sample weak dataset $\mathcal{D}_w$.
...
%The online adaptation starts optimizes $\thetainitadapt^{K_1}$ and obtains the new model parameter $\thetaonlineadapt^{K_2}$, which is formally given by
%
\begin{equation}
...
%We define a validattion dataset $\dataval=\dataval^\text{std}\cup\dataval^\text{hard}$.
...
% that might appear in standard object tracking.
...
%Specifically, we sample $64$ annotated videos from $\Gamma-\{\mathcal{V}\}$ and sample $2$ image patches from each video, where the image patches are tightly overlapped with a ground-truth annotation sampled from a video.
...
%To make meta-validation set $\mathcal{D}_v$ appropriate for robust model parameter evaluation, we consider introducing hard instances in $\mathcal{D}_v$
%\hyeonwoo{Refine below to explain hard instances for meta-validation}
...
We label all examples in $\dataval^\text{hard}$ as negative.
%\ilchae{Triplet: We apply triplet loss between $X_\mathrm{v}^\mathrm{pos}$ and $X_\mathrm{v}^\mathrm{neg+}$.}
...
% the cross-entropy loss $\mathcal{L}$ and the triplet loss $\losstriple$.
%which is designed in consideration of hard instances.
%We prefer a model parameter with smaller validation loss.
%Given a validation dataset $\dataval$ and a model parameter $\thetageneral$, the triplet loss is given by
...
%\text{where}~~g(\x;\thetageneral)=\frac{\f(\x;\thetageneral)}{\|\f(\x;\thetageneral)\|^2}
\end{align}
%
...
%Introducing triple loss for hard instances is important not to suppress positive probability in output of a model from positive samples
%Introducing triple loss for hard instances is to prevent the model from suppressing positive probability for these samples.
%\textcolor{blue}{(Delete? : Because following sentences have rich information..?)Introducing triplet loss for hard instances is to allow the model to capture objectness~\cite{alexe2012measuring}, which is a property that distinguishes general objects from backgrounds.}
...
% the cross-entropy loss enforces the model to classify them as negatives, like the instances from backgrounds.
%This objective may lead to suppressing discriminativeness between a target and backgrounds.
%?suppressing objectness and prevent distinguishing a target from backgrounds.
%On the contrary, the triplet loss does not suppress hard instances to negatives, while still encouraging difference with the positive samples.
...
%\hyeonwoo{(TODO) formal definition of episode trajectory $\mathcal{E}$}
%\hyeonwoo{$\mathcal{E}\sim p(\mathcal{E}|\mathcal{M}, \Gamma)$}
...
%Because $\thetainitadapt^{\Kinit}$ and $\thetaonlineadapt^{\Konline}$ are further represented by the meta-parameters based on Eq.~\eqref{eq:strong_adaptation} and \eqref{eq:weak_adaptation}, this loss function is fully differentiable with respect to the meta-parameters as well.
%
%$\mathbb{E}_\mathcal{E}[\cdot]$. 
%defined on a distribution of episode trajectories given meta-parameters and a video dataset $\mathbb{E}_{p(\mathcal{E}|\mathcal{M}, \Gamma)}[\cdot]$
%where $\dataval$ is a meta-validation dataset collected in the episode $\mathcal{E}$ and meta-parameters are evaluated by expectation over multiple episodes $\mathbb{E}_\mathcal{E}[\cdot]$.
%
%We learn meta-parameters $\mathcal{M}$ to minimize $\lossmeta$.
%Because $\lossmeta$ is fully differentiable with respect to meta-parameters, we can optimize the loss function iteratively with respect to $\mathcal{M}$.
%Because $\mathcal{L}_\mathrm{v}$ is fully differentiable function of meta-parameters based on Eq.~\eqref{eq:strong_adaptation} and \eqref{eq:weak_adaptation}, we could directly optimize the loss function with respect to meta-parameters based on gradient descent.
...
\end{split}
\end{equation}
%By employing an optimizer such as ADAM~\cite{diederik2014adam}, we update the meta-parameters as follows:
%\begin{eqnarray}
\iffalse
\begin{equation}
...
%\textcolor{blue}{Similarly computed from Eq.~\eqref{eq:meta_parameters_update}, gradients from $\Ainit$ and $\Aonline$ are also used to optimize $\mathcal{M}$.}
%-onshot_pruning
...
%The learing procedure requires the observation of full episode, 
...
%\subsubsection{Objective of our LASSO pruning}
...
%\begin{equation}
%\begin{split}
%\textstyle \mathcal{L}_\text{lasso}&(\mathcal{D}, \mathcal{B} ; \boldsymbol{\theta})
%	= \textstyle  \lambda \sum_{l=1}^L\big\| \boldsymbol{\beta}^l\big\|_1
%	\\
%	+ & \textstyle \frac{1}{N}\sum_{i=1}^N
%		\sum_{l\in\mathcal{S}} 
%			\big\|F^l(\x_i ; \boldsymbol{\theta}) -  F^l_\mathcal{B}(\x_i; \boldsymbol{\theta})\big\|_2,
%\label{eq:lasso_prune}
%\end{split}
%\end{equation}
%
...
%The $\mathcal{D}=\{(\x_i,y_i)\}_{i=1,...,N}$ is a set of inputs and outputs to the network, where $\x_i$ is a bounding box in this paper. 
%The $\theta$ is parameter of a convolutional network, 
...
\end{equation}
%We use notations for features as follows.


%The masked features are denoted as $F^{l+1}_\theta(\x, \mathcal{B}) = \beta^{l+1} \odot \sigma\big(F^l_\theta(\x, \mathcal{B}) * W^l\big)$, where $F^0_\theta(\x, \mathcal{B})=\x$.

%Notation for features in a network is defined as follows.
%Given an input bounding box $F^0_\theta(\x) = \x$, the unmasked network sequentially compute features in each layer by $F^{l+1}_\theta(\x)=\sigma\big(F^l_\theta(\x) * W^l\big)$, where $W^l$ is the convolution parameters for $l^{th}$ layer and $\sigma(\cdot)$ is a non-linearity function. We discard components such as pooling layers for notational simplicity. 
%The masked features are denoted as $F^{l+1}_\theta(\x, \mathcal{B}) = \beta^{l+1} \odot \sigma\big(F^l_\theta(\x, \mathcal{B}) * W^l\big)$, where $F^0_\theta(\x, \mathcal{B})=\x$.

%\subsubsection{Objective for a simulated tracking episode}
...
%We minimize the following objective for an episode trajectory $\mathcal{E}$. 
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
%where $\mathcal{E}$ is an episode trajectory defined by a tuple containing all datasets and parameters during an object tracking episode.
...
\begin{table*}[t!]
	\begin{center}
		\caption{
			...
			%The proposed components for meta-learning are all necessary to observe significant speed up compared to RT-MDNet without sacrificing performance.
		}
		\label{tab:ablation}
		%\setlength{\tabcolsep}{5pt}
		%\scalebox{0.85}{
		\resizebox{2.1\columnwidth}{!}{
			%@{}L{2.2cm}@{}|@{}C{1.0cm}@{}|@{}C{1.0cm}@{}|@{}C{1.8cm}@{}@{}C{1.8cm}@{}@{}C{1.8cm}@{}
			\begin{tabular}{
					l|cccccc|ccc
					%@{}L{8cm}@{}|@{}C{1.0cm}@{}@{}C{1.0cm}@{}@{}C{1.0cm}@{}@{}C{1.0cm}@{}@{}C{1.0cm}@{}@{}C{1.6cm}@{}|@{}C{2.0cm}@{}@{}C{2.0cm}@{}@{}C{2.0cm}@{}
			}
				%\hline%\noalign{\smallskip}
	...
	\begin{center}
		...
		\scalebox{0.85}{
			...
			\begin{tabular}{
				...
				MetaRTT+Prune  & $\surd$& $\surd$ & $\surd$  & $\surd$ & $\surd$ & $\surd$  & 65.1 & 87.4 & 65 \\
				\hline
				%MetaTracker~\cite{park2018metatracker} &{}&  {} & $\surd$  & $\surd$    & {}   &{}     &   56.8 & 81.2 & 40 \\
...
%\subsubsection{Pruning network for mask prediction}
...
%\subsubsection{Meta-learning objective}
...
%For efficient feature extraction from multiple bounding boxes in an image (e.g. $\{\x_i=(I, b_i)\}_{i=1}^N$), RT-MDNet employs a two-stage feature extraction pipeline similar to \cite{girshick2015fastrcnn,he2017maskrcnn}.
%In the first stage, a convolutional neural network (CNN) extracts a feature map from an input image $I$.
%Because this feature map is shared across multiple bounding boxes, redundant computation is reduced in this stage.
%To extract the bounding box specific features from a common feature map, RT-MDNet uses the RoIAlign layer~\cite{girshick2015fastrcnn,he2017maskrcnn}.
%This layer pools $1 \times 1$ dimensional feature maps from the shared feature map based on the bounding box positions (e.g. $\{b_i\}_{i=1}^{N}$).
%This $1 \times 1$ feature for each bounding box is used to compute outputs corresponding to $\hat{y}^{+}$, $\hat{y}^{-}$, and $g(\x)$ based on two fully connected layers.
...
%In equations for one-shot channel pruning, output activations of fully connected layers are considered as feature maps with $1 \times 1$ spatial dimension.
\fi
...
% Standard approach for finding such $\mathcal{B}=\mathcal{B}^\text{conv}\cup \mathcal{B}^\text{fc}$ is learning it with iterative optimization of an objective function.
% For example, an approach based on feature reconstruction objective for learning $\mathcal{B}^\text{conv}$~\cite{lassoprune} minimizes the following objective function with respect to $\beta^l$ for each layer $l$.
% \begin{equation}
% %\textstyle
% \hat{\beta}^l=\argmin_{\beta^l}\Bigg\lVert F^{l} -  \beta^l F^{l-1}*W^{l} \Bigg\rVert^{2}_{F}
% + \lambda_{\beta^l} \lVert \beta^l \rVert_{1},
% \label{eq:prune_objective}
% \end{equation}
% where $W^l_c\in\mathbb{R}^{C^l_\text{in}\times k^l_h \times k^l_w}$ is a kernel weight of output channel $c$, $*$ is a convolution operation and $\lambda_{\beta^l}$ is a penalty coefficient controlling the sparsity of $\beta^l$. 
% %We can start optimizing the objective with $\lambda_{\beta^l}=0$, and gradually increase it until a desired sparsity condition $\lVert\beta^l\rVert_{0} \leq C_S$ is satisfied.
% %$C_S$ is desired number of non-zero entry after the pruning.
% %However, learning $\mathcal{B}$ with objective in Equation~\ref{eq:prune_objective} requires iterative optimization, which is too costly to use within an online tracking pipeline.
% However optimization of Equation~\ref{eq:prune_objective} is too costly to use within an online tracking pipeline.

% Our approach is learning an inference network performing the optimization of Equation~\ref{eq:prune_objective} as a single step inference.
% We consider a scenario where $W^l_c$ and $\lambda^{\beta^l}$ is fixed for learning $\mathcal{B}^\text{conv}$ for a single network.
% In this scenario, the solution of the learning problem is highly dependent on the feature map $F^l$ because the all other variables are known or can be inferred. Note that $F^{l-1}$ can be inferred from the relation $F^l_c = F^{l-1}*W^l_c$.
% Therefore, we construct the pruning inference network as 
% \begin{equation}
% \textstyle \hat{\beta}^{l} \approx \phi_{\beta}^{l}(F^{l};\theta_{\beta}^{l}),
% \end{equation}
% where $\phi_{\beta}^{l}(\cdot)$ and $\theta_{\beta}^l$ are the function and parameters for the pruning network. 
% In this work, we parameterize $\phi_{\beta}^{l}(\cdot)$ as a composition of the global average pooling and the multi-layer perceptron.
% Learning the pruning inference network $\phi_{\beta}^{l}(\cdot)$ and the corresponding parameters $\theta_{\beta}^l$ is described in the following section.

% Given the pruning inference network $\phi_{\beta}^{l}(\cdot)$, we compute pruning masks $\hat{\beta}^l$ for shared convolution layers based on the featured computed with the first frame image $I_t$ before the strong update.
% After getting $\hat{\beta}^l$, we make hard channel selection mask having only $\{0,1\}$ values and we use it to prune feature maps $F^l$ and the corresponding convolution kernels $W^l$.
% The pruning reduces the computation for feature extraction by allowing not computing pruned parts of the feature map at all.
% Unlike shared convolution layers, layers after RoIAlign layer is pruned after strong update so that we can prune network based on the updated parameters.
% Note that strong update is applied to parameters after RoIAlign layer only.~\footnote{\hyeonwoo{This should be also mentioned earlier than this part}}
% We use a first frame image $I$ and the ground-truth target position $R^*$ to prune the network.
...
% To train $\phi^l_\beta$, we get feature map ${F^l}_I$ and soft channel selection mask ${\hat{\beta}^{l}}$ from ${I_{s}}$. Note that channel selection function is obtained with original feature map.
% Then, we obtain soft masked feature map $ F^{l\prime}$ on $I_{v}$ as $F^{1\prime}_I(I_s;\theta_\beta) = \phi^l_\beta(I_s;\beta) \odot F^1_I$ and $F^{l\prime}_I(I_s;\theta_\beta) = \phi^l_\beta(I_s;\beta) \odot \big(F^{l-1\prime}_I(I_s;\theta_\beta) * W^{l}_\text{init}\big)$. $W^{l}_\text{init}$ is a convolution kernel whose parameters are from $\theta_\text{init}$. We modify reconstruction loss~\cite{lassoprune} for pruning channels of convolution layer as
% \begin{equation}\label{conv_prune_loss}
% \begin{aligned} 
% \mathcal{L}_F(I_{s};\theta_{\beta}) =&  \mathbb{E}_{I\sim \mathcal{D}_s \cup \mathcal{D}_v}\Big[
%     \big\lVert  F^{L}_I - F^{L\prime}_I(I_s;\theta_\beta) \big\rVert^{2}_{F}\Big] \\
% &\textstyle+\lambda_{\beta} \sum_{l=1}^{L}\big\lVert \phi^l_\beta(I_s;\theta_\beta) \big\rVert_{1}.
% \end{aligned}
% \end{equation}
% Reconstruction loss is used only to the top shared feature map $F^L$ rather than to all feature map, because reconsturcting high semantic information, which is used in detector, is much important than remaning whole feature. 
...
% To get $\beta^m$, we use RoI aligned feature of soft masked feature map $F^{L\prime}_{I_s}(I_s;\theta_\beta)$. The feature of whole samples $X^\prime = \{\text{RoI}(F^{L\prime}_{I_s}(I_s;\theta_\beta), R_i)|R_i \in \mathcal{D}_s\}$. 
% With $X^\prime$ and strong updated parameter $\theta_\text{s}$, we can get feature map of linear layers $G^m$. Then we obtain soft unit selection mask ${\hat{\beta}^{m}}$ by using $G^m$ and $\phi^m_{\beta}$. Note that ${\hat{\beta}^{m}}$ is also from $I_s$. 

% After we have  ${\hat{\beta}^{m}}$, we computed original feature map $G^{m}$ on $I_v$. We get pruned feature map $G^{m\prime}$ like $G^{L+1\prime} = \hat{\beta}^{L+1} X^\prime*W_{s}^{L+1}$ and $G^{m\prime} = \hat{\beta}^{m} G^{m-1} * W^{m}_{s}$.
% We also use reconstruction loss on $G^{m}$ without $G^M$, which is binary classification score $\hat{Y}$, which is decribe as
% \begin{equation}\label{fc_prune_loss}
% \begin{aligned} 
% &\mathcal{L}^m_{G}(I_s;\theta_{\beta})= \\
% &\textstyle  \mathbb{E}_{(I,R)\sim \mathcal{D}_s \cup \mathcal{D}_v}  \lVert  G^{m}_{I,R} - G^{m\prime}_{I,R}(I_s,\mathcal{R}_s;\theta_\beta) \rVert^{2}_{F} \\
% &+\lambda_{\beta} \lVert \phi^m_\beta(I_s,\mathcal{R}_s;\theta_\beta) \rVert_{1}.
% \end{aligned}
% \end{equation}
...
%We present the results from two versions of our trackers; one is the model without one-shot network pruning, MetaRTT, and the other is the version with pruning, MetaRTT+Prune. 
...
%with modifications for fast adaptation and network pruning.
%We perform one-shot network pruning and initial model adaptation at the first frame while online adaptation is applied during object tracking.
%Note that the learning rates for the initial and online adaptations are given by $\Ainit$ and $\Aonline$, respectively, and that the number of iterations, $\Kinit$ and $\Konline$ are set to 5 unless otherwise mentioned.


\subsection{Application to RT-MDNet}

%(camera-ready)We apply our meta-learning algorithm to RT-MDNet~\cite{jung2018realtime}, which is one of the state-of-the-art real-time object tracker.
...
%Because this feature map is shared across multiple image patches, this stage reduces redundant feature computations.
...
%This layer pools a $3 \times 3$ dimensional feature map grid from the shared feature map based on the image patch positions.
...
%during the initial or online adaptation.
%Formally, $\thetainit=\thetainitadapt^{k_\text{int}}=\thetaonlineadapt^{k_\text{onl}}$ for all $k_\text{int}=1,\dots,\Kinit$ and $k_\text{onl}=1,\dots,\Konline$ for the shared CNN, while the model adaption for the fully connected layers is performed by Eq.~\eqref{eq:strong_adaptation} and~\eqref{eq:weak_adaptation} as described in the main paper.
...
%Therefore, we assume that the dimension of the parameters corresponding 
%to the shared CNN are same for $\thetainit$, $\thetainitadapt$, and $\thetaonlineadapt$.
...
%For one-shot network pruning, the channel pruning is performed to all the layer except for the final convolution layer and all fully connected layers after the RoIAlign layer.
%not applied to, denoted by $\mathcal{S}$, includes the final convolution layer and all fully connected layers after the RoIAlign layer.
%In equations for one-shot channel pruning, output activations of fully connected layers are considered as feature maps with $1 \times 1$ spatial dimension.
...
%For two-phase simulated episodes, we draw 500 positive and 5000 negative samples for the initial adaptation $\datainit$, and draw 200 positive and 800 negative sample for each frame of the online adaptation $\dataonline$.
...
%More details about our implementation are described in supplementary material.
...
%We select our baseline algorithm as RT-MDNet~\cite{jung2018realtime} which has state-of-the-art accuracy with real-time speed. 
%Like RT-MDNet, our algorithm initialize three convolutional layers transferred from the corresponding parts of VGG-M network ~\cite{ken2014return}. 
%And then, similar to ~\cite{nam2016learningmulti}, we initialize three fully connected layers from random initialization without domain specific branch generation. 
\iffalse
...
\fi
%And then, $\gamma$ and $\zeta$ are set to 0.1 and 0.7, respectively. 
%IoU Criterion for strong update and weak update are same as RT-MDNET's criterion.


\iffalse
\subsection{Evaluation methodology}
We perform experiments
%We evaluate our tracker on two standard datasets including~\cite{wu2015object} OTB2015 and TempleColor~\cite{liang2015encoding}.
...
%\subsection{Analysis of the meta-learning algorithm}
\subsection{Ablation studies}
%\subsection{Analysis: meta-learning for fast adaptation}
...
%We analyze the proposed meta-learning algorithm for fast adaptation on the OTB2015~\cite{wu2013online}.
%Table~\ref{tab:ablation} illustrates the results.
%\textbf{Online adaptation with estimated targets:}
%\textbf{Analyzing meta-learning algorithm:}
...
%(camera-ready)Our full algorithm maintains competitive accuracy and achieves significant speed-up compared to the baseline technique, RT-MDNet~\cite{jung2018realtime}.
...
%\textbf{Meta-validation with hard instances:}
...
%We compare MetaRTT with RT-MDNet to analyze the impact of the proposed meta-learning.
%Because MetaRTT is a meta-learned version of RT-MDNet, most implementation details are the same, except that RT-MDNet does not use meta-learning.
...
%Table~\ref{tab:fast_adapt} analyzes the source of the acceleration.
...
\begin{table}[t!]
	\begin{center}
		\caption{
			...
			%The effect of meta-learning for fast adaptation evaluated on OTB2015. Proposed meta-learning facilitates using very small number of gradient steps during model adaptation.
		}
...
\begin{table}[t!]
	\begin{center}
		\caption{
			...
			%Effect of one-shot network pruning on OTB2015.
			%The proposed meta learning based one-shot network pruning bring significant speed up without sacrificing performance much. Using proposed meta-learning objective is essential.
		}
...
%Contrary to our meta-learning algorithm that optimizes the pruning network for complete simulated episodes, this variant optimizes the pruning network only for the first frame of a simulated tracking episode.
%The other option is the fixed channel pruning, which learns a universal channel pruning mask $\mathcal{B}$ offline for every video.
...
%%\vspace{-0.4cm}
%%\paragraph{The effects of different meta-learning components}
%We compare several variants of \textrm{MetaRTT} to confirm the effects of each training part of our algorithm.
%Table~\ref{tab:ablation} shows the results of each models on OTB2015.
%\textrm{No-Neg} means the model meta-learned without hard negative instances for meta-validation.
%By employing hard negative instances, accuracy is significantly improved.
%It proves that meta-validation based on hard negative instances facilitate generalization to real tracking scenario.  
%Not using estimated target during weak adaptation (Using GT) or independent learning rate for each iteration(\textrm{Common lr}), drops performance significantly compared to \textrm{MetaRTT}.
%It verifies that the proposed simulated tracking episode works well and imply that all proposed components are very important and effective for meta-learning fast and accurate tracker.  
%Surprisingly, \textrm{MetaRTT} has accuracy gain over baseline RT-MDNet while it accelerate the speed of RT-MDNet with 1.4 times.
%Moreover, by applying one-shot network pruning, \textrm{MetaRTT+Prune} has 8$\%$ of speed gain over \textrm{MetaRTT} while maintaining almost full performance of \textrm{MetaRTT}.
%We observe that reducing the number of channels from convolutional layers or hidden units from fully connected layer does not lose much accuracy while shorten the execution time.
%
%For comparison with other meta-learning method for tracking, we adopt the approach proposed by MetaTracker~\cite{park2018metatracker} to our RT-MDNet based model. Unlike proposed meta-learning framework, MetaTracker employ meta-learning only for strong adaptation without using hard negative instance for meta-validation. 
%In our experiment, we observe relatively poor performance with the basic meta-learning approach.
%Interestingly, including hard negative instance-based meta-validataion for this model improves the performance significantly.
%
%%\vspace{-0.4cm}
%%\paragraph{The effects of meta-learning for fast adaptation}
%Table~\ref{tab:fast_adapt} illustrate the effects of meta-learning for fast adaptation.
%The numbers in right-side of tracker's name means the number of gradient descent iterations for initial adaptation $K_1$ and for online adaptation $K_2$. 
%RT-MDNet$\_$50$\_$15 is correspond to original RT-MDNet.
%In this result, our tracker \textrm{MetaRTT} overwhelms all variants of RT-MDNet in terms of the nubmer of online iteration step.
%It shows that the proposed meta-learning is effective to maintain high accuracy even with very small number of iterations during both initial adaptation and online adaptation.
%%Moreover, in terms of execution time, the results show that weak update is more important than strong update to accelerate execution time.
%%This result demonstrateproves that we find robust offline training method for both fast and stable adaptation of both strong- and weak-phase.
...
% \iffalse
% \textcolor{blue}{Additionally, table~\ref{tab:ablation_neg} illustrates the impact of augmentation of negative data from an angle of feature embedding space.
% We compute variance of $\theta_{init}$'s positive score between same target in different frames(\textrm{Intra-Var}) and between different targets from different videos(\textrm{Inter-Var}) for every 30000 iteration of pre-training.
% From the observation,  Without data augmentation(\textrm{MetaRTT w/o neg}$^{+}$), targets are indistinguishable from each other while \textrm{MetaRTT} shows small \textrm{Intra-Var} and big \textrm{Inter-Var}.
% }
% \fi

%\vspace{-0.4cm}
%\paragraph{The effects of one-shot network pruning}
%We evaluate the proposed one-shot network pruning based on meta-learning and present results in Table~\ref{tab:ablation_prune}. Fixed denotes off-line network pruning approach that is not adaptive to input tracking sequence.
%One-shot baseline is one-shot network pruning baseline not using meta-validation dataset $\dataval$ as part of a reconstruction loss.
%Our full approach for one-shot network pruning is denoted as Meta (ours).
%The result demonstrates that the propose meta-learning approach for one-shot network pruning is important to maintain accuracy while improving the fps.

% To confirm our two-layer training adaptive channel selector perform well than find {$\beta_{i}$} on the whole dataset, we train {$\beta_{i}$} directly using {\textit{$D_{s}$}} with loss ~\ref{pruneloss}. We also train two-layer MLP just using on {\textit{$D_{s}$}} to pair comparison. To train two implementation, we sample same number of images to original MLP in the same video and the number of iteration is also same for pair comparison. Training result is on the table ~\ref{tab:ablation_prune}.
% Using MLP rather than just finding {$\beta_{i}$} preserve the performance although it reduces same or more number of channels. This is because MLP could be understand relation of channels. And training adaptive channel selector with meta-learning also perform better than just using reconstruction error. It prevents overfitting to first frame of video. It could predict some channels can be used in the future and do not erase them.
...
% (changed in camera ready)We compare MetaRTT, MetaRTT+Prune and MetaRTT+COCO to the state-of-the-art real-time trackers~\cite{luca2016fullycovolutional,martin2017eco,fan2017parallel,hamed2017learning,jack2017endtoend,martin2017discriminative,jung2018realtime,zheng2018distractor}.
...
%Because our goal is to accelerate object tracking speed while maintaining accuracy, we compare only with the real-time trackers including the baseline tracker, RT-MDNet.
...
%\vspace{-0.4cm}
%\paragraph{Comparison on OTB2015}
...
%Note that the success rate and the precision of our best model on OTB2015 are 67.2 and 89.9, respectively when the model is pretrained on ImageNet-Vid and COCO dataset and does not perform channel pruning.
%Especially, MetaRTT has better performance than DaSiamRPN~\cite{zheng2018distractor}, a winner of real-time object tracking in VOT2018~\cite{matej2018vot}.
Figure~\ref{fig:temple_plot} shows that our trackers present outstanding performance on TempleColor as well. 
%Both models again significantly outperforms the DaSiamRPN, which might have been tuned carefully for VOT2018.
%Especially, our models significantly outperforms DaSiamRPN~\cite{zheng2018distractor}, a winner of real-time object tracking in VOT2018~\cite{matej2018vot}, which might have been tuned carefully for VOT2018.
%On the contrary, we observe that the hyperparameter meta-learned on ImageNet-Vid~\cite{russakovsky2015imagenet} generally works well on standard benchmarks without too much effort on finetuning them. 
%(camera-ready)These results show that the hyperparameters meta-learned on ImageNet-Vid~\cite{russakovsky2015imagenet} works well on the standard benchmarks without manual tuning for the benchmarks; we fix all the hyperparameters except the meta-parameters in both datasets.
...
%These results show that the hyperparameters meta-learned on ImageNet-Vid~\cite{russakovsky2015imagenet} works well on the standard benchmarks without additional offline training. 
The high accuracy by MetaRTT+COCO also suggests that our tracker can further improve by pretraining with more datasets.
%benefit from betterthe strength of meta-learning; we could improve the performance our tracker by pretraining with additional training data.
%In summary, the proposed tracking algorithm based on meta-learning, MetaRTT and MetaRTT+Pruning, are successful in terms of accuracy and efficiency, and 
...
%\includegraphics[width=0.49\linewidth]{OTB100_error_OPE_threshold_sota3.png}
%\includegraphics[width=0.49\linewidth]{OTB100_overlap_OPE_AUC_sota3.png}
%\includegraphics[width=0.49\linewidth]{OTB100_error_OPE_threshold.png}
%\includegraphics[width=0.49\linewidth]{OTB100_overlap_OPE_AUC.png}
% \subfloat{\includegraphics[width=0.49\linewidth]{figures/graphs/OTB100_error_OPE_threshold.png}}
% \subfloat{\includegraphics[width=0.49\linewidth]{figures/graphs/OTB100_overlap_OPE_AUC.png}}
% \end{center}
%\vspace{-0.2cm}
...
\begin{figure}[t!]
	...
	%	\includegraphics[width=0.49\linewidth]{TempleColor_error_OPE_threshold.png}
	%	\includegraphics[width=0.49\linewidth]{TempleColor_overlap_OPE_AUC.png}
	\end{center}
%\vspace{-0.2cm}
...
%\vspace{-0.4cm}
%\paragraph{Comparison on TempleColor}
...
% \begin{table}[t!]
% \begin{center}
% \caption{Effect of data augmentation for negatives}
% \label{tab:ablation_neg}
% \setlength{\tabcolsep}{5pt}
% \scalebox{0.9}{
% \begin{tabular}{l|l|ccc}
% %\hline%\noalign{\smallskip}
% Method & {} & 30\text{k Iter} & 60\text{k Iter} & 90\text{k Iter}\\
% \hline
% \hline
% %\noalign%{\smallskip}
% \multirow{2}{*}{MetaRTT}  
...
% \subsection{Evaluation on VOT-Challenges}
% \ilchae{Not yet.}

%\subsection{Ablation study}

%There are four variants to compare.
%First model indexed by \textit{Common lr} shares learning rate for all iteration in a update step while it separates $\alpha_{init}$ and $\alpha_{on}$.
%Secondary, we changes the number of update iterations on initial frame.
%The stable embedding to target-specific domain significantly depends on the number of iterations on first frame.
%Therefore, we compare two models that first one has 15 iterations on initial frame (\textit{Init15-Online5}) and the other has 5 iterations on initial frame (\textit{Init5-Online5})
...
%has a simple structure but robust to obtain tracker for fast and stable adaptation.
%By employing the offline training method, our algorithm accelerates tracking speed significantly while maintaining high accuracy.
%Additionally, with meta-learning, we propose a new one-shot channel pruning algorithm which accelerates execution speed.
%-------------------------------------------------------------------------
...
%This work was partly supported by the ICT R\&D programs of MSIT/IITP[2014-0-00059, 2017-0-01780, 2016-0-00563], LG-CNS co., Ltd., Seoul,. and NRF Korea (NRF-2017M3C4A7069369). 

%\textcolor{blue}{This work was partly supported by the ICT R\&D programs of MSIT/IITP[2014-0-00059, 2016-0-00563], LG-CNS co., Ltd., Seoul,. and NRF Korea (NRF-2017M3C4A7069369). }

%\textcolor{blue}{This work was partly supported by the ICT R\&D programs of MSIT/IITP[2014-0-00059, Development of Predictive Visual Intelligence Technology and 2016-0-00563, Research on Adaptive Machine Learning Technology Development for Intelligent Autonomous Digital Companion], and LG-CNS co., Ltd., Seoul,. Korea and Next-Generation Information Computing Development Program through the National Research Foundation of Korea(NRF) funded by the Ministry of Science, ICT (NRF-2017M3C4A7069369). }

%\textcolor{blue}{This work is partly supported by Institute of Information \& Communications Technology Planning \& Evaluation(IITP) grant funded by the Korea government(MSIT) (No.2014-0-00059,Development of Predictive Visual Intelligence Technology), LG-CNS corporation and Next-Generation Information Computing Development Program through the National Research Foundation of Korea(NRF) funded by the Ministry of Science, ICT (NRF-2017M3C4A7069369). }
