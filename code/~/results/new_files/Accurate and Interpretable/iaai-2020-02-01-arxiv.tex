...
/Author (Lumiata Data Science and Engineering, Rohun Kshirsagar, Li-Yen Hsu, Vatshank Chaturvedi, Charles H. Greenberg, Matthew McClelland, Hilaf Hasson, Anushadevi Mohan, Wideet Shende, Nicolas P. Tilmans, Min Guo, Ankit Chheda, Meredith Trotter, Shonket Ray, Miguel Alvarado)
...
\affiliations{
  ...
  \textsuperscript{*}Work completed prior to joining Amazon
    % \{rohun, liyen, anusha, wideet, miguel\}@lumiata.com \\
    % \{greenberg.charles.h, ntilmans, mer.trotter, sray12345678, Kearny.mcc, ankitjchheda, xmguomin\}@gmail.com \\
    % \newline
}
...
\begin{abstract}
% Healthcare costs in the United States have continued to climb over the past 15 years, with the costs increasingly being transferred onto patients.  Due to the ever increasing volatility in the employer-sponsored health insurance market, insurers are pricing insurance products in a risk-averse manner, which results in a more costly experience for the patient, and a turnover-prone client-base for the insurer. Machine-learning approaches using electronic health records have been used previously to predict disease and risk in patient populations, but have not been noted in the literature to augment actuarial pricing formulas used by most insurers today.  
...
% We also describe the importance of rigorous data quality tests, ensuring consensus on analysis from different stakeholders. The four areas to check are: (1) before data is sent on the originators side, (2) immediately after receiving the data, (3) after producing predictions, and (4) on the originators side again alongside the predictions.
...
% \subsection{Patient Involvement}
% We analyzed de-identified patient healthcare data.  Patients were not involved in the experimental design development, outcome measures, or study questions.
...
% \begin{figure}
%  \centering
%   \includegraphics[width=90mm, scale=0.4]{fig-1-slicing-holdout-logic.pdf}
%   \caption{Holdout set ``dynamic time-slicing".  This picture shows how holdout set groups are time-sliced according to their renewal dates, and thus have different lengths of historical data (shown as ``Features") and different projection periods (shown as ``Targets"). Lumiata was blinded from the information in blackout and projection periods before submitting predictions to Delphi.}
%   \label{fig:fig1}
% \end{figure}
...
% This dataset is distinct from the data in Figure~\ref{fig:fig1}.

% training dataset, and censored versions of these files omitting the projection period data for the holdout dataset from Delphi (we subsequently received the uncensored holdout data after our final predictions were submitted). 
...
% which health plans a patient was enrolled in, what time periods these enrollments occurred, and what plan benefits were offered through these plans.
...
% In Table~\ref{tab:a1}, we summarize the features used by our individual-level model.
...
%\item the expected cost based on the average claims rate at the zipcode level
% \end{itemize}
...
% Please refer to Appendix \ref{sec:a1} \& \ref{sec:a2} for more details on these two models and the features they used. We therefore compared the performance of our 
% sequence of two models with Delphi's ``best estimate model running in production.
% \begin{figure}
%   \includegraphics[width=\linewidth]{model_diagram.pdf}
%   \caption{Sequence of two models we used to produce group-level cost predictions. The first model is a member-level model regressed on per month cost; the second one is a group-level model regressed on per member per month (pmpm) cost.}
%   \label{fig:fig4}
% \end{figure}

% \subsection{Data Quality}\label{sec:dq}

% \paragraph{} In our context here, we call Extract-Transfer-Load (ETL) the process by which employer-group-centric administrative claims data is re-shaped into a patient-centric data model, called the Lumiata Data Model (LDM). Data Quality is defined as the tests and checks done on the ETL and the downstream modeling pipeline to ensure faithful representation of the source claims data and credibility of the data in the LDM. Due to fact that claims data can be messy \cite{noisyclaims}, data quality is an essential prerequisite to building accurate and precise ML models using claims data.  Without this piece, any performance results achieved can be called into question because of a disagreement about the ``basics".

\subsection{Model Evaluation}

% Using unseen data from the holdout set, we could evaluate different models by calculating several standard machine learning performance metrics. However, several actuarial concepts in group underwriting 
% must be applied in this process. We present a couple of definitions in the following.

% where the former is defined as:
% \begin{equation*}
% \text{true pmpm cost} = \frac{\text{true total cost}}{\text{member months during projection period}}
% \end{equation*}
...
% Our employer-group insurance population included 648 employer groups spanning 349,715 members in the holdout set enrolled at the time of their respective group's renewal date. The renewal dates for the groups in holdout spanned 05/01/2016 -- 04/01/2017.
...
%Baseline - 20 cost features only & 0.207 & xx & 0.575 \\
...
% is $\sim$ 0.89, which is a safer decision rule than using 1.0 (as discussed in 'Concession Opportunities...') if a higher precision is desired.
...
% \begin{figure}
%  \centering
%   \includegraphics[width=90mm, scale=0.4]{model-explain.png}
%   \caption{Diagram shows the aggregation of member-level SHAP values at employer group level.  This information can explain how a rate was arrived at, and what actions could be taken to modify the rate in future.}
%   \label{fig:fig6}
% \end{figure}
...
% While we proceeded with a comparison of models the usual ML metrics (as in the results section), we felt the lift plot most faithfully captures the comparative value of Lumiatas model over Delphis since it directly uses trend predictions for both models.
...
% \begin{figure}
%   \includegraphics[width=\linewidth]{mae_plot.pdf}
%   \caption{Normalized pmpm MAE of Lumiata and Delphi's models by employer group size segment. Lumiatas model is better across all segments.}
%   \label{fig:fig6}
% \end{figure}
