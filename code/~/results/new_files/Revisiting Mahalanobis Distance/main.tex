...
%&   CNN MSP  &   $92.3\pm1.2$    &    $75.5\pm5.8$     &     $33.4\pm2.9$       &     $32.8\pm9.7$      \\
%\cline{2-6} 
&   CNN MSP  & $94.1\pm0.6$ & $80.8\pm2.1$ &     $26.4\pm4.0$       &     $24.4\pm2.8$      \\
%\cline{2-6} 
%& CNN+ER+AEPOG & 95.8 & --  &  -- & 23.7 \\
...
%\cline{2-6} 
%& $\rm{BERT_{large}}$ Maha & $97.9\pm0.1$ & $91.7\pm0.6$ & $8.3\pm0.7$ & $9.1\pm0.7$ \\
%\cline{2-6} 
...
%To calculate Mahalanobis distance by PCA, the preparation steps are performed:
%(i) class-wise centering of the ID training data: 
%$\tilda{\psi(x)}=\psi(x)-\mu_{c}$ for each x from the class $C$, $\mu_c$ is the class centroid;
%(ii) extraction of the Principal Components for the centered data.

%At test time, given vector $\psi(x)$, we may calculate the distance to each class by subtracting the class centroid and then performing the PCA decomposition of the vector $\psi(x)-\mu_{c}$.
%The final distance is the minimum of all class distances, and has the following form:
\begin{equation}
\begin{gathered}
...
% For the fine-tuned Transformer embeddings, the terms with numbers higher than $N$ are highlighted for OOD examples. This property holds neither for CNN nor for Transformer embeddings without fine-tuning.
...
\section{Conclusion}
%\input{parts/dependence_of_train_data_fig}
\begin{figure}[t]
    \centering
