...
def run_feature_extraction(list_latex_files,list_pdf_files,list_bib_files):
    ...
    columns = []
    for i in range(len(list_latex_files)):
        columns.append('Doc' + str(i))
    Rows = []
    ...
    Formula_keys = ['page', 'column', 'start_y', 'end_y', 'height','bad_detection_formula' ,'space_between_this_object_and_last_object',
                    'space_between_this_object_and_the_next_object']
    Algorithm_keys = ['page', 'column', 'start_y', 'end_y', 'spread_on_more_than_1_column', 'height','algorithm_position_param',
                      'space_between_this_object_and_last_object', 'space_between_this_object_and_the_next_object']
    ...
    for file_index in range(len(list_latex_files)):
        res = test.run(latex_path="pdf-tests\\" + list_latex_files[file_index] + ".tex",pdf_path="pdf-tests\\"+list_pdf_files[file_index] +".pdf",bib_path="pdf-tests\\"+list_bib_files[file_index]+".bib")
        max_height = res[-1]
        res = res[:-1]
        print(res)
        # print(order)
        ...
        for object in res:
            ...
            if(object['Type'] == 'Figure'):
                ...
                position_par = object['Text'][2]
                if position_par == "":
                    dct[object_name]['figure_position_param'] = 0
                elif position_par == "b":
                    dct[object_name]['figure_position_param'] = 1
                elif position_par == "t":
                    dct[object_name]['figure_position_param'] = 2
                elif position_par == "h":
                    dct[object_name]['figure_position_param'] = 3
                else:
                    dct[object_name]['figure_position_param'] = 4
                ...
                dct[object_name]['space_between_caption_and_figure'] = abs(last_object['First_line_bbox'][1] - object['First_line_bbox'][0])
                figure_has_caption = False
                ...
                if pdf != []:
                    ...
                    dct[object_name]["last_line_length_chars"] = 0
                    dct[object_name]["last_line_length_words"] = 0
                ...
                position_par = object['Text'][2]
                if position_par == "":
                    dct[object_name]['table_position_param'] = 0
                elif position_par == "b":
                    dct[object_name]['table_position_param'] = 1
                elif position_par == "t":
                    dct[object_name]['table_position_param'] = 2
                elif position_par == "h":
                    dct[object_name]['table_position_param'] = 3
                else:
                    dct[object_name]['table_position_param'] = 4
                ...
                #dct[object_name]['figure_caption'] = object
                dct[object_name]['space_between_caption_and_table'] = abs(last_object['First_line_bbox'][1] - object['First_line_bbox'][0])
                table_has_caption = False
                ...
                if pdf != []:
                    ...
                    dct[object_name]["last_line_length_chars"] = 0
                    dct[object_name]["last_line_length_words"] = 0
                if (dct[object_name]["number_of_lines"] > max_lines_caption[1]): # we can make it more specific to CaptionTable insted of combining CaptionFigure and CaptionTable
                    max_lines_caption = (object_name, dct[object_name]["number_of_lines"])
                if (dct[object_name]["number_of_lines"] < min_lines_caption[1]):
                    min_lines_caption = (object_name, dct[object_name]["number_of_lines"])
            ...
            elif(object['Type'] == 'Par' or object['Type'] == 'Paragraph' or object['Type'] == 'Title' or object['Type'] == 'AbstractSection' or object['Type'] == 'AbstractPar' or object['Type'] == 'Section' or object['Type'] == 'SubSection' or object['Type'] == 'SubSubSection' or  object['Type'] == 'Enum' or object['Type'] == 'Algorithm'):
                if(object['Type'] == 'Par'):
                    par_index+=1
                    if (object['j'] == 0):
                ...
                if (object['Type'] == 'Algorithm'):
                    ...
                    position_par = object['Pos']
                    if position_par == "":
                        dct[object_name]['algorithm_position_param'] = 0
                    elif position_par == "b":
                        dct[object_name]['algorithm_position_param'] = 1
                    elif position_par == "t":
                        dct[object_name]['algorithm_position_param'] = 2
                    elif position_par == "h":
                        dct[object_name]['algorithm_position_param'] = 3
                    else:
                        dct[object_name]['algorithm_position_param'] = 4

                if(object['Type'] != 'Algorithm' and object['Type'] != 'Section' and object['Type'] != 'SubSection' and object['Type'] != 'SubSubSection' and object['Type'] != 'AbstractSection'):
                    pdf = object['pdf_array']
                    for i in pdf:
                        if(i[1][-1] == '-'):
                            if(add_space):
                    ...
                    if(object['Type'] == 'Par' or object['Type'] == 'Paragraph'):  #we could change it that many of the same size will count as max, not only a single first max (or min)
                        ...
                        if(dct[object_name]["number_of_lines"] < min_lines_par[1] and dct[object_name]["number_of_lines"] >= 0):
                        ...
                        if(dct[object_name]["number_of_lines"] < min_lines_enum[1] and dct[object_name]["number_of_lines"] >= 0):
            ...
            #spaces part
            if(last_object['Type'] == 'Matrix' or last_object['Type'] == 'Formula' or last_object['Type'] == 'Figure' or last_object['Type'] == 'Table'):
                if(object['Type'] == 'Figure' or object['Type'] == 'Table'): #trying to detect subfigures
                    if(object['First_line_bbox'] == last_object['First_line_bbox']):
                        print(dct)
                        if('space_between_this_object_and_last_object' not in dct[last_object_name]):#if we start the doc with an image
                            dct[object_name]['space_between_this_object_and_last_object'] = abs(50.01389562499992 - object['First_line_bbox'][0])
                        else:
                            dct[object_name]['space_between_this_object_and_last_object'] = dct[last_object_name]['space_between_this_object_and_last_object']
                    else:
                        dct[object_name]['space_between_this_object_and_last_object'] = abs(last_object['First_line_bbox'][1] - object['First_line_bbox'][0])
                else:
                    # print(last_object['Type'])
                    # print(object['Type'])
                    dct[object_name]['space_between_this_object_and_last_object'] = abs(last_object['First_line_bbox'][1]-object['First_line_bbox'][0])
                dct[last_object_name]['space_between_this_object_and_the_next_object'] = dct[object_name]['space_between_this_object_and_last_object']
            else:
                ...
                if(last_object == object): #for title we will calculate the space between the title and the coords of the first figure if it existed (coords of first figure in a page is: (50.01389562499992))
                    ...
                    if(last_object['Last_line_bbox'][1] -object['First_line_bbox'][0] > 100): #next page difference, so we will take space from the top of the page which is: (50.01389562499992)
                        dct[object_name]['space_between_this_object_and_last_object'] = abs(50.01389562499992 - object['First_line_bbox'][0])

                    else:
                        dct[object_name]['space_between_this_object_and_last_object'] = abs(last_object['Last_line_bbox'][1] -object['First_line_bbox'][0])
                dct[last_object_name]['space_between_this_object_and_the_next_object'] = dct[object_name]['space_between_this_object_and_last_object']
        ...
        if(min_lines_par[1] == 1000 or min_lines_par[1] < 0):
            min_lines_par = list(min_lines_par)
            min_lines_par[1] = 0
            min_lines_par = tuple(min_lines_par)
        ...
        if (min_lines_enum[1] == 1000 or min_lines_enum[1] < 0):
            min_lines_enum = list(min_lines_enum)
            min_lines_enum[1] = 0
            min_lines_enum=tuple(min_lines_enum)
        ...
        if(min_lines_caption[1] == 1000 or min_lines_caption[1] < 0):
            min_lines_caption = list(min_lines_caption)
            min_lines_caption[1] = 0
            min_lines_caption=tuple(min_lines_caption)
        ...
        if (min_figure_y_space[1] == 10000 or min_figure_y_space[1] < 0):
            min_figure_y_space=list(min_figure_y_space)
            min_figure_y_space[1] = 0
            min_figure_y_space=tuple(min_figure_y_space)
        ...
        if (min_table_y_space[1] == 10000 or min_table_y_space[1] < 0):
            min_table_y_space=list(min_table_y_space)
            min_table_y_space[1] = 0
            min_table_y_space=tuple(min_table_y_space)
        ...
        if (min_height_object[1] == 10000 or min_height_object[1] < 0):
            min_height_object=list(min_height_object)
            min_height_object[1] = 0
            min_height_object=tuple(min_height_object)
    ...
    #df.at[Rows[0],'Doc1'] = summative_features[Rows[0]]
        ...
        while (i < len(Rows)):
            if(i < 43):
                if(i < 12):
                    df.at[Rows[i], 'Doc'+str(file_index)] = summative_features[Rows[i]][1]
                    i += 1
                else:
                    df.at[Rows[i], 'Doc'+str(file_index)] = summative_features[Rows[i]]
                    i += 1
            elif(i<155):
                if('Par' in count_dct):
                    if(counter_par <= count_dct['Par']):
                        ...
                        for value in dct[stri].values():
                            if value:
                                df.at[Rows[i], 'Doc'+str(file_index)] = value
                            else:
                                df.at[Rows[i], 'Doc'+str(file_index)] = 0
                            # df.loc[Rows[i]] = value
                            i += 1
                        counter_par+=1
                    else:
                ...
                if ('Paragraph' in count_dct):
                    if (counter_paragraph <= count_dct['Paragraph']):
                        stri = 'Paragraph' + str(counter_paragraph)
                        for value in dct[stri].values():
                            if value:
                                df.at[Rows[i], 'Doc'+str(file_index)] = value
                            else:
                ...
                if('Figure' in count_dct):
                    if (counter_figure <= count_dct['Figure']):
                        ...
                        for value in dct[stri].values():
                            # print(Rows[i])
                            if value:
                                df.at[str(Rows[i]), 'Doc' + str(file_index)] = value
                            else:
                                df.at[str(Rows[i]), 'Doc' + str(file_index)] = 0
                            i += 1
                        counter_figure += 1
                    else:
                        df.at[Rows[i], 'Doc'+str(file_index)] = 0
    ...
    print(df.T)
    #df2 = df.T
    return df,summative_features,dct,count_dct
