...
def run_feature_extraction(list_latex_files,list_pdf_files,list_bib_files):
    ...
    for file_index in range(len(list_latex_files)):
        ...
        res = test.run(latex_path="" + list_latex_files[file_index] + ".tex",pdf_path=""+list_pdf_files[file_index] +".pdf",bib_path="pdf-tests\\"+list_bib_files[file_index]+".bib")
        #print(res)
        max_height = res[-1]
        res = res[:-1]
        print(res)
        ...
        with open('pdf_extraction\\adi_comparing\\dct'+str(file_index),'wb') as dct_file:
            pickle.dump(dct,dct_file)
        """
        with open('pdf_extraction\\adi_comparing\\latex_path'+str(file_index),'wb') as latex_path_file:
            objject = "pdf-tests\\" + list_latex_files[file_index] + ".tex"
            pickle.dump(objject,latex_path_file)
        with open('pdf_extraction\\adi_comparing\\bib_path' + str(file_index), 'wb') as bib_path_file:
            objject = "pdf-tests\\"+list_bib_files[file_index]+".bib"
