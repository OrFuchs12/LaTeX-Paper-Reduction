...
% Sultan J. Majeed (2017-present)
% http://sultan.pk/article-template.tex
%--------------------------------------------------------
...
% TODO: find a clean way to define these options

% type =  conference | journal | report
\newcommand{\type}{report}

% verbosity =  short | long
\newcommand{\verbosity}{short}

% mode =  draft | review | submission | final
\newcommand{\mode}{final}

% apx =  proofs | proofs-and-lemmas | <nothing, if not specified>
\newcommand{\apx}{proofs}

% numberlevel = section | subsection | <document level, if not specified>
\newcommand{\numberlevel}{section}

% counterlevel = same | <different, if not specified>
\newcommand{\counterlevel}{same}

% colorthm = true | <false, if not specified>
\newcommand{\colorthm}{false}

%--------------------------------------------------------
% Fixed configurations (do not edit)
%--------------------------------------------------------
...
% adjust the custom macros used in the title
\newcommand{\contact}[1]{}
\newcommand{\affiliation}[1]{\\#1}

%--------------------------------------------------------
\ifdefstring{\type}{conference}{%
    % Conference version settings
    %--------------------------------------------------------

    % load a custom conference template
    ...
    % set conference bibstyle here
    \renewcommand{\bibstylename}{aaai21} % aaai21


    % TODO: REMOVE THIS HACK IN THE FINAL SUBMISSION!!!!!
    %\renewcommand{\baselinestretch}{1.0}
    %\advance\textwidth by 6mm
    %\advance\textheight by 3mm
    % TODO: REMOVE ABOVE HACK IN THE FINAL SUBMISSION!!!!!
    ...
    %--------------------------------------------------------
        ...
        % Journal version settings
        %--------------------------------------------------------

        % load a custom journal template
        \documentclass{report} % <- here

        % set journal bibstyle here
        \renewcommand{\bibstylename}{apalike}

        %--------------------------------------------------------
    }{
        % Technical-report version settings
        %--------------------------------------------------------
        % default technical report style
        ...
        % set techical report bibstyle here
        \renewcommand{\bibstylename}{unsrtnat}

        % temporary fix for overfull hbox
...
% basic mathematics, theorem & symbols
\usepackage{amsmath, amsthm, amssymb, thmtools}

% TODO: a patch for cases when used with breqn and hyperref

\makeatletter
%\newbox\boxzero
%\patchcmd{\numcases}{\setbox\z@}{\setbox\boxzero}{}{}
%\patchcmd{\numcases}{\kern\wd\z@}{\kern\wd\boxzero}{}{}
%\patchcmd{\numcases}{\box\z@}{\box\boxzero}{}{}
%\patchcmd{\numcases}{\unhbox\z@}{\unhbox\boxzero}{}{}
%\patchcmd{\numcases}{\wd\z@}{\wd\boxzero}{}{}
...
% line-numbers for proof-reading [review mode]
\ifreview
...
% watermarking [draft and review mode]
...
% TODO: (FUTURE) ADD BIBLATEX SUPPORT
% bibliography
\usepackage[square,sort,comma,numbers]{natbib}
\usepackage{usebib}
% Mendeley generates a header text in the bib file
% which causes an error at this point.
% hack: remove that text from the bib file
% TODO: find a permanent fix for this error
% This is not DRY because TeXstudio does not support cite hints
% if a variable is provided in \bibliography
% This is required for \citetitle functionality
...
% ref and label keys [draft mode]
\ifdraft\usepackage{showkeys}\fi%

% color boxes around important environments
...
%--------------------------------------------------------
% Theorem environments - conditional def. and color
%--------------------------------------------------------
...
\title{
    ...
    %
%    \ifdefstring{\type}{report}{
%        \thanks{A shorter version to be appeared in the proceedings of the AAAI 2021 conference.}
%    }{}
}
...
%--------------------------------------------------------
% Todo & Notes Page - [draft mode only]
%--------------------------------------------------------
...
\ifdefempty{\mode}{
    ...
    % assuming the pagestyle is plain
    \pagestyle{plain}
    \setcounter{page}{1}
}{}
...
\begin{proof}
    ...
    %    &\overset{(c)}{=} \sum_{o'r'} P(o'r'|ha) \\ &\phantom{=}\left(r' + \lambda^d \max_{\v x' \in \B^d} \u Q^{*}(g\inv(\tau \vo{xor_\bot}_{< d} \v x_d o'r'), D(\v x'), r_\bot)\right) \\
...
\begin{proof}
    ...
    %By \Cref{thm:esa} this induces an $\eps$-optimal policy for $P$.
...
This work has been supported by Australian Research Council grant  DP150104590. Thanks to the anonymous reviewers for their feedback and to Andrs Gyrgy who pointed out that the sequentialized/binarized process in \Cref{fig:example} preserves the Markov property, which encouraged us to
...
\begin{small}
    ...
    Inventore quae dolorum hic maxime eos nobis praesentium consectetur, possimus perspiciatis ab repudiandae placeat quae nemo.Accusamus incidunt est sapiente voluptatum corrupti fuga ut fugit ipsa, ipsa culpa velit ratione aliquid.Non fugiat minus totam nihil cumque fuga dolores dolor nesciunt ad debitis, iste at accusantium architecto voluptates necessitatibus esse molestiae voluptatem voluptas nisi deleniti, mollitia quas dolorum labore quam non iste rerum commodi, repellat autem architecto
