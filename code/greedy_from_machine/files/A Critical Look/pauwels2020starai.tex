...
/Author (Johan Pauwels, Gyorgy Fazekas, Mark B. Sandler)
} %Leave this	
% /Title ()
...
%We end by briefly touching upon Bayesian logic networks (BLNs), an alternative formalism in the encompassing field of Statistical Relational Learning. While BLNs do not solve all practical issues associated with MLNs, they do seem a better fit for music signal analysis, while retaining some of the core ideas.
%In recent years, Markov logic networks (MLNs) have been proposed as a potentially useful paradigm for music signal analysis. Because all hidden Markov models can be reformulated as MLNs, the latter can provide an all-encompassing framework that reuses and extends previous work in the field. However, just because it is theoretically possible to formulate a problem as MLNs, does not mean that it is advantageous to do so. In this paper, we discuss a number of practical hurdles, such as the lack of support for sequences and for arbitrary continuous probability distributions, that make MLNs less than ideal for the proposed musical applications, both in terms of ease of formulation and computational requirements.
...
%The latter requires a certain degree of specialist knowledge to interpret. 
...
%Its function is comparable to a script for an actor. Just like the script of a play, some aspects of the performance are defined very precisely, while others are represented more coarsely. A script gives you the exact words to say, but not the exact voice inflections nor their exact timing. Similarly, sheet music gives exact information about the pitch of the notes to be played, but only a quantised version of their timing and little info about its timbre. So when we want to transcribe a script or sheet music from a voice or music recording, we need to learn what aspects of the signal to discard and which ones to retain. And since a performance involves a fair amount of human interpretation, many different audio signals can be produced from the same script or sheet music.
...
% Theoretical modelling capabilities are the same as MN, but practicality differs
...
%The exact observations are therefore only applicable to one specific index of the sequence, which means that the FOL formulas are defined over singleton domains.

\subsection{Handling sequences}
%Only the transitions between hidden states are stationary and song-independent, and can therefore be described by first-order logic.
...
% the network has the provision to make arbitrary connections
...
%When it comes to defining additional connections, it is always clearer to just specify the connections that need to be made, because these connections need to be given by propositional predicates anyway, as there is no regularity that can be used to compactly describe them.
...
%Chain-like network architectures can be equally well represented as undirected or directed graphs, so there is no problem in that regard.

\begin{figure}[htb]
\medmuskip=0mu
\begin{minipage}[t]{0.4\linewidth}
  \centering
%  \beginpgfgraphicnamed{dbn-ismir1}
...
\begin{minipage}[t]{0.6\linewidth}
  \centering
  %\beginpgfgraphicnamed{dbn-ismir2}
  \begin{tikzpicture}[every node/.append style={align=center, text width=1cm}]
	  % Define nodes
	  \node[latent] (c1) {$\operatorname{c}\left(t-1\right)$};
	  \node[latent, right=6mm of c1] (c2) {$\operatorname{c}\left(t\right)$};
...
\begin{minipage}[t]{\linewidth}
  \centering
  %\beginpgfgraphicnamed{dbn-ismir3}
  \begin{tikzpicture}[every node/.append style={align=center, text width=1cm}]
	  % Define nodes
	  ...
	  % Plates
%	  \plate {} {(c2)(o)(k2)} {time $t$} ;
%	  \plate {} {(c1)(k1)} {time $t-1$} ;
  \end{tikzpicture}
  %\endpgfgraphicnamed
...
\begin{minipage}[b]{\linewidth}
  \centering
  %\beginpgfgraphicnamed{bn-icassp}
  \begin{tikzpicture}
	  % Define nodes
...
%a chord estimation system is proposed that exploits structural relationships between beat-synchronised segments.

%%QUESTIONS: is fig 2c a chordal graph? Has it maximum clique of two? It is not a tree. Is every graph with maximum clique two a chordal graph?
...
%Moreover, the weights are all derived or can be interpreted as such.


%A criticism of papadopoulos2012ismir
%
%Intent/Applicability The proposed relations between key, chord and musical structure do not exploit the benefits of MLN, but can equally well be described in a purely probabilistic framework.
%
%Execution The framework of MLNs handles only discrete variables. Therefore modelling an HMM with (multivariate) continuous emission distributions is theoretically unsupported. A hack around this limitation has been proposed by enumerating all continuous observations in a specific music piece and considering them as discrete observations. The drawback is that a specific MLN is required for every music piece, thereby confounding the separation between MLN program and evidence, which counters the elegance that the MLN framework is supposed to bring.


%no arithmetic support and no continuous observations for MLN, no continuous observations for BLN, hacks necessary to make it work, no separation between network and evidence, unelegant formulations of evidence, suboptimal generated networks, unnecessary introduction of variables to create FOL where propositional logic would suffice and be clearer, could use probability of similarity
