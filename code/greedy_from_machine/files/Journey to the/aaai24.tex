...
%     Yuheng Chen\textsuperscript{1,2},
%     Pengfei Cao\textsuperscript{1,2},
%     Yubo Chen\textsuperscript{1,2},
%     Kang Liu\textsuperscript{1,2},
%     Jun Zhao\textsuperscript{1,2}
% }
% \affiliations{
%     %Afiliations
%     \textsuperscript{\rm 1}National Laboratory of Pattern Recognition, Institute of Automation, CAS, Beijing, China\\
...
% The ideal baseline vector ${w'}^{(l)}_{j}$ should be devoid of information \cite{transparent}, and current methods approximate it with a zero vector. But such a setting does not take into account the differences among various PLM architectures, leading to sub-optimal performance. To alleviate this problem, we design an architecture adaptation technique to compute the baseline vectors for various PLM architectures.
...
% For specific hyperparameter settings, please refer to the Appendix.
...
\midrule
\end{tabular}}
% \caption{Results of the localization of knowledge neurons. B-KN is the baseline method, the symbol ``\textuparrow" indicates the increase in success rate compared to B-KN for our method, and bold indicates the method with a higher $SR$.}
...
\bottomrule
\end{tabular}}
% \caption{Results of cross-lingual knowledge editing. LIKN represents editing language-independent knowledge neurons, Mono-KN denotes editing knowledge neurons in one language's dataset corresponding to another, and Seq-KN denotes sequentially editing knowledge neurons in two languages. The symbol  ``\textuparrow" indicates the increase in success rate compared to Mono-KN for LIKN, while ``\textdownarrow" indicates the decrease in success rate compared to LIKN for Seq-KN.}
...
% \vpara{Fact-Checking Experimental Results}
...
% The key point to note here is that the low prediction probability of the model does not conflict with the localization of knowledge neurons. In Equation~\ref{eq:1}, we employed the true value \( y^* \), ensuring that even when the model outputs an incorrect prediction, we can still accurately locate the position of the correct knowledge neurons.\
...
% \caption{Results of the fact-checking experiments. wo\_DKN and with\_DKN represent methods without and with degenerate knowledge neurons, respectively. The symbol ``\textuparrow" indicates an increase in F1-score for with\_DKN over wo\_DKN, represented as $\frac{\text{with\_DKN} - \text{wo\_DKN}}{\text{wo\_DKN}}$, and bold highlights the higher F1-score.}
