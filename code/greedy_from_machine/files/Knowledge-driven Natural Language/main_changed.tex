...
%\titlespacing{\section}{2.75pt}{2.75pt}{2.75pt}
...
%\author {Kinjal Basu, Sarat  Varanasi, Farhad Shakerin, Joaquin Arias and Gopal Gupta\\
...
%Both of them showcases their understanding of the language by providing natural language explanation of the their responses while maintaining high accuracy.
%Gopal20
...
%KB20
%To get the semantics of a sentence, at first, the sentence is parsed and then, the parsed information, along with the frame-semantics of VerbNet, is used to represent the semantics of a sentence.
...
\noindent\textbf{ASP:}
%
%CITE aqua (PADL 20), square (ICLP,2020), caspr (PADL 19)
...
%Thus, the constraint {\tt :- u, v.} states that {\tt u} and {\tt v} cannot be both true simultaneously in any model (called an answer set).
...
%KB20
%Note that there can be multiple frames for a given verb class, one for each type of usage  of the verb. The first line in the figure (\textit{NP V NP}) shows the syntax tree pattern for this particular usage. The second line shows an example of an English sentence that matches the usage. The third line  (we call it \textit{frame syntax}) shows the constraints on the syntactic categories NP, V, PP, etc. (e.g., the first noun phrase should be of type AGENT). The final line (we call it \textit{frame semantics}) shows the meaning expressed using basic primitive predicates of VerbNet (\textit{continue, cause, contact}, etc.). A verb in an English sentence generally corresponds to an event. The meaning expressed using primitive predicates represents the semantic relations between event participants across the course of the event.
...
%Gopal2020: better phrasing?
%possess greater strengths compared to the machine learning based systems, and (iii) shows how commonsense knowledge can be included for reasoning and how the goal-directed s(CASP) answer set programming engine guarantees a correct answer with justification as long as the semantic representation and query generation are correct.
%and, (iv) analyzes the error and pointed out the deficiency on purely machine learning based systems on the dataset.
%KB20
...
%Gopal2020: we had to add it because it was not in Verbnet;
%For sentences that have a \textit{be} verb, we process their parse-trees in a similar way with only one exception---we use manual mapping of the dependency graph to its semantics instead of using VerbNet.
%KB20
%Please note that VerbNet does not cover the semantics of the \textit{`be'} verbs (i.e., \textit{am, is, are, have,} etc.). So, for the sentences that contains \textit{`be'} verbs,  the \textit{semantic generator} uses pre-defined hand-crafted mapping of the parsed information (i.e., syntactic parse tree, dependency graph, etc.) to its semantics and represents it as an answer set program.
%KB-JOA
...
%KB20
%Next, we present two applications of the above mentioned approach---(a) \textit{SQuARE: \underline{S}emantic based  \underline{Qu}estion  \underline{A}nswering and  \underline{R}easoning  \underline{E}ngine} and (b) \textit{StaCACK -  \underline{Sta}teful  \underline{C}onversational  \underline{A}gent using  \underline{C}ommonsense  \underline{K}nowledge.}
%Next, we present two applications of the above mentioned approach - the SQuARE and the StaCACK. In both  applications, to obtain good performance on different tasks, the generated ASP knowledge for the considered text along with commonsense knowledge is used. Our commonsense knowledge is coded in ASP as default rules, exceptions to defaults, and preferences between (multiple) defaults. We have tested the SQuARE system on the bAbI QA dataset \cite{babi} and the StaCACK framework on the bAbI dialog dataset \cite{babi_dialog} from Facebook research.
%Our semantic knowledge generation process works well on simple sentences and the datasets are largely consists of simple sentences with the main focus on reasoning.
...
%So, SQuARE assigns a unique time identifier for each event in a temporal order corresponding to the order of the sentences containing these events.
%ASP knowledge of the passage is the union of answer set program of its sentences, where the meaning of each sentence is also represented as an answer set program.
...
%KB20
%if \verb|X| is a person and the emotional state of \verb|X| is \textit{thirsty}, then normally \verb|X| takes the action of drinking but there can an exception (e.g., a medical test needs to be performed which requires person does not drink any fluid in last 1 hour), represented using the NAF clause.
...
%KB20
%As discussed earlier, the algorithm \ref{algorithm1} takes the parsed information from the previous step and generates the  semantics. Figure \ref{fig:verbnet-example} shows an example of a skeletal parse tree along with the frame-syntax and semantics of the verb \textit{grab} from VerbNet. The algorithm processes and generates multiple semantics for different verbs and because of the space constraint, we only give a snippet of knowledge generated from the third sentence of the story.
...
%KB-JAH
%The\textit{ ASP query generator} generates the following query in addition to the query-rule for the question - \textit{``How many objects is John carrying?''}. The process template for counting is used.
...
%KB
%Current chat-bots learn patterns in data from large corpora and compute a response without understanding the conversation. Due to lack of ``understanding'' of the content, these bots' capabilities are rather limited.
...
%Even though users of a conversational system can talk about anything they want, we found that the task based conversations can be grouped into several similar types. Therefore, to design the StaCACK framework, using the finite state machine (FSM) would be an efficient way to tackle the cases.
%Gopal2020
...
%KB_11.08: SQuARE Performance Table
\begin{table*}[b]
\centering
\tiny
...
%As discussed earlier, for the SQuARE system, first each story/passage is converted into an answer set program and the query generator generates the ASP query from the English question. These information along with the pre-defined commonsense knowledge in ASP are executed against the s(CASP) engine to get the answer and then the predicted answer is checked with the actual one to get the accuracy.
%KB20
%For StaCACK, we follow a process of testing similar to the SQuARE system, where it matches the predicted response with the actual bot response to obtain response-accuracy. By matching the whole dialog we obtain the dialog-accuracy.
...
%without indicating any particular one.
...
\hline
% \multirow{2}{4em}{\backslashbox[15em]{\hphantom{thequ}\bf Tasks}{\bf Attributes\hphantom{th}}}%removedVspace
  \multirow{2}{5em}{~ \ }
  & \multicolumn{1}{c|}{No.}
...
elsewhere~\cite{dmn}.
\fi
% KB20 The baseline model N-gram classifier \cite{babi} and the basic
% deep-learning model LSTM \cite{lstm} are the result of weak
% supervision. Whereas the Structured SVM \cite{babi}, the dynamic
% memory network \cite{dmn}, and the memory-neural-network (MemNN)
% with adaptive memory (AM), N-grams (NG), and non-linearity (NL)
% \cite{babi} are trained on the training data and as well as some
% external supporting facts. Multitasking with MemNN shows the
% accuracy if all the data from all the 20 tasks are used for
% training.
