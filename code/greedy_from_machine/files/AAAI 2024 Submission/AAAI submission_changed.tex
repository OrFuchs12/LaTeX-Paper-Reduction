...
%The paper now, following Section~\ref{relatedWork} with related work, explains the basic model and introduces the regular Partition mechanism in Section~\ref{prelim}. We then present our two algorithms in Section~\ref{algos}: one is a strict Two-Stage Partition (Algorithm~\ref{2StagePartition}), and the other has a more flexible structure, allowing the decisions of the first stage to be overturned (Algorithm~\ref{2StageFlexPartition}). Following the algorithms' descriptions, we explain our experimental setup in Section~\ref{setup}, followed by our results in Section~\ref{results}. We finish in Section~\ref{disc}, exploring various results and noting several future directions.
...
%\begin{algorithm}
%\caption{Two-Cluster Partition}\label{partition}
%\begin{algorithmic}[1]
%\Require
%Set of agents $N$; %, valuations ($v1$, \ldots , $vn$ ) of the agents,
%\newline
%$k$ the number of agents to be chosen (we assume $k$ is divided by 2);
%\newline
%$m$ the number of reviews per agent;
%\newline
%$Reviewer2Agents(C_{1},C_{2},m)$ a function that assigns for each agent $i$ a group $N_{i}$ of size $m$ of candidates from the other cluster.
%\Ensure Set of accepted agents $W$.
%\State Generate a partition {$C_{1}$, $C_{2}$} of $N$ randomly
%% where the difference between the sizes of the two clusters is at most 1. \\ For simplicity lets assume $odd agent \in C_{1} else agent \in C_{2}$.
%%Lets define $N_{i}$ as list of $m$ agents that not in $i$ cluster.
%%that agent $i \in C_{j}$ need to review,
%%$N_{i} = \{ (i+j-1)\% N, (i+2+j-1)\% N, \ldots, (i+2m+j-1)\%N \}$
%
%\State $Reviewer2Agents(C_{1},C_{2},m)$
%\For{agent $i \in N$}
% \State Agent reviews agents $\in N_{i}$, producing a grade $A_{i}$.
%\EndFor
%\State Individual grades are aggregated \Comment original Partition paper used approval voting, so aggregated by summing up approval votes; We compute average grade for each candidate.
%\State $W \gets \{ \frac{k}{2}$ top candidates $\in C_{1} \} \cup \{ \frac{k}{2}$ top candidates $\in C_{2} \}$
%\State return W
%\end{algorithmic}
%\end{algorithm}
...
% \begin{definition}
% Multi step ranking
% \omer{What is this? Do we use it later anywhere? See the explanation before Partition's explanation, and this needs an explanation as well.\\ Also, notice the algorithm itself runs over (text beyond the page), and it needs to be fixed.}
% \begin{algorithm}
% \caption{S step ranking}\label{euclid}
% \begin{algorithmic}[1]
% %S step ranking
% %\begin{algorithmic}
% \Require Set of agents $N$, valuations ($v1$, \ldots , $vn$ ) of the agents, $m$ the number of reviews per agent, $f$ the number of reviews in first round, $h$ size of the higher candidates group, $l$ size of the lower candidates group .
% \Ensure Set of winning agents $W$.
% \State $s \gets m - f + 1$ \Comment{Number of rounds}
% \State H = $\emptyset$; \Comment{Current higher candidates}
% \State L = $\emptyset$; \Comment{Current bottom candidates}
% \State m' = f; \Comment{Number of reviews per candidate in Current round}
% \State AgentReviewed = $(\emptyset , \ldots , \emptyset)$;
% \For{$round=0$ to $s$}
% \For{$agent$ $\in N$}
% \State ReviewGroup $\in$
% $\left\{
% \begin{array}{lr}
% N \setminus \\
% \{AgentReviewed[agent], H, L\},\\
% |ReviewGroup| = m'
% \end{array}
% \right\}$
% \State Agent review agents $\in ReviewGroup$
% \State AgentReviewed[agent] $\gets$ AgentReviewed[agent] $\cup ReviewGroup$
% \EndFor
% \State H = choose $h$ higher agents $\in N$
% \State L = choose $l$ higher agents $\in N$
% \State m' = $1$
% \EndFor
% \end{algorithmic}
% \end{algorithm}
...
%\State $numOfWinners \gets h$ \Comment{Number of agents to add to winner group in current round}
\State AgentReviewed = $(\emptyset , \ldots , \emptyset)$
\For{$round=0$ to $s$} \Comment Only two rounds in this algorithm
 \State $Reviewer2Agents(C_{1},C_{2},W,L,AgentReviewed,m')$
 \For{agent $i \in N$}
 % \State OtherClusterAgent = $\left\{
 % \begin{array}{lr}
 % C_{2} & \text{if } agent \in C_{1}\\
 % C_{1} & \text{if } agent \in C_{2}
 % \end{array}
 % \right\}$
 % \State ReviewGroup $\in$
 % $\left\{
 % \begin{array}{lr}
 % OtherClusterAgent \setminus \\ \{ AgentReviewed[agent] \cup W \cup L\}, \\ |ReviewGroup| = m'
 % \end{array}
 % \right\}$
 ...
 \State $m' \gets m-f$
 %\State $numOfWinners \gets k - numOfWinners$
\EndFor
\State return W
\end{algorithmic}
\end{algorithm}
...
%\oshri{I added the difference between the algorithms here}
%\omer{I rewrote this. Please read and verify I got it.}
...
% In Two-Cluster Partition Two-Step ranking we will divide our ranking process to maximum of two steps and high and low candidates rank groups will be chosen only once.We are also using a hard approach for the high and lower rank groups, meaning that if candidate been added to one of the groups he will stay there. For Two-Cluster Partition multi-step flexibility ranking we are using more flexibility approaches. First we are not limiting the maximum of ranking process, we will choose a number of votes for the first step and will add candidates for the higher and lower candidates rank groups and then we will keep one votes per candidate in each step until reach the number of votes per candidates $m$, and in each step we will choose the higher and lower candidates rank groups again. This is more flexibility approach for the high and lower rank groups, meaning that if candidate been added to one of the groups he still can be remove from the group later on.
...
\end{proof}
%\oshri{Write we did 2 different algorithms and explain it(Flex), write review group as function.}

%\omer{I don't understand why these algorithms use rounds, since the reported thing from each agent is not a value, but a ranking. I suggest assuming a matching between each agent and those it reviews (defined above as $N_{i}$ for agent $i$). You can write that such a matching exists, since Partition allows for a greedy assignment for the agents (e.g, if each agent needs $x$ reviews, agents $1,2,\ldots x$ review first paper, etc.).}
\begin{algorithm}[t]
\small
...
\For{$round=0$ to $s$}
 ...
 % $\left\{
 % \begin{array}{lr}
 % OtherClusterAgent \setminus \\ \{AgentReviewed[agent] \cup H \cup L\}, \\ |ReviewGroup| = m'
 % \end{array}
 % \right\}$
 \State Agent reviews candidates $N_{i}$ %ReviewGroup$
 \State AgentReviewed[i] $\gets$ AgentReviewed[i] $\cup N_{i}$
...
\subsection{Setup}
%We want to experiment on two step partition.
...
%(Tested values: $\frac{m}{4}$,$\frac{m}{2}$,$\frac{3m}{4}$)
...
%\begin{enumerate}
%\item Generate a noisy ranking for agents with a Mallows distribution
%\item Divide the agents to two clusters
%\item Run all algorithms on these clusters.
%\end{enumerate}
...
% 3 and Figure 4 show us the recall improvement for the new algorithms, we see that on lower winner group, less vote per candidate and higher PHI give the new algorithms the most improvement over the classic partition.
...
%candidates received votes compare to 1 step partition

%Figure 1 \omer{Figure numbers should not be hardcoded, but referenced, by writing \textbackslash ref\{figure-name\} instead of the number. Your figures have names, which are their label: what is written in the \textbackslash label\{figure-name\}}
...
% It seems that choosing $h$ (the size of the group of candidates we pass after the first stage) to be pretty low ($\frac{k}{4}$) \omer{Did we try lower values and found that this is best?} leads to better results in all our settings.
