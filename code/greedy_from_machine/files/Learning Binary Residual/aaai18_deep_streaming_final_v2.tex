...
\begin{document}
	...
	%
	%	\begin{itemize}
	%		\item datasets: three games, KITTI (TODO)
	%		\item evaluation metric - bitrates v.s PSNR, SSIM
	...
	%
	%	\begin{itemize}
	%		\item use the tracking dataset
	%		\item contain 47 sequences, 18062 images
	%		\item resize to 360 $\times$ 1200
	%		\item still test on training set?
	%		\item train models with different bitrate combinations
	%		\item compare different binarization methods?	
	%	\end{itemize}
	%
	\begin{figure*}[t]
		\centering
