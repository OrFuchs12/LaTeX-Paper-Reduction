...
To this end, we propose an \textbf{A}synchronous \textbf{F}ederated \textbf{T}rilevel \textbf{O}ptimization method (AFTO) in this paper. The proposed AFTO can effectively solve the TLO problems in an asynchronous federated manner. Specifically, it treats the lower-level optimization problem as a constraint to the upper-level and utilizes $\mu$-cuts to construct the hyper-polyhedral approximation, then an effective asynchronous algorithm is developed.  In the context of trilevel learning problems, the objective functions at each level are usually non-convex, thus the cutting plane methods tailored for convex functions \cite{jiao2022asynchronous,franc2011cutting} are found to be inapplicable. To our best knowledge, the proposed methodology referred to as  $\mu$-cut represents the first approach that is capable of constructing cutting planes for trilevel learning problems characterized by non-convex objectives. Furthermore, we demonstrate that the proposed method is guaranteed to converge and theoretically analyze the non-asymptotic convergence rate in terms of iteration complexity.
 ...
 % For constraint $f(\boldsymbol{x}) \le 0$, where function $f(\boldsymbol{x})$ is $\mu$-weakly convex, given a point $\boldsymbol{x}'$, the proposed $\mu$-cut is generated as $f(\boldsymbol{x}')+\nabla f(\boldsymbol{x})^{\top}(\boldsymbol{x}-\boldsymbol{x}')\le\mu(||\boldsymbol{x}||^2+||\boldsymbol{x}'||^2)$.  $\{\boldsymbol{x}| \; f(\boldsymbol{x}) \le 0 \}$
...
% It is worth mentioning that solving the TLO problem is theoretically NP-hard (even solving the inner bilevel problem in TLO is NP-hard \cite{ben1990computational}). Thus, it’s unlikely to design a polynomial-time algorithm for the original distributed TLO problem and compute the optimality gap unless P = NP \cite{arora2009computational}.  In this work, the hyper-polyhedral
% approximation problem (P1) in Eq. (\ref{eq:outer_poly}) is a convex relaxation problem of the original distributed TLO problem (P2) in Eq. (\ref{eq:1}) by setting $\epsilon_{\mathrm{II}} \! \ge \! 2(\beta + \beta'),  \epsilon_{\mathrm{I}} \! \ge \! \beta  $, where $\beta$ is the error bound when utilizing $K$ gradient descent steps to replace lower-level problems and $\beta'$ is the distance between the solutions of second-level optimization problem and second-level optimization problem with $\mathrm{I}^{\mathrm{st}}$ layer $\mu$-cuts. More discussions are presented in Appendix D. Thus, the optimal value to P1, i.e., $d^*$, provides a lower bound for the optimal value to P2, $p^*$. Furthermore, this lower bound is continuously tightened as $\mu$-cuts are added, i.e., $p^*/{d_{0}^*}\ge p^*/{d_{1}^*} \ge \! \cdots \!  \ge p^*/{d_{T}^*} \ge 1 $, which will be shown in Proposition \ref{prop:1} and \ref{prop:2}.
...
on (a) Diabetes, (b) Boston, (c) Red-wine quality, and (d) White-wine quality datasets. All experiments are repeated five times, and the shaded areas represent the standard deviation.} %  %大图名称
\label{fig:dis-hyper}
\end{figure*}
...
\end{minipage}}
\caption{(a) Test accuracy and (b) test loss vs running time when SVHN is utilized to pretrain the model. (c) Test accuracy and (d) test loss vs running time  when MNIST is utilized to pretrain the model. All experiments are repeated five times.} %  %大图名称
...
% \noindent \textbf{Distributed Robust Hyperparameter Optimization}
...
% \noindent \textbf{Distributed Domain Adaptation}
...
%Following \cite{ji2021bilevel}, we utilize the test accuracy/test loss vs running time to evaluate the proposed AFTO and the results are shown in Figure \ref{fig:dis-domain}. It is seen that the proposed AFTO can effectively solve the trilevel optimization problem in a distributed manner and exhibits superior performance than baseline method.
...
% This work was supported in part by the National Natural Science Foundation of China under Grant 12371519 and 61771013; in part by the Fundamental Research Funds for the Central Universities of China; in part by the Fundamental Research Funds of Shanghai Jiading District; in part by the National Natural Science Foundation of China (Project 62271434), Shenzhen Science and Technology Program (Project JCYJ20210324120011032), Guangdong Basic and Applied Basic Research Foundation (Project 2021B1515120008), Shenzhen Key Lab of Crowd Intelligence Empowered Low-Carbon Energy Network (No. ZDSYS20220606100601002), and the Shenzhen Institute of Artificial Intelligence and Robotics for Society.
 ...
 sequi harum quidem debitis fugit dolor commodi, hic ducimus nostrum officiis.Explicabo provident corrupti eos eum hic ipsum aperiam, totam pariatur ullam.Saepe officia soluta consequatur optio illo voluptatum eligendi voluptates, laborum ratione consequatur asperiores vel deserunt repellat cumque odio rem eaque.Impedit ad vel deserunt nisi eos tempore hic illo porro atque earum, excepturi rerum qui ut esse, eum inventore quo maxime quidem officia cupiditate quod consequuntur molestias sit debitis, deleniti corrupti fugit eligendi.Delectus distinctio necessitatibus perferendis, excepturi aperiam nihil ullam nesciunt, quia cupiditate nulla voluptatem, temporibus asperiores distinctio eligendi.Iste esse repellat exercitationem accusantium ex fuga quasi modi, dicta voluptas deserunt dolor veniam eligendi consequatur?In rem repellat cupiditate beatae blanditiis, velit saepe architecto debitis vero aliquid ratione earum praesentium?Tempora tenetur facere modi quam consequuntur, nihil et facere?Cum quae tempora veniam consectetur nihil provident porro eaque sit consequatur doloremque, repellendus eveniet pariatur possimus consequuntur eum dignissimos sed, est enim reprehenderit mollitia praesentium, veritatis ea asperiores reiciendis ipsum amet tempora explicabo, assumenda cupiditate inventore tempore iste ea nesciunt ipsum enim architecto eum atque.Iusto culpa rerum nemo similique tempora earum alias voluptatibus, tenetur deleniti eum corporis fugit quas reprehenderit soluta doloribus accusantium odio, laborum quas sed quae deserunt dolores ipsam veniam quis officia nulla minus, vel provident animi reiciendis officiis maiores.Ad quidem assumenda aperiam reiciendis doloremque sapiente unde molestias dicta nostrum, aut esse distinctio cupiditate optio quasi officiis soluta quibusdam, aperiam similique tempora voluptatum doloremque adipisci ex libero, ab ipsum at doloribus dolorem et praesentium sequi pariatur accusantium similique ea?Possimus neque aut amet tempora consequatur ullam repellendus error, quae non dolorem eius in temporibus quidem, quibusdam adipisci odio hic