...
%
%Due to the intractable nature of probabilistic inference and sampling, existing generative models do not work well in learning structures subject to combinatorial constraints. 
%
...
%
%that takes expected linear time (\textit{w.r.t.} number of clauses in the formula) to draw valid assignments.
%
...
\begin{algorithm}[t] 
	...
	\Require{
	 ...
	 % \Ensure{ Sample $x$ drawn from $P_{\theta}(X=x|\mathcal{C})$.}
...
% marked out due to page limit
% In practice, since GPUs do not work well with branching operations, we run the algorithm for a fixed number of iterations when the algorithm is fully deployed on GPUs.
...
\appendix
\input{tex/10.apx.prob}
\input{tex/10.apx.time}
\input{tex/11.gradient}
\input{tex/13.expSet}
