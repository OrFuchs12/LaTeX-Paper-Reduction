...
%As depicted in Table \ref{table:different_input}, down-sampling the input images during both training and testing procedures achieves a significant speed-up but inevitably suffers from a dramatic drop in performance due to the loss of information.
...
%These methods mainly focused on reducing the inference time. Tai et al. \cite{tai2015convolutional} further proposed to parameterize the convolutional kernels in a way that naturally enforces the low-rank constraint, and it could train the CNN from scratch with low-rank regularization.
...
%It can be regarded as a complementary method of the existing methods, and has notable potential to work cooperatively with them to further improve the CNN efficiency.
