...
/Author (Juan Jose Garau-Luis, Skylar Eiskowitz, Nils Pachler, Edward Crawley, Bruce Cameron)
...
% ; more complex control processes must be developed to manage these new capabilities, especially
...
% Talk 1 paragraph (I'd say 4-5 sentences) about the problem's scope (What do we need to do?). The idea is to mention the two agents (users and resources), so that we can explain them in the following lines.
...
% Talk about the users (types of services, mobility vs non-mobility, demand over time, etc).
...
% The main difference between these works is the objective function. Specifically, Wang \cite{wangdynamic} proposes a multi-objective formulation to maximize system throughput while minimizing packet delay. Deng \cite{Deng2018} suggests using maximum number of tasks, minimum number of changes in the scheduling scheme and minimum number of sub-divisions in the tasks as figures of merit. He \cite{He2017} contrasts the resources allocated to voice and non-voice services. On a full-system design, Anzalchi \cite{Anzalchi2010} tries to minimize both the unmet system capacity (USC, as a measure for user satisfaction) and total power. Finally, Angeletti \cite{Angeletti2006} proposes a formulation to maximize capacity while ensuring constraint satisfaction. In a similar line \cite{hu2020dynamic} suggests a Q-learning Deep Reinforcement Learning (DRL) model to optimize packet delay, maximum non-voice throughput, and total fairness at the same time. Center lat/lon and number of beams with SA \cite{Camino2014}. [The tasks can be center lat/lon, beam-task scheduling, number of beams, and beam shape]
...
%Paragraph 1: there are gaps in the literatuer, subsystem entabglement bullet 1
...
%Paragraph 3: an architecture needs to look at all subsystems
...
%\section{A title like here is the architecture we consider}
\section{Addressing system-level complexity with an AI-based framework}
\label{sec:4}
% Potential section contents:
% \begin{itemize}
%     \item System overview, why this consideration makes sense
%     \item The demand estimator: what is it and why it needs to rely on learning
%     \item The capacity planner optimizer: what is it and why it needs to rely on AI (here I would consider AI as in the Russell book, i.e., more than just ML and trees)
%     \item The real-time engine: what is it and why it needs to rely on learning
%     \item Watch out these systems interact: Why we need to consider a systems perspective, the importance of designing algorithms from this perspective
% \end{itemize}
...
%remind reader what system challenges are that have not been solved by literature
...
%focus on the third one because the first 2 are obvious (systems interactions)
%now we have abstracted what this third problem means for waht an archiecture must be able to handleLuncertainty and limited compuational time
...
%what are scenarios that make task harder for AI
 
% First paragraph: There is interaction, specifically around the pairs, we summarize it below.
...
%What happens with overestimation and underestimation. What is the effect of overestimation and underestimation on OP/RTE. Possible ways to mitigate that effect. Citations on uncertainty in metaheuristics and uncertainty in RL. 
...
% Which are the events that trigger an RTE action, and which challenges do those pose for AI (e.g., demand spike is out-of-distribution). Worst case planning: Planning for CIR and the consequences: less new users, etc. 
...
% Final paragraph: The challenges of system interaction pose problems to AI and are one of the directions of future work, in the next section we extend on this idea and explain which are the other directions.


%Although we have abstracted these subsystems and the high-level tasks they must accomplish, it is clear there is interaction and feedback among the different blocks. These interactions emerge due to requirements that flow from one block to another. Historically, these key pieces to the system level problem have been overlooked. Instead research tends to focus on specific machine learning problems and not at the system level performances.

%In Figure \ref{fig:BlockDiagram}, it is shown that the interaction between the DE and the RTE and between the DE and the OP relies in the information contained in the user distribution. Specifically, the requirements of the OP and RTE govern the type of time series forecast that must be made. This may include the location of the prediction window, the granularity of predictions, and if the predictions are of a statistic nature (maximum in a window). For the OP, in addition, it is important to consider a much longer forecast horizon as well as uncertainty metrics (a probabilistic forecast) in order to determine reasonable over-provisioning bounds.

%In the right side of Figure \ref{fig:BlockDiagram}, it is clear that the OP and RTE control the same set of variables and, thus, have an intrinsic interaction. Although the objectives for both systems are well defined and significantly different, the RTE needs to have defined limits on when and how to tune a decision to avoid a systems control issue. Specifically, for each use case, the role of the RTE must be accurately defined. Since the real use cases may range from demand changes to hardware failures, the RTE needs to encompass a large amount of algorithms and procedures to accurately resolve each situation.

%The RTE can also quickly amend any predicted service failures.
...
%5.1 problem AI has on subproblem level: literature does this
%5.2 how AI must help in system level challenges
%5.3 what are next step towards automony: more flexibility (orbits as a resource)transfer learning
...
% Introduction paragraph of what this section is about. (challenges in subproblems and interactions, as well as future work).
...
% E.g., given sometimes RTE is given out-of-distribution data it needs to explore better ways to address that problem, it is especiallychallenging for RL, as seen in many numerous examples in the literature. 
