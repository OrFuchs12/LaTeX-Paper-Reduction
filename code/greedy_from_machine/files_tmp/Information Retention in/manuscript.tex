...
/Author (Author(s) Anonymized for Submission)
...
% the argument for why studying information loss in the context of scientific research is important
...
%For example, \citet{javadiHippocampalPrefrontalProcessing2017} found evidence for what parts of the brain are responsible for processing how people navigate. Yet soon, online sites began to claim that satellite navigation systems ``shut down'' the corresponding parts of the human brain \citep{burnettHowMediaWarp2017}. In losing context about how the brain worked, the story of the findings of \citet{javadiHippocampalPrefrontalProcessing2017} rapidly changed.

% Problem of information distortion/loss
...
%\citet{ribeiroMessageDistortionInformation2019} provides compelling experimental evidence of distortion along an information cascade. 
...
%[The more general applicability of this work / why this context is meaningful for more general understandings.]
%Although scientific articles and information from research outputs may be considered an exceptional case, online content about scientific content follow similarly bursty behaviors as other types of online content \cite{zakhlebinDiffusionScientificArticles2020, gilbertWidespreadUnderprovisionReddit2013}. 
...
%A growing body of work demonstrates and argues for the value of multi-platform analyses, looking across platforms to paint a more accurate understanding of how people engage with information online \citep{leskovecMemetrackingDynamicsNews2009,tanLostPropagationUnfolding2016,phadkeManyFacedHate2020,zakhlebinDiffusionScientificArticles2020}. 
...
%\citet{leskovecMemetrackingDynamicsNews2009} for example track memes across news and blog sites; \citet{tanLostPropagationUnfolding2016} examine information propagation across "layers" of activity, looking to a set of source documents, news articles mentioning them, and Facebook posts referencing the relevant news articles; \citet{phadkeManyFacedHate2020} characterize the different information sharing styles of hate groups on Facebook versus Twitter.
...
% Identifying scientific articles to examine
...
% Getting the online mentions of those scientific articles
...
% Collecting the text of the online mentions
...
%Banshal et al: Medical Sciences and Biology are found to account for more than 50\% of all instances in Altmetrics. In terms of coverage, disciplines like Biology, Medical Science and Multidisciplinary Sciences have more than 60% of their articles covered in Altmetrics, whereas disciplines like Engineering, Mathematics and Material Science have less than 25\% of their articles covered in Altmetrics. 
...
%\begin{figure}[t]
%    \centering
%    \includegraphics[width=0.8\columnwidth]{figs/pairwise_matching.png}
%    \caption{Per pair-question, the percent of responses matching the computational measure (x-axis) against the pairwise agreement among responses.}
...
%\citet{ribeiroMessageDistortionInformation2019}'s experimental work helps tease apart two important effects on information that we might observe as a piece of information spreads: loss or distortion that may be due to deprecation over the sequence of a cascade as information becomes further removed from the source (the telephone, or word-of-mouth effect) or due to deprecation because the text is shorter and cannot fit as much information (the summary effect). In non-experimental real-world settings, these effects are more difficult to tease apart because a given mention of a piece of information might be simultaneously subject to both telephone and summary effects. However, \citet{ribeiroMessageDistortionInformation2019} provides a basis for understanding how these effects play out that can inform real-world analyses. 
...
%Using the metadata in the Altmetric LLC dataset, which includes timestamps, we calculated bursts per research article per platform. One exception to the burst calculation process was Twitter, as the Altmetric API only stored tweet IDs and no other metadata. As a result, Twitter bursts were generated using timestamp data from the Twitter API. This resulted in a loss of some tweets, which had been deleted between the time of the Altmetric LLC data collection and the burst generation.
...
%\begin{figure}[t]
%    \centering
%    \includegraphics[width=0.85\columnwidth]{figs/burst_count_hist.png}
%    \caption{Burst count across the 9,765 research articles analyzed in this study.}
%    \label{fig:burstcount_hist}
%\end{figure}
...
%We note that our burst-based framework does not produce information \textit{cascades} but temporally-arranged sequences. As a result, our examinations of information over time is not a clear corollary to the telephone effect from the information cascades in \citet{ribeiroMessageDistortionInformation2019}. While a major motivation for this work is the fact that information tends to distort as it spreads, our objective is not to develop new measures of evaluating distortion arbitrarily. Instead, we focus on information retention: whether key information from a source document is retained or not over time. As a result, rather than distortion from the telephone effect, we seek to understand whether information is retained over time, given that any burst in the sequence may come directly from the source document. 

%Moreover, because platforms have different text length---Twitter having a a 280 character limit versus a much longer blog post---our examination of information across platforms may be a corollary to the summary effect in \citet{ribeiroMessageDistortionInformation2019}. However, platforms also have different constraints about who can post: while Twitter and Facebook have low barriers to posting content, news sites have much higher institutional barriers. As a result, distinguishing between platforms is more accurately a question of platform effects. Given that we are interested in non-experimental settings in which information is at the mercy of such multi-faceted platform effects, this is more appropriate for this study.
...
%We note here two types of bursts of particular interest for understanding information retention over time and across platforms: (1) bursts that are first in a research paper's sequence and (2) co-occurring bursts, which happen on different platforms on the same day for the same research paper.\footnote{For a research paper, there can only be one burst for a given day on a specific platform. As a result, co-occurring bursts inherently involve bursts on different platforms.}
...
% drop in retention is driven by news and blogs
...
\section{Discussion}
% You want to make the discussion self contained enough such that if a non-expert were to read it (and nothing else), it could take away the most important takeaways of the paper.
...
% Propensity towards information loss
...
% Platforms matter --- having more of them suggests better information retention overall
...
% \citep[For effect of diversity on performance in a different context, see][]{bearRoleGenderTeam2011}.
...
% It seems like high information retention is high either when the paper is discussed in fewer platforms or in (some) co-occurring platforms. This suggests that synchronization is required for multi-platform discussion to have high fidelity. Could we think of an explanation for why synchronization is required? Could it be that when multiple platforms are active at the same time you get a diversity of actors and that ensures accuracy?
...
%let's add in the "and de-bias" part after a bit more discussion? maybe if this is accepted, in CRV we can add this in alongside references to work about diversity and bias in collective/crowds
...
% possible NEGATIVE outcomes of sharing our findings
...
% data privacy concerns like, cambridge analytica
...
%The measure and design of our study also lends to other potential impacts of this work to consider. For example, the simplicity of the measure provides a simple basis on which to build on. Moreover, we believe that the burst-based framework we leverage in this project can be extended to further multi-platform analyses, which are pressing to improve our understandings of [information diffusion???] given [how information/interactions/online experiences are always multi-platform???].
% the scale of the project is important: 5 platforms! lots of data!
%[something like: Indeed, one of the main contributions of this piece is its examination of content across \textit{five} platform categories.] As a initial study in this regard, our study is limited in whether it can provide causal claims. However, our analyses provide a broad image of a complex information landscape, thus giving a solid foundation on which further studies might build. In particular, we highlight multiple potential mechanisms for patterns we observe, which---while outside the scope of this project---can be tested in experiments and surveys to more directly develop principles for improving information retention in the multi-platform information era. For example, [this is where we say more about the impact of our findings?].
%We thus see one of the impacts of this piece as a generative one, highlighting the important of multi-platform understandings and pointing to potential and important policy-motivated future work.

\section{Conclusion}
% contributions in terms of:
    % understanding science space. 
    % measurements
    % cross-platform thinking.
% this has impact for Web & Society, what would this mean? this informs practitioners, policymakers, researchers...
