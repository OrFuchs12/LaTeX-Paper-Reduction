...
def generate_dictionaries(counts):
    dict_for_indexes = {}
    dict_for_indexes_inverse = {}
    
    for key, value in counts.items():
        for i in range(1, value + 1):
            new_key = f"{key}{i}"
            dict_for_indexes[len(dict_for_indexes)] = key
            dict_for_indexes_inverse[new_key] = len(dict_for_indexes_inverse)
...
def simulating_vspace_for_prediction(value, key,operator_value,df_copy,models,prob_dict, operators_list):  # value = object_used_on(dict),key = object_used_on_name+index,operator_value = which operator value to use, in the case of vspace its 1,2,3,4
    ...
    if (value['space_between_this_object_and_last_object'] > 10):  # candidate to add vspace
        ...
        df_to_pred = copy.deepcopy(df_copy.drop(['type', 'value', 'object_used_on', 'num_of_object', 'y_gained', 'lines_we_gained', 'binary_class', 'herustica'], axis=0))
        df_to_pred = df_to_pred.T
...
def simulating_combine_paragraphs_for_prediction(value,key_1, key_2,operator_value,df_copy,models,prob_dict, operators_list):  # value = object_used_on(dict),key = object_used_on_name+index,operator_value = which operator value to use, in the case of vspace its 1,2,3,4
    ...
    num_of_object = re.findall(r'\d+', str(key_2))[0]
    # print(object_used_on)
    # print(num_of_object)
    # print(value['space_between_this_object_and_last_object'])
    table_sizes = [1]
...
def simulating_using_vspace(value_of_operator,object_used_on,num_of_object,df_copy,summative_features,dct_of_elements_in_order,count_dict_of_elements): # value = object_used_on(dict),key = object_used_on_name+index,operator_value = which operator value to use, in the case of vspace its 1,2,3,4
    ...
    if('Par' in count_dict_of_elements.keys()):
        num_of_pars = count_dict_of_elements['Par']
    else:
        num_of_pars = 0

    if('Paragraph' in count_dict_of_elements.keys()):
        num_of_paragraphs = count_dict_of_elements['Paragraph']
    else:
        num_of_paragraphs = 0
    if ('Algorithm' in count_dict_of_elements.keys()):
        num_of_algorithms = count_dict_of_elements['Algorithm']
    else:
        num_of_algorithms = 0
    if ('Formula' in count_dict_of_elements.keys()):
        num_of_formulas = count_dict_of_elements['Formula']
    else:
        num_of_formulas = 0
    if ('Enum' in count_dict_of_elements.keys()):
        num_of_enums = count_dict_of_elements['Enum']
    else:
        num_of_enums = 0
    if ('CaptionFigure' in count_dict_of_elements.keys()):
        num_of_captionfigures = count_dict_of_elements['CaptionFigure']
    else:
        num_of_captionfigures = 0

    if ('CaptionTable' in count_dict_of_elements.keys()):
        num_of_captiontables = count_dict_of_elements['CaptionTable']
    else:
        num_of_captiontables = 0

    if ('Figure' in count_dict_of_elements.keys()):
        num_of_figures = count_dict_of_elements['Figure']
    else:
        num_of_figures = 0

    if ('Table' in count_dict_of_elements.keys()):
    ...
    if (num_of_pars + num_of_paragraphs != 0):
        avg_num_of_words_from_pars = sum_of_words_from_pars / (
                num_of_pars + num_of_paragraphs)
    ...
    # df_copy = df_copy.T
    # df_copy.to_csv('example_test_for_me_1_4_2023_2.csv')
    # sys.exit()
...
def simulating_using_enum_operator(value_of_operator,object_used_on,num_of_object,df_copy,summative_features,dct_of_elements_in_order,count_dict_of_elements): # value = object_used_on(dict),key = object_used_on_name+index,operator_value = which operator value to use, in the case of vspace its 1,2,3,4
    ...
    if(cancel_simulation):
        return
    bool_exp = False
    if ('Par' not in count_dict_of_elements.keys()):
        num_of_pars_new = 0
        bool_exp = True
    else:
        num_of_pars_new = count_dict_of_elements['Par']
    if(num_of_pars_new == 8):
        return
    else:
        if(bool_exp == True):
            count_dict_of_elements['Par'] = 0
...
def simulating_using_combining_pars_operator(value_of_operator,object_used_on,num_of_object,df_copy,summative_features,dct_of_elements_in_order,count_dict_of_elements): # value = object_used_on(dict),key = object_used_on_name+index,operator_value = which operator value to use, in the case of vspace its 1,2,3,4
    ...
    for key in new_dct_of_elements_in_order.keys():
        ...
        if(new_key_for_par == 'Par'):
            counter+=1
            if key in dict_for_indexes_inverse.keys():
            #get his represented number in the csv
                df_copy.at['page' + str(dict_for_indexes_inverse[key]), 'Doc0'] = new_dct_of_elements_in_order[key]['page']
                df_copy.at['column' + str(dict_for_indexes_inverse[key]), 'Doc0'] = new_dct_of_elements_in_order[key]['column']
                df_copy.at['start_y' + str(dict_for_indexes_inverse[key]), 'Doc0'] = new_dct_of_elements_in_order[key]['start_y']
    ...
    for j in objects_after_chosen_object:
        ...
        if (object_without_index == 'Par' or object_without_index == 'Paragraph' or object_without_index == 'Enum'):  # to move a par,paragraph,enum to a before column we will need at least 3 rows of free space, so we will need to check it
            ...
            #print(j)
            if indexx == 0:
                before_object_key = new_key_to_remember
            else:
                before_object_key = objects_after_chosen_object[indexx - 1]
            get_index_representing_before_object_in_csv = dict_for_indexes_inverse[before_object_key]
            if (indexx != len(objects_after_chosen_object) - 1):
    ...
    last = -1
    while not found:
        last_element = list(new_dct_of_elements_in_order)[last]
        if last_element in dict_for_indexes_inverse.keys():
            found = True
            get_index_of_last_element = dict_for_indexes_inverse[last_element]
        else:
...
#             to_check = None
#         else:
#             to_check = tree[to_check][1][7]
#             depth +=1
#     return depth

def get_all_leafs(tree):
    leafs = [key for key, value in tree.items() if value == []]
    return leafs

def check_valid_operator(list_of_used_operators, new_operator):

    for operator in list_of_used_operators:
        if operator[0] == '1': # if the operator is vspace
            if operator[0] == new_operator[0] and operator[2] == new_operator[2] and operator[3] == new_operator[3]:
                return True
        
        else: # other operators
            if operator[0] == new_operator[0] and operator[2] == new_operator[2]:
                return True
...
def generate_search_graph(depth_graph, file_name,last_pages_pdf, models, prob_dict,bib_path, operators_list= None):
    ...
    #models = reload_models.load_regression_models_hist("pdf_extraction\\adi_comparing\\regression_models_hist")
    ...
    while current_depth < DEPTH_OF_TREE:
        depth_added = False
        to_expand_next_iteration = get_all_leafs(dict_based_tree_levels)

        for current in to_expand:

            for node in dict_based_tree_levels[current]:
                
                if node[2] == 1:
                    dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_vspace(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])
                    if dct_of_elements_in_order == -1:
                        continue
                    
                elif node[2] == 2:
                    dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_figure_reduction(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])

                elif node[2] == 3:
                    dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_algorithm_reduction(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])

                elif node[2] == 4:
                    try:
                        dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_enum_operator(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])
                    except Exception as e:
                        continue

                elif node[2] == 5:
                    try:
                        dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_paragraph_tag_removal_operator(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])
                    except Exception as e:
                        continue

                elif node[2] == 6:
                    dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_combining_pars_operator(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])

                elif node[2] == 7:
                    dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_table_reduction(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])

                else:
                    print("not found")
                

                node[6] = df_copy.copy(deep=True)
                base_df = df_copy.copy(deep=True)
                to_remember[node[7]] = (copy.deepcopy(summative_features), copy.deepcopy(dct_of_elements_in_order), copy.deepcopy(count_dict_of_elements))
                #to_expand.append(node[7])
                depth = node[7]
                counter = 0
                found_enum = False
                key_1 = 0
                ...
                for key in dct_of_elements_in_order.keys(): #preforming vspace on all elements
                    ...
                    for iteration in range(1,5):
                        ...
                        used_operators = copy.deepcopy(node[8])
                        candidate = (str(type), str(iteration), str(index_for_object[object_used_on]), num_of_object)
                        if check_valid_operator(used_operators, candidate):
                            continue
                    ...
                    if(key.startswith('Figure')):
                        for iteration in range(0,5):
                            ...
                            used_operators = copy.deepcopy(node[8])
                            candidate = (str(type), str(value_of_operator), num_of_object)
                            if check_valid_operator(used_operators, candidate):
                                continue
                            used_operators.append(candidate)
        ...
        visited_nodes.extend(to_expand)
        to_expand = list(set(to_expand_next_iteration[:]) - set(visited_nodes))
        ...
        if depth_added:
            ...
            print("can not build tree in depth:", DEPTH_OF_TREE)
            break
    ...
    for current in to_expand:
        ...
        for node in dict_based_tree_levels[current]:
            ...
            if node[2] == 1:
                dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_vspace(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])
                if dct_of_elements_in_order == -1: # new if
                    continue
            elif node[2] == 2:
                dct_of_elements_in_order,count_dict_of_elements,summative_features,df_copy = simulating_using_figure_reduction(node[3],node[4],node[5],node[6],to_remember[current][0],to_remember[current][1],to_remember[current][2])
    ...
    #print(dict_based_tree_levels.keys())
    print("total number of nodes in tree:", len(dict_based_tree_levels.keys()))

    # print("depth 0:", len(dict_based_tree_levels[1]))
    # print("depth 1:",len(dict_based_tree_levels[2]))
